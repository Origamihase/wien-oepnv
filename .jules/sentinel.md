## 2026-05-06 - GitHub Actions Template Injection in Shell Run Blocks
**Vulnerability:** `${{ inputs.requirements }}`, `${{ github.repository }}` and `${{ env.SITE_BASE }}` were interpolated **directly into bash run scripts** in `.github/actions/install-deps/action.yml`, `.github/workflows/seo-guard.yml`, and `.github/workflows/build-feed.yml`. GitHub renders `${{ … }}` expressions *before* the shell parses the script, so the bash quoting around the value (`"…"`) is irrelevant — any `;`/`$()`/backtick in the expanded value executes as a command. The current sources happen to be format-constrained (GitHub-supplied slug, hardcoded URL, internal action input), so no live exploit, but it's a classic **latent CI injection primitive**: any future change that lets user input flow into one of those expressions silently becomes RCE on the runner with `contents: write` and the GITHUB_TOKEN.
**Learning:** This is the canonical "Untrusted Input in Bash Run" pattern from GitHub's own security hardening guide, and Sentinel-style audits should grep for `\$\{\{` *inside `run:` blocks* (not in `env:`/`if:`/`with:` contexts, which are evaluated by the runner itself, not by bash). The `env:` block is the correct landing zone for any context expression that has to reach a shell — the runner sets the env var with proper escaping, and the shell dereferences it with bash's own (safe) rules.
**Prevention:** Never write `${{ … }}` inside a multi-line `run: |` block. Always declare the expression as a step-level `env:` entry and reference it from bash as `"$VARNAME"`. The GitHub-Actions-injection grep pattern is `awk '/run: *\|/{flag=1; next} flag && /^[[:space:]]*[^[:space:]]/ && !/^[[:space:]]/{flag=0} flag && /\$\{\{/'` over `.github/workflows/*.yml` and `.github/actions/**/*.yml`.
