## 2026-05-16 - `_SENSITIVE_HEADER_PARTIALS` Third-Sibling Drift Closure (Canonical-Floor Alignment Across Three Redaction Codepaths): The HTTP-Header Redirect-Strip Path's Missing `csrf`/`xsrf`/`samlart`/`nonce`/`state` Partial Substrings — Aligns The Cross-Origin Redirect Header Stripping (`_strip_sensitive_headers` → `_is_sensitive_header`) With The URL-Query-Param Path (PR #1531) And The Log-Message Path (PR #1532) For The SAML 2.0 / CSRF / XSRF / WordPress / Generic-Nonce Credential Family

**Vulnerability:** The 2026-05-16 SAML/CSRF/WordPress drift family was closed across TWO of three redaction codepaths in successive PRs: PR #1531 closed the **structured-URL path** (`_SENSITIVE_QUERY_KEYS` + `_SENSITIVE_KEY_SUBSTRINGS` in `src/utils/http.py`, used by `_sanitize_url_for_error` for operator error-log redaction and `_strip_sensitive_params` for cross-origin redirect URL stripping); PR #1532 closed the **raw-text log path** (`_keys` + `_header_keys` regex alternations in `src/utils/logging.py`, used by the canonical log sanitizer `sanitize_log_message` and its wrappers `sanitize_log_arg` / `clean_message` / `_sanitize_log_detail`). This round closes the **THIRD parallel codepath**: the **HTTP-header redirect-strip path** at `src/utils/http.py:_SENSITIVE_HEADER_PARTIALS` (used by `_is_sensitive_header()` and called from `_strip_sensitive_headers()` to strip sensitive headers BEFORE following a cross-origin redirect). Pre-fix, the partials set was missing FIVE entries (`csrf`, `xsrf`, `samlart`, `nonce`, `state`) — every leaked bare `X-CSRF: <value>` / `CSRF: <value>` / `X-XSRF: <value>` / `XSRF: <value>` / `SAMLArt: <value>` / `X-SAMLArt: <value>` / `RelayState: <value>` / `X-WP-Nonce: <value>` / `X-Nonce: <value>` / `X-OAuth-State: <value>` header passed through `_strip_sensitive_headers` with the credential value PRESERVED and sent to the cross-origin redirect target host. The token-suffixed forms (`X-CSRF-Token`, `X-XSRF-TOKEN`, `X-WP-Nonce-Token`) were ALREADY covered via the existing `token` partial substring; only the BARE forms slipped through. The drift root cause: `_SENSITIVE_HEADER_PARTIALS` was NEVER updated to align with the canonical floor established by `_keys` and `_header_keys` log regex sets (which DO include `nonce|state` since the original log-redaction round, plus `samlart|csrf|xsrf` added in PR #1532). The TWO log-redaction sites were synchronised, but the HTTP-header redirect-strip site drifted apart.

**Threat model:** A cross-origin redirect (host change, scheme downgrade, port change, or any non-safe-upgrade combination) triggers `_strip_sensitive_headers` to evaluate each outbound header. Headers NOT flagged as sensitive by `_is_sensitive_header` are PRESERVED and sent to the redirect target. If the redirect target is malicious (DNS rebinding, hostile 3xx, compromised CDN, allow-listed-host open-redirect chain, attacker-controlled subdomain), the target gains the user's CSRF token / SAML artifact / WordPress nonce / OAuth state for the credential's validity window. Per-credential severity:

* **X-WP-Nonce (HIGH for WordPress REST-API-consuming applications)**: WordPress REST API authentication uses the `X-WP-Nonce` HTTP header per [the WP REST API documentation](https://developer.wordpress.org/rest-api/using-the-rest-api/authentication/). A leaked X-WP-Nonce within its ~24-hour validity window enables state-changing-action replay against the WordPress site (delete-post, modify-user, install-plugin, approve-comment) if paired with the user's session cookie. The leak surface: any WP-using SP that follows a third-party redirect (e.g., admin AJAX calling an external service that returns a 3xx to an attacker-controlled host).

* **Bare X-CSRF / X-XSRF (MEDIUM-HIGH)**: Custom internal APIs that use bare-form CSRF / XSRF tokens (without `-Token` suffix). Session-lifetime replay surface (minutes to hours, framework-dependent). Less common than the standard `X-CSRF-Token` form (already covered) but real in legacy / custom enterprise systems.

* **SAMLArt / X-SAMLArt (HIGH, but rare in headers)**: SAML 2.0 Artifact in HTTP header form. The canonical SAML 2.0 transport is URL query params (already closed by PR #1531) or POST body, but some custom SAML SP shims use a header form for internal RPC-style flows. A 5-min ARS-resolvable bearer credential — full SAML assertion retrieval via the IdP's Artifact Resolution Service within the validity window.

* **RelayState / X-OAuth-State (MEDIUM)**: SP-supplied state preservation strings (SAML 2.0 §3.4.4 / OAuth 2.0 §4.1.1). Not credentials per se but context leaks (post-auth landing URL, tenant identifiers, feature flags) enabling targeted phishing reconnaissance.

* **X-Nonce / generic nonce headers (MEDIUM)**: Custom protocols that use bare `X-Nonce` for replay-protection challenges. Severity bounded by protocol-specific nonce validity windows.

End-to-end PoC verified pre-fix: 16 header-name variants across 5 credential families (4 CSRF + 3 XSRF + 3 SAMLArt + 3 nonce + 3 state) all LEAKED through `_is_sensitive_header()` pre-fix and were preserved through cross-origin `_strip_sensitive_headers` in the integration tests.

**Sites closed (1 set extension covering 1 consuming function):**

* `src/utils/http.py:_SENSITIVE_HEADER_PARTIALS` — five new partial-substring entries appended to the canonical frozenset (alongside the existing 18 entries for `token`/`key`/`secret`/`password`/`passphrase`/`credential`/`signature`/`session`/`cookie`/`auth-`/`authorization`/`client-id`/`client-secret`/`access-token`/`access-key`/`access-id`/`assertion`):
    ```python
    _SENSITIVE_HEADER_PARTIALS = frozenset({
        # ... existing 18 entries ...
        "csrf",          # X-CSRF, CSRF, custom *-csrf-* headers
        "xsrf",          # X-XSRF, XSRF, Angular custom XSRF
        "samlart",       # SAMLArt, X-SAMLArt (header form of artifact)
        "nonce",         # X-WP-Nonce, X-Nonce, custom challenge nonces
        "state",         # RelayState, X-OAuth-State, custom state
    })
    ```
    The single consuming function (`_is_sensitive_header`) iterates the set via `any(partial in normalized for partial in _SENSITIVE_HEADER_PARTIALS)`. The single set update closes the cross-origin redirect strip path with zero additional code changes. Zero changes to `_is_sensitive_header`, zero changes to `_strip_sensitive_headers`, zero C901 complexity impact, zero per-call performance overhead (the frozenset is iterated O(N) per header check, N grows from 18 to 23 — negligible).

**Severity:** **MEDIUM-HIGH** — the X-WP-Nonce case (HIGH for WordPress users) and SAMLArt-in-header case (HIGH but rare) are the worst paths; the bare CSRF / XSRF cases (MEDIUM-HIGH) and nonce / state cases (MEDIUM) are session-lifetime-bounded. The propagation is to ANY cross-origin redirect following — a single misconfigured open-redirect or hostile 3xx target leaks the credential. CodeQL / Semgrep do not flag this drift because their default rules focus on the well-known `Authorization` / `Cookie` / `X-API-Key` family and don't enumerate the bare SAML / CSRF / XSRF / WP / OAuth state header forms.

**Fix:** Single-edit change: append `"csrf"`, `"xsrf"`, `"samlart"`, `"nonce"`, `"state"` as partial-substring entries to `_SENSITIVE_HEADER_PARTIALS`. The partial-substring approach (substring match within the lowercase header name via `any(partial in normalized for partial in _SENSITIVE_HEADER_PARTIALS)`) handles every hyphenated / underscored / X-prefixed / case-shifted variant in a single addition per the existing convention (`token` matches `Bearer-Token`, `api_token`, `X-API-Token`, etc.).

Zero false-positive risk for cross-origin redirect stripping: over-stripping a benign header is safe (the alternative is leaking credentials to potentially-malicious redirect targets). The added partials are highly specific to credential / state contexts — "csrf" / "xsrf" / "samlart" / "nonce" don't appear as substrings of common transport-layer headers. The `state` partial may match `X-Trace-State` (W3C Trace Context distributed-tracing header), but stripping that on cross-origin redirect is a minor diagnostic-visibility cost outweighed by the credential-leak prevention.

**Tests added:** `tests/test_sentinel_sensitive_header_partials_drift.py` — 59 tests across 6 categories:
  1. Per-family attribution PoCs (17 header-name variants across CSRF/XSRF/SAMLArt/Nonce/State families): proves the bare and X-prefixed forms are flagged as sensitive.
  2. Membership invariant: pins the 5 new entries in `_SENSITIVE_HEADER_PARTIALS` against future regression.
  3. Regression guards (19 cases): existing token-suffixed forms (Authorization / Proxy-Authorization / X-Api-Key / X-CSRF-Token / X-XSRF-TOKEN / Cookie / Set-Cookie / X-Saml-Assertion / etc.) continue to be flagged — proves the additive change is non-destructive.
  4. Negative cases (15 cases): truly benign headers (Content-Type / Accept / User-Agent / Cache-Control / X-Forwarded-For / etc.) are NOT flagged — proves the new partials don't over-redact common transport headers.
  5. End-to-end cross-origin redirect strip PoCs (3 cases): `_strip_sensitive_headers` removes X-WP-Nonce / X-CSRF / SAMLArt headers from the headers dict when a cross-origin redirect is detected.
  6. Canonical-floor alignment invariant: the three sibling redaction codepaths (`_SENSITIVE_QUERY_KEYS`, `_keys`/`_header_keys` log regex, `_SENSITIVE_HEADER_PARTIALS`) now share the same SAML/CSRF/WP keyword family — a future regression in any one path that removes a keyword without removing it from the others fails this invariant fast.

**Learning:** The codebase has THREE complementary redaction codepaths for sensitive credentials in URL/header/log surfaces: (1) `_SENSITIVE_QUERY_KEYS` for structured URLs parsed by urlparse, (2) `_keys`/`_header_keys` regex for raw log text, and (3) `_SENSITIVE_HEADER_PARTIALS` for HTTP headers on cross-origin redirects. Closing a sensitivity-family drift requires updating ALL THREE paths in sequence. The 2026-05-16 SAML/CSRF/WordPress family closure took three rounds (PR #1531 → #1532 → this round) to align all three paths. **The canonical-floor invariant test added in this round** (`test_canonical_floor_alignment_across_three_paths`) explicitly cross-references the three set memberships to detect future drift between them — adding a keyword to one path without the others now fails the invariant immediately. **The partial-substring vs exact-match distinction matters per path**: `_SENSITIVE_QUERY_KEYS` uses exact-match-on-normalised-key (high signal-to-noise; the URL parser provides clean key boundaries), `_keys`/`_header_keys` use regex alternation with structural anchors (`=`, `:`, `"`), and `_SENSITIVE_HEADER_PARTIALS` uses partial-substring (broadest coverage; safest default for cross-origin redirect stripping where over-stripping is preferable to under-stripping). All three paths now share the same keyword family for the SAML/CSRF/WP drift, but each path's match semantics is tailored to its consuming function's input shape and FP risk tolerance.

**Next-round candidates (named-but-deferred):** **OAuth 1.0 `oauth_verifier`** — bare `oauth_verifier` and similar OAuth 1.0 specific parameters not currently covered across any of the three paths. **WordPress action-specific nonces** precise attribution beyond bare `nonce` (e.g., `_ajax_nonce` is covered via `nonce` substring but lacks per-action context attribution). **`X-Forwarded-Authorization`** — proxy-forwarded auth headers (some proxies preserve the original Authorization under this name). Not currently in any path. **The five remaining auth-scheme deferred candidates** (Mutual / vapid / SCRAM / Digest / AWS4-HMAC-SHA256 — all multi-field bodies) remain pending body-structure-aware framework.
...
1. **Surfaced verbatim in operator error-log streams** via `_sanitize_url_for_error` — pre-fix the credentials reached CI logs, GitHub PR comments, Datadog / Splunk / ELK ingest, pre-commit hook output, and any sink that consumed the sanitised URL for diagnostic display.

2. **Was CARRIED THROUGH cross-origin redirect chains** via `_strip_sensitive_params` (called from `_process_redirect` at `src/utils/http.py:1817` BEFORE following a redirect) — pre-fix the credentials landed in the redirect target's access logs, where a malicious target (DNS rebinding, hostile 3xx, compromised CDN, open redirect on an allow-listed host) gained the user's SAML / CSRF / WP context for the credential's validity window.

End-to-end PoC verified pre-fix across all 9 case + separator variants (`SAMLArt` / `RelayState` / `_csrf` / `csrf` / `CSRF` / `xsrf` / `XSRF-TOKEN` / `_wpnonce` / `wpnonce`): only `XSRF-TOKEN` (which normalises to `xsrftoken` containing the `token` substring) was redacted. The remaining 8 variants leaked verbatim through BOTH consuming functions.

**Threat model:** Five-credential class shares the same root cause (incomplete `_SENSITIVE_QUERY_KEYS` set, no overlapping substring entry) and shares the same fix (single set extension covers BOTH consuming functions). Per-credential severity:

* **`SAMLArt` (HIGH severity)**: SAML 2.0 Artifact per OASIS SAML 2.0 §3.6.4 (HTTP-Artifact binding). A 5-minute-validity bearer credential pointing at a stored SAML assertion on the IdP's Artifact Resolution Service (ARS). An attacker who recovers `SAMLArt` within its validity window can resolve to the full `SAMLResponse` (the user's identity assertion, signed by the IdP and trusted by every SP in the SSO federation) via a single back-channel call from the attacker to the IdP. Blast radius: full user authentication on every IdP-trusted SP for the assertion's lifetime (typically 1 hour). The redirect-strip leak path is the worst case — a malicious redirect target gains the artifact and resolves it before the legitimate SP receives the ACS POST.

* **`_csrf` / `csrf` / `CSRF` (MEDIUM-HIGH severity)**: Bare CSRF token names (Spring Security's `_csrf` GET-based protection, generic framework CSRF params). Replayable within session lifetime (typically 30 minutes to 8 hours, framework- and config-dependent). Enables state-changing-action replay if the attacker also has session cookie access (e.g., via a separate XSS vector, cookie-jar-share misconfiguration, or stolen authenticated browser session).

* **`xsrf` (MEDIUM-HIGH severity)**: Angular's default XSRF cookie / header name in bare form (without the `-token` suffix). The `XSRF-TOKEN` form is ALREADY covered via the `token` substring match, but the bare `xsrf` form appears in some legacy AJAX bootstrap flows and custom Angular middleware configurations. Same replay surface as `_csrf` above.

* **`RelayState` (MEDIUM severity)**: SAML 2.0 SP-supplied state preservation string per §3.4.4. Often contains the post-auth landing URL (e.g., `?dest=/admin/dashboard`) or serialised session context (tenant IDs, user identifiers, feature flags). Not a credential per se but high-value reconnaissance for targeted phishing — the attacker learns the user's pre-auth destination and can craft a convincing post-auth phishing page.

* **`_wpnonce` (MEDIUM severity)**: WordPress's per-action nonce protection. Validity: typically 24 hours (WP's default `DAY_IN_SECONDS / 2` lifetime). A leaked `_wpnonce` within the window enables arbitrary WP action replay (delete-post, install-plugin, modify-user, approve-comment) if paired with session cookie access. Mitigated by WP's nonce rotation policy on plugin / theme / core updates.

Real-world emission patterns include: SP-initiated SSO error logs landing `SAMLArt` + `RelayState` in nginx / Apache request logs and application error sinks; CSRF protection failure logs emitting `_csrf` verbatim; WordPress WP-DEBUG / WP-CLI logs emitting `_wpnonce`; cross-origin redirect chains carrying the original query string (including sensitive params) to the redirect target's access logs; pre-commit secret scanner failure output emitting the URL verbatim for operator triage; Postman / Insomnia / Bruno HAR-style request export JSON; Datadog / Splunk / ELK ingest of unredacted application logs from microservices that log the inbound request URL via standard logger formatters.

**Sites closed (1 set extension covering 2 consuming functions):**

  * `src/utils/http.py:_SENSITIVE_QUERY_KEYS` — five new exact-match entries appended to the canonical frozenset (alongside the existing 60+ entries for OAuth / OIDC / AWS / session / vendor-specific tokens):
    ```python
    _SENSITIVE_QUERY_KEYS = frozenset({
        # ... existing 60+ entries ...
        # SAML 2.0 query parameters per OASIS SAML 2.0 §3.4.4 / §3.6.4
        "samlart",       # SAML 2.0 Artifact (5-min ARS-resolvable)
        "relaystate",    # SAML 2.0 SP state preservation
        # CSRF / XSRF tokens (bare normalised form):
        "csrf",          # Spring _csrf normalised; bare csrf
        "xsrf",          # Angular xsrf cookie (bare form)
        # WordPress nonce protection (per-action):
        "wpnonce",       # WP _wpnonce normalised
    })
    ```
    `_normalize_key` strips non-alphanumeric characters before matching, so `SAMLArt`, `_csrf`, `CSRF`, `XSRF-TOKEN`, `_wpnonce` all reduce to their bare normalised form. A single exact-match entry per key covers every real-world casing / separator variant.

  * BOTH consuming functions (`_sanitize_url_for_error` for error-log redaction, `_strip_sensitive_params` for cross-origin redirect protection) iterate `_SENSITIVE_QUERY_KEYS` directly via `if normalized in _SENSITIVE_QUERY_KEYS or any(s in normalized for s in _SENSITIVE_KEY_SUBSTRINGS)`. The single set update closes BOTH leak paths with zero additional code changes. Zero changes to the consuming functions, zero C901 complexity impact, zero per-call performance overhead (the frozenset lookup is O(1)).

**Severity:** **MEDIUM-HIGH** — the SAML 2.0 Artifact case (HIGH severity) is the worst path: a 5-minute-validity Artifact carried through a cross-origin redirect to a malicious target enables full SAML assertion retrieval via the IdP's Artifact Resolution Service, granting full user authentication on every IdP-trusted SP for the assertion's lifetime. The CSRF / XSRF cases (MEDIUM-HIGH) are bounded by session lifetime and require additional session cookie access. The RelayState / _wpnonce cases (MEDIUM) are context leaks and rate-limited replays respectively. The redirect-strip leak path is the highest-severity vector across all five classes because the malicious target is by definition adversarial. CodeQL / Semgrep do not flag this drift because their default rules cover the well-known token / secret / api_key substring patterns but NOT the SAML / WP nonce / bare CSRF / bare XSRF specific identifiers — these require per-spec knowledge of the framework / protocol literals.

**Fix:** Single-edit change:
1. Append `"samlart"`, `"relaystate"`, `"csrf"`, `"xsrf"`, `"wpnonce"` as exact-match entries to `_SENSITIVE_QUERY_KEYS`. The exact-match contract (vs substring) narrows the false-positive surface: only keys whose normalised form is exactly one of the five names match. Benign keys like `postal_state` (normalised `postalstate`), `sample` (normalised `sample`), `related` (normalised `related`), typo'd `crsf` (normalised `crsf`), `serif` (normalised `serif`) do NOT match.

Zero changes to `_sanitize_url_for_error`, `_strip_sensitive_params`, `_normalize_key`, or any other consuming code path — the set extension is the single source of truth and both functions iterate it uniformly.

**Tests added:** `tests/test_sentinel_sensitive_query_key_drift.py` — 33 tests across 7 categories:
  1. Error-log redaction PoCs (9 case + separator variants × per-credential): `SAMLArt` / `RelayState` / `_csrf` / `csrf` / `CSRF` / `xsrf` / `XSRF-TOKEN` / `_wpnonce` / `wpnonce` — proves the credentials are redacted in error-log output and benign companion params (`other=visible`) survive (regression guard against over-redaction).
  2. Cross-origin redirect-strip PoCs (7 case + separator variants): `_strip_sensitive_params` REMOVES the sensitive params entirely (not just redacts) so they are not carried through to the redirect target; benign `other=visible` companion survives; the sensitive key name itself does NOT appear in the stripped URL.
  3. Combined PoCs (2 tests): real-world SP-initiated SSO URLs carrying `SAMLArt` + `RelayState` side by side — both must be redacted in error logs AND stripped in cross-origin redirects.
  4. Regression guards (8 cases): existing `_SENSITIVE_QUERY_KEYS` entries (`token` / `access_token` / `password` / `client_secret` / `api_key` / `session_id` / `authorization` / `signature`) continue to redact — proves the fix is additive and doesn't break prior behaviour.
  5. Negative cases (5 cases): benign URLs with similar-but-distinct query parameter names (`postal_state`, `sample`, `related`, typo'd `crsf`, `serif`) must NOT have their query values redacted — proves the exact-match-on-normalised-key contract narrows the false-positive surface.
  6. Membership invariant: the 5 new entries are present in `_SENSITIVE_QUERY_KEYS` — pins membership against future regressions.
  7. Header equivalents regression guard: `X-CSRF-Token` / `X-XSRF-TOKEN` / `X-CSRFToken` HTTP headers continue to be flagged by `_is_sensitive_header` — proves the query-param-only update doesn't accidentally affect the header-redaction path.

**Learning:** The `_SENSITIVE_QUERY_KEYS` exact-match set + `_SENSITIVE_KEY_SUBSTRINGS` substring set form a TWO-LAYER redaction contract for URL query parameters. The substring layer catches token-suffix / token-prefix variants (`csrf_token`, `access_token`, `client_secret_v2`) via the small `token` / `secret` / `key` substring family. The exact-match layer catches BARE forms whose normalised name is a specific framework / protocol literal (`samlart`, `csrf`, `xsrf`, `wpnonce`) that has NO token-/secret-/key-suffix and therefore bypasses the substring layer entirely. **The single-line additions extend the EXACT-match layer without changing the substring layer** — this preserves the existing FP surface (the substring layer's broad coverage is unchanged) while adding precise coverage for the bare-form gap. The contract-invariance pattern: TWO consuming functions iterate the same canonical set, so a single set extension closes BOTH leak paths with zero per-function code changes — this is the canonical "single source of truth" pattern that the codebase enforces for redaction contracts. **The bare-form-vs-suffixed-form distinction is the key insight**: token-suffixed CSRF / XSRF variants (`csrf_token`, `XSRF-TOKEN`) match the `token` substring layer, but the bare forms (`_csrf` normalised to `csrf`, bare `xsrf`) need explicit exact-match entries. The same pattern applies to WordPress nonces (`_wpnonce` normalised to `wpnonce`) and SAML literals (`SAMLArt` normalised to `samlart`) — the framework-/protocol-specific bare literals always need exact-match entries because they don't carry the token/secret/key suffix that the substring layer expects.

**Next-round candidates (named-but-deferred):** **OAuth 1.0 `oauth_token_secret` / `oauth_verifier`** — OAuth 1.0 specific tokens; `oauth_token_secret` is already covered via `token` substring, but `oauth_verifier` (the temporary verifier code for OAuth 1.0 PIN-based flow) normalises to `oauthverifier` and doesn't match any substring. Rare in modern apps (OAuth 2.0 dominates) but worth completing. **WordPress action-specific nonces (`_ajax_nonce`, `_wpnonce_action`)** — normalised forms not currently covered. **`SigAlg`** (SAML 2.0 XML signature algorithm parameter) — not a credential but reveals the signing algorithm. Lower priority. **AWS Sig V4 multi-field `AWS4-HMAC-SHA256 Credential=AKIA.../...,SignedHeaders=...,Signature=<64-hex>`** — still deferred pending body-structure-aware framework (the `Credential` field's inner `AKIA<16>` IS already detected by `_AWS_ID_RE`, but the full Sig V4 attribution is lost). **The five other auth-scheme deferred candidates** (Mutual / vapid / SCRAM / Digest / AWS4-HMAC-SHA256 — all multi-field) remain pending body-structure-aware framework architecture.

**Marker:** SENTINEL_QUERY_KEY_SAML_CSRF_DRIFT.

## 2026-05-16 - HOBA Detector Drift (RFC 7486 HTTP Origin-Bound Authentication, Sibling Of Token-Scheme Round): The Next Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-16 Token-Scheme Round Closed In Full — A Leaked `Authorization: HOBA result="<KID>.<challenge>.<nonce>.<signature>"` In Committed Source Carried Two Orthogonal Failure Modes (Per-Segment Entropy Splitting With Attribution Drift For Realistic Bodies, Silent Undetection For All-Letter Per-Field Bodies) That Lost The HOBA-Specific Recovery Surface (Rotate The Client's HOBA Key Pair, Reissue The `KID`, Audit For Replay Within The Server's (Challenge, Nonce) Freshness Window) Entirely

**Vulnerability:** The 2026-05-16 Token-Scheme round (`security(secret-scanner): add Token-Scheme detector per GitHub REST API`) listed HOBA as one of the next-round candidates ("rare in practice but a complete auth-scheme enumeration would include it") alongside Mutual (very rare), vapid (multi-field), SCRAM-SHA-1/256 (multi-field), Digest (multi-field), and AWS4-HMAC-SHA256 (multi-field). Of those, HOBA is the only one whose body shape — a quoted dot-separated 4-tuple `result="<KID>.<challenge>.<nonce>.<signature>"` per RFC 7486 §3 — fits a single-regex detector pattern (Mutual is multi-field key=value; vapid / SCRAM / Digest / AWS4-HMAC-SHA256 are all multi-field requiring body-structure-aware framework). Pre-fix `src/utils/secret_scanner.py` had NO direct HOBA detector — the only coverage for a leaked `Authorization: HOBA result="..."` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`), which matched the **individual segments BETWEEN dots** (because `.` is OUTSIDE the entropy alphabet `[A-Za-z0-9+/=_-]`), splitting one logical credential into multiple findings AND losing the HOBA-specific attribution. The five existing auth-scheme detectors (Bearer / Basic / Negotiate / NTLM / Token) closed their respective auth-schemes, but NOT HOBA — the IANA-registered HTTP authentication scheme for client-asserted public-key authentication WITHOUT TLS client certificates (RFC 7486). HOBA's structural distinction is the **parameterised quoted-string body shape** (`result="<KID>.<challenge>.<nonce>.<signature>"`) — the only HTTP auth-scheme in the single-body family with this shape, all the existing siblings using the simple `<scheme> <body>` form. Two orthogonal failure modes resulted: **(1) Per-segment entropy splitting with attribution drift** — a realistic HOBA result with mixed character classes (KID = 43-char base64url SHA-256 hash, challenge = 16-32 char base64url nonce, nonce = 16-32 char base64url freshness anchor, signature = 86-char base64url ECDSA P-256 sig or 342-char base64url RSA-2048 sig) is split into up to FOUR separate `Hochentropischer Token-String` findings (one per dot-separated segment that exceeds the 24-char entropy floor), losing the HOBA-specific attribution AND consolidation. Pre-fix incident-response triage had to GUESS whether the leaked dotted entropy structure was a JOSE JWT (revoke at the issuing IdP), HOBA result (rotate the client's HOBA key pair at the HOBA server and reissue a new `KID`; audit for replay within the (challenge, nonce) freshness window which is server-specific but typically 5-60 seconds), or some other opaque dotted secret. **(2) Silent undetection** — all-letter per-field bodies trip the `candidate.isalpha()` skip in the entropy fallback's per-segment loop (added to suppress LongCamelCaseClassName false positives). HOBA result strings whose 4 fields happen to be all-letter (rare in practice but possible for hand-crafted test fixtures, CTF challenges, or pathological base64url encodings where every byte happens to map to a letter) are SILENTLY UNDETECTED — the full result body `<all-letters>.<all-letters>.<all-letters>.<all-letters>` is composed entirely of `[A-Za-z]` plus dots; the dots don't break `isalpha()` because `isalpha()` is applied per-segment, AND each segment individually trips the skip. End-to-end PoC verified pre-fix across every case variation (`hoba` / `HOBA` / `Hoba` / `HoBa` / `hOBa`) and every body class (realistic 164-char mixed-class with 43+16+16+86 char fields per the canonical ECDSA P-256 shape, all-letter 4×16-char fields, smaller 4×14-char mixed body): only per-segment `Hochentropischer Token-String` findings fired (when the segments exceeded 24 chars), losing all HOBA-specific attribution AND splitting one credential into N findings. 8 real-world emission shapes verified pre-fix: wire-format HTTP logs, Python `requests` debug, browser HAR, YAML config, Wireshark text exports, Python docstrings, academic snippets, IoT firmware fixtures.

**Threat model:** A leaked direct-HOBA credential is structurally distinct from the other auth-scheme siblings in that **the body is a parameterised quoted-string `result="<KID>.<challenge>.<nonce>.<signature>"`** carrying four base64url-encoded fields whose individual semantics map to: (a) **the `KID`** — a base64url-encoded SHA-256 hash of the client's public key per RFC 7486 §6 (43 chars for the canonical SHA-256 output); reveals which client public key signs HOBA requests; combined with a separately-leaked public-key file or a directory enumeration of `/.well-known/hoba/public-keys`, can be cross-referenced to identify the user; (b) **the `signature`** — a base64url-encoded digital signature over `KID || challenge || nonce` using the client's private key (ECDSA P-256 = 86 base64url chars; RSA-2048 = 342 base64url chars; algorithm depends on the JOSE algorithm registry entry the HOBA implementation selects); a one-time-use replay-able auth proof bounded by the (challenge, nonce) freshness window; if the server accepts replay within the freshness window (a defect, but real in some lazy HOBA implementations that don't store seen nonces), the attacker can authenticate as the holder of the private key; (c) **the `challenge`** — server-supplied base64url-encoded nonce identifying the auth-request session (8-32 chars typically); exposes the server-side challenge issuance pattern, useful for protocol analysis and timing-attack reconnaissance; (d) **the `nonce`** — client-supplied base64url-encoded freshness anchor (8-32 chars typically); a sufficiently weak client-side nonce-generator (small entropy pool, timestamp-based, predictable) reveals the client's PRNG state, enabling future-nonce prediction and pre-image attacks. Real-world emission patterns include WebAuthn-pre-cursor HOBA implementations (most legacy HOBA deployments predate WebAuthn / FIDO2 and use the original RFC 7486 format), academic research papers on HTTP authentication that publish example captures verbatim, IoT device firmware that uses HOBA for backend auth in lieu of TLS client certs (constrained-device contexts where the TLS client-cert ladder is too heavyweight), specialised enterprise PKI integrations, browser dev-tools Network tab HAR exports of HOBA-authenticated sites, Wireshark / tshark capture text exports rendering the Authorization header verbatim, Python `requests` debug logs, YAML / Postman / Insomnia / Bruno request export files embedding the Authorization header. The blast radius is LOWER than Bearer/Basic/NTLM/Token (HOBA is RARE in production — "the canonical rare but registered" auth-scheme per the journal entries that classified it as deferred-but-completeness-worthy), but when HOBA IS in use the leak surface is meaningful: replay-based authentication attacks against lazy implementations plus key-binding information enabling targeted social-engineering attacks.

**Sites closed (1 detector regex + 1 table entry):**

  * `src/utils/secret_scanner.py:_HOBA_RE` — new regex appended to `_AUTH_SCHEME_DETECTORS` (placed BEFORE Token-scheme so the IANA-registered HOBA attribution wins over the de-facto Token catch-all):
    ```python
    _HOBA_RE = re.compile(
    ...
    ```
    The body alphabet `[A-Za-z0-9_\-]` is the canonical base64url alphabet per RFC 4648 §5 (URL-safe base64; HOBA uses base64url per RFC 7486 §3 to render binary key material / signatures into the textual HTTP scheme without `+/` characters that would require URL-percent-encoding in query-string contexts). The 8+ char per-field floor is a structural disambiguator chosen to reject the minimum example values in RFC 7486's normative text (e.g. `result="kid.6.6.34"` from §9.4) while accepting every realistic real-world HOBA credential (the canonical KID alone is 43 chars = base64url SHA-256; even truncated CTF examples typically use 8+ char per-field bodies). The quote alphabet `["']` accepts both canonical RFC-double-quote shape (`result="..."`) AND the de-facto single-quote shape that appears in YAML serialisations of HOBA captures, Python `requests` debug logs with f-string formatting, and Postman / Insomnia request exports. The `\\?` optional-backslash prefix accepts both the canonical wire-format quotes (`result="..."`) AND JSON-escaped / C-escaped quotes (`result=\"...\"`) — emission patterns from Python `requests` debug logs, JSON-serialised HAR exports, C-string literals in IoT firmware, and Python docstrings all use the escaped form. The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer / Basic / Negotiate / NTLM / Token siblings.

  * `_AUTH_SCHEME_DETECTORS` table extension — the canonical `_scan_auth_scheme_credentials` helper (single source of truth for HTTP-auth-scheme detector processing, extracted by the 2026-05-16 Basic Auth round) processes the new entry uniformly via the existing iteration loop with the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1` so uniform-character-class bodies are NOT silently undetected, lenient uniqueness floor) and `covered_ranges` mutation contract. No changes needed to `_scan_content` (C901 complexity unchanged) or to `_scan_auth_scheme_credentials` (no new branches). The structural invariant established by the 2026-05-16 Basic Auth round and confirmed by the Negotiate / NTLM / Token rounds held end-to-end for the SIXTH HTTP auth-scheme detector — even with HOBA's structurally-distinct parameterised quoted-string body shape, adding the detector required only a new regex constant + a new tuple entry, the canonical processing helper handles it without modification. Position invariant: `_HOBA_RE` lives BEFORE `_TOKEN_SCHEME_RE` (the canonical catch-all Bearer-alias position at the END of `_AUTH_SCHEME_DETECTORS`) so the more-specific IANA-registered HOBA scheme wins over the de-facto Token catch-all via `covered_ranges` arbitration. The six-detector ladder (Bearer / Basic / Negotiate / NTLM / HOBA / Token) covers every realistic single-body Authorization-header emission shape in the HOBA-conformant family; the multi-field shapes (Mutual / vapid / SCRAM / Digest / AWS4-HMAC-SHA256) remain deferred pending body-structure-aware framework.

**Severity:** **LOW-MEDIUM** — HOBA is RARE in production (the canonical "rare but registered" auth-scheme), so the realistic leak surface is bounded to academic / IoT / specialised PKI deployments. The high-severity case is the silent-undetection branch (all-letter-fields body): the HOBA credential sits committed in plaintext with NO detection at all. The attribution-drift branch is medium-severity (the body's individual segments ARE caught generically per-segment, but the HOBA-specific recovery surface — rotate the client's HOBA key pair, reissue the `KID`, audit for replay within the (challenge, nonce) freshness window — is lost AND the credential is split into N findings). Mitigated only by the requirement that the leaked credential live alongside a `HOBA result="..."` literal in the same content blob; in practice this covers every leak through HOBA-conformant IoT firmware fixtures, academic research example captures, browser HAR exports, Wireshark text dumps, Python `requests` debug logs, and YAML / Postman / Insomnia exports. CodeQL / Semgrep do not flag this drift because their default rules don't cover HOBA at all (HOBA is too rare for default rule maintainers to include; only complete auth-scheme enumerations include it).

**Fix:** Two-part change:
1. Add `_HOBA_RE = re.compile(r'''(?i)HOBA\s+result\s*=\s*\\?["']([A-Za-z0-9_\-]{8,}(?:\.[A-Za-z0-9_\-]{8,}){3})\\?["']''')` alongside `_BEARER_RE`, `_BASIC_AUTH_RE`, `_NEGOTIATE_RE`, `_NTLM_RE`, and `_TOKEN_SCHEME_RE` (single source of truth for the auth-scheme detector body shape).
2. Append `(_HOBA_RE, "HOBA Authentication Credential gefunden")` to `_AUTH_SCHEME_DETECTORS` BEFORE the Token-scheme entry — the canonical iteration helper `_scan_auth_scheme_credentials` processes the new entry uniformly. No code changes elsewhere — the helper preserves byte-for-byte behaviour of the Bearer/Basic/Negotiate/NTLM/Token detectors while extending coverage to RFC 7486 HOBA.

Zero false-positive risk: the structural shape `HOBA result="<8+>.<8+>.<8+>.<8+>"` is the disambiguator — natural English prose essentially never contains the literal `HOBA` followed by a quoted 4-tuple of base64url segments. The 8+ char per-field floor rejects the minimum example values in RFC 7486's text (e.g. `result="kid.6.6.34"` would have 3 chars max per field, far below the 8-char floor). The `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`, uniqueness ≥ `max(6, min(len // 4, 32))`) catches any structured string that does happen to match the regex syntactically — the dots break `isalpha()` so the uniqueness floor matters more here than in other detectors.

**Tests added:** `tests/test_sentinel_secret_scanner_hoba_drift.py` — 46 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × 164-char realistic body with ECDSA P-256 signature shape) — proves attribution-drift closes for every RFC-7235-conforming HOBA literal.
  2. Quote-shape PoCs (single-quote + double-quote acceptance for YAML / Python / wire-format emissions).
  3. Per-case silent-undetection PoCs (4 case variants × 4×16-char all-letter fields) — proves `candidate.isalpha()` skip bypass closes for HOBA result strings.
  4. Negative cases (17 natural-language passages, code-shape candidates, partial-body fragments, too-short-field bodies, missing-quote shapes, 3-field and 5-field bodies, English prose mentioning HOBA) — proves false-positive surface is bounded.
  5. Five regression guards — proves canonical Bearer, Basic Auth, Negotiate, NTLM, and Token-scheme continue to detect after the HOBA addition (the SIXTH HTTP auth-scheme detector in `_AUTH_SCHEME_DETECTORS`).
  6. Compiled-regex invariant — pins `_HOBA_RE.flags & re.IGNORECASE` programmatically against future case-sensitivity regression.
  7. Auth-scheme detector table membership invariant — pins `_HOBA_RE` membership in `_AUTH_SCHEME_DETECTORS` and reason-string membership so a future regression that adds the regex but forgets the table binding fails fast.
  8. Auth-scheme detector position invariant — pins `_HOBA_RE` appearing BEFORE `_TOKEN_SCHEME_RE` in `_AUTH_SCHEME_DETECTORS` so the more-specific IANA-registered HOBA scheme wins over the de-facto Token catch-all via `covered_ranges` arbitration.
  9. End-to-end emission-shape inventory (8 real-world emission patterns: wire-format HTTP logs, requests/urllib3 DEBUG, browser HAR, YAML config, Wireshark text exports, Python docstrings, academic snippets, IoT firmware fixtures).
 10. Masking-contract test — proves the raw HOBA result body never appears unmasked in finding output (mirrors Bearer/Basic/Negotiate/NTLM/Token masking-contract tests).
 11. Per-credential-consolidation test — proves a single HOBA result yields exactly ONE finding (not multiple per-segment entropy findings), consolidating noise and restoring credential-level attribution.

**Learning:** The HTTP-auth-scheme detector family extends BEYOND IETF-RFC-defined simple-body schemes (Bearer / Basic / Negotiate) AND vendor-spec schemes (NTLM) AND de-facto literals (Token) to include **IETF-RFC schemes with parameterised body shapes** (HOBA per RFC 7486) — the structural invariant ("HTTP-auth-scheme literals go to `_AUTH_SCHEME_DETECTORS` regardless of their body shape") proved end-to-end for the SIXTH detector, even though HOBA's parameterised `result="..."` body shape is structurally DIFFERENT from the five preceding detectors' simple `<scheme> <body>` shape. The 2026-05-16 Basic Auth round's `_AUTH_SCHEME_DETECTORS` tuple + `_scan_auth_scheme_credentials` helper architecture continues to hold byte-for-byte: adding a parameterised-body auth-scheme required ONLY a new regex constant + a new tuple entry. ZERO changes to the iteration helper, ZERO changes to `_scan_content`, ZERO C901 complexity impact. **The body-length floor remains the per-scheme structural disambiguator** — HOBA uses 8+ chars per field × 4 fields + 3 dots = 35-char minimum body (matches truncated CTF examples while excluding RFC 7486 §9.4 example placeholders). **The body alphabet remains determined by the spec's encoding** — HOBA, like Token, uses URL-safe base64 `[A-Za-z0-9_\-]` per RFC 4648 §5 (no `+/=` because the dot separators handle delimitation between fields). **The classification question — "parameterised-body literal in `_AUTH_SCHEME_DETECTORS` or skip pending multi-field framework?" — resolves to the auth-scheme table** when the parameterised body has a SINGLE captured-value within a single parameter (HOBA's `result="..."` carries one value; Mutual / vapid / SCRAM / Digest / AWS4-HMAC-SHA256 all carry MULTIPLE parameters that genuinely need a multi-field framework). HOBA's structural simplicity — one parameter, dot-separated 4-tuple inside a quoted string — makes it amenable to a single-regex detector despite the parameterised-body shape; the multi-parameter schemes remain deferred. **The quote-shape acceptance contract** — the regex accepts both canonical wire-format quotes (`"`, `'`) AND JSON-/C-escaped quotes (`\"`, `\'`) via the optional-backslash prefix `\\?` — handles the FOUR emission contexts where HOBA credentials realistically leak: HTTP wire format (canonical double-quote), YAML literal-block (single-quote), JSON serialisation / Python docstring / C string literal (escaped double-quote).

**Next-round candidates (named-but-deferred):** **Mutual auth detector** (`(?i)Mutual\s+...`) — RFC 8120 HTTP Mutual Authentication using cryptographic keys with key=value structured body (`user="..."`, `host="..."`, `algorithm="..."`, `nc="..."`, etc.); multi-field, very rare in production, deferred pending body-structure-aware framework. **vapid detector** (`(?i)vapid\s+t=...,k=...`) — RFC 8292 Voluntary Application Server Identification for Web Push; the body is multi-field (`t=<JWT>,k=<base64url-encoded-public-key>`); the embedded JWT (`eyJ...` prefix) already matches `_KNOWN_TOKENS`'s JWT regex, so the vapid scheme detector would be attribution-only for the wrapping. **SCRAM-SHA-256 / SCRAM-SHA-1 detector** (`(?i)SCRAM-SHA-(?:1|256)\s+...`) — RFC 7804 Salted Challenge Response Authentication Mechanism; the body is multi-step base64 message data with field separators; deferred pending body-structure-aware framework. **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth STILL deferred pending body-structure-aware detector framework (the credential is split across the `nonce` / `response` / `cnonce` fields; this remains the longest-deferred candidate from the auth-scheme family). **AWS4-HMAC-SHA256 detector** (`(?i)AWS4-HMAC-SHA256\s+Credential=...`) — AWS Sig v4 signed-request auth-scheme; the body is multi-field (`Credential` / `SignedHeaders` / `Signature`); similarly deferred pending body-structure-aware framework. With HOBA closed, the six-detector single-body ladder (Bearer / Basic / Negotiate / NTLM / HOBA / Token) covers every realistic single-body Authorization-header emission shape; the next architectural lift is the multi-field framework for the remaining FIVE candidates (Mutual / vapid / SCRAM / Digest / AWS4-HMAC-SHA256), which is meaningfully larger than the per-detector additions of the past six rounds.

**Marker:** SENTINEL_HOBA_DRIFT.

## 2026-05-16 - Token-Scheme Detector Drift (De Facto GitHub `token` HTTP Auth-Scheme, Sibling Of NTLM Round): The First-Priority Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-16 NTLM Round Closed In Full — A Leaked `Authorization: token <opaque-body>` In Committed Source Carried Two Orthogonal Failure Modes (Attribution Drift For Mixed-Class Bodies, Silent Undetection For All-Letter Bodies) That Lost The Token-Scheme-Specific Recovery Surface (Per-Issuer Rotation Flow: github.com/settings/tokens For GitHub Legacy 40-Char Hex Tokens, /user/settings/applications For Gitea/Forgejo, cloud.digitalocean.com/account/api/tokens For DigitalOcean V1) Entirely

**Vulnerability:** The 2026-05-16 NTLM round (`security(secret-scanner): add NTLM detector per [MS-NLMP]`) named **GitHub `Token` scheme detector** (`(?i)token\s+[A-Za-z0-9_\-]{36,}`) as the first-priority next-round adjacent-detector candidate (HOBA / Mutual / Digest / AWS4-HMAC-SHA256 remain deferred — HOBA & Mutual are rare in practice, Digest & AWS4-HMAC-SHA256 still require a body-structure-aware framework). Pre-fix `src/utils/secret_scanner.py` had NO direct `token`-scheme detector — the only coverage for a leaked `Authorization: token <body>` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`) which matched typical opaque API token bodies generically, BUT only when the body had ≥2 character categories AND was not all-letter (the `candidate.isalpha()` short-circuit in `_HIGH_ENTROPY_RE`'s loop suppresses LongCamelCaseClassName false positives). The four existing auth-scheme detectors (Bearer / Basic / Negotiate / NTLM) closed their respective IETF-RFC and MS-vendor-spec auth-schemes, but NOT the **de facto GitHub `token` scheme** — used by every Git-host that mirrors the GitHub REST API shape (Gitea, Forgejo, Codeberg, Gogs) plus DigitalOcean's legacy v1 API and various other tools. The `token` scheme is NOT in the IANA HTTP Authentication Schemes registry — it is a de facto convention popularised by GitHub's documentation, but it carries the same wire-protocol semantics as Bearer (the body is an opaque API token, not a structured payload). Two orthogonal failure modes resulted: **(1) Attribution drift** — opaque API tokens (typically 36+ chars from the `[A-Za-z0-9_\-]` alphabet — GitHub PAT body shape, Gitea PAT body shape, DigitalOcean v1 hex token shape) with mixed character classes lost the token-scheme-specific attribution and landed as generic `Hochentropischer Token-String` (or, if the file content contained `Authorization` which triggers `_SENSITIVE_ASSIGN_RE`'s `auth` keyword family, the assignment-heuristic finding `Verdächtige Zuweisung eines potentiellen Secrets`), so incident-response triage had to GUESS the recovery surface across five distinct flows (GitHub: rotate at github.com/settings/tokens, audit `GET /user` API calls for token's usage history; Gitea/Forgejo: rotate at `/user/settings/applications`, audit Gitea's OAuth2 Applications audit log; DigitalOcean: rotate at cloud.digitalocean.com/account/api/tokens, audit Spaces/Droplet/Kubernetes API calls; Bearer: revoke at IdP; Basic Auth: rotate password). **(2) Silent undetection** — all-letter bodies trip `candidate.isalpha()` and are skipped by the entropy fallback. Pre-fix every leaked `token <all-letter-body>` was SILENTLY UNDETECTED (when not also matching the assignment heuristic for a sensitive variable name in the surrounding context). End-to-end PoC verified pre-fix across every case variation (`token` / `TOKEN` / `Token` / `ToKeN` / `tOkEn`) and every body class (mixed-class 46-char opaque, all-letter 40-char, legacy 40-char hex GitHub pre-April-2021 shape): only `Verdächtige Zuweisung eines potentiellen Secrets` fired (when the `Authorization:` line triggered the assignment heuristic), losing all token-scheme-specific attribution. 10 real-world emission shapes verified pre-fix: curl debug logs (lowercase/uppercase/title-case), browser HAR exports, Python JSON fixtures, YAML manifests, GitHub Actions logs, urllib3 debug traces, Gitea curl examples, DigitalOcean v1 curl examples.

**Threat model:** A leaked direct-`token`-scheme credential is structurally similar to Bearer but semantically distinct in that the body is **an opaque API token issued by a non-OAuth-2.0 issuer** — typically a long-lived Personal Access Token (PAT) from a Git host. The blast radius depends on the token's permission scope and the issuer: **(a) GitHub PAT/legacy tokens** — scope ranges from read-only `public_repo` to full `repo + admin:org + admin:gpg_key + admin:public_key + workflow + delete_repo + write:packages`; the worst-case blast radius includes private repo source code exfiltration, secret extraction from GitHub Actions environments, branch protection bypass leading to RCE via malicious workflow modification, deletion of organization repositories, GPG/SSH key registration for impersonation, and account takeover via the `user:email` scope (attacker can change primary email and reset password); **(b) Gitea/Forgejo/Codeberg/Gogs tokens** — similar scope structure with same blast radius patterns on self-hosted infrastructure (often with weaker rate limiting than github.com and weaker IP-allowlisting controls); **(c) DigitalOcean v1 tokens** — full account access by default (Droplets, Kubernetes, Spaces, Load Balancers, DNS records, Reserved IPs); a leaked DO token allows arbitrary VM provisioning (cryptocurrency mining at the victim's expense), arbitrary DNS hijacking (BGP-like takeover for any DO-hosted domain), and Kubernetes cluster takeover for hosted K8s. The cross-detector boundary matters: a `Authorization: token ghp_xxx...` leak carries TWO matchable spans — the `ghp_<36>` GitHub PAT span (matched by `_KNOWN_TOKENS`'s `ghp_` regex, RUNS FIRST) and the `token <body>` HTTP auth-scheme span (matched by the new `_TOKEN_SCHEME_RE`, RUNS LATER). The `_KNOWN_TOKENS` matcher wins via `covered_ranges` arbitration, so the GitHub-PAT-specific reason is preserved for prefixed tokens; the new detector's attribution applies ONLY to opaque non-prefixed tokens (legacy pre-April-2021 GitHub 40-char hex tokens still in circulation, Gitea/Forgejo/DigitalOcean opaque tokens, third-party `token`-scheme issuers). Real-world emission patterns include legacy GitHub REST API curl examples in shell scripts and tutorials (`curl -H "Authorization: token ghp_xxx" https://api.github.com/user`), `hub` CLI configuration (`~/.config/hub` YAML with `oauth_token:` field whose underlying transport uses the `token` HTTP scheme), Gitea / Forgejo / Codeberg / Gogs API examples (every fork that mirrors GitHub's API shape), DigitalOcean v1 API docs that pre-date the cloud.digitalocean.com console redesign, CI/CD workflow files with hard-coded fallback tokens, Python `requests` debug logs (`logging.DEBUG` on `urllib3.connectionpool`) emitting the full Authorization header, browser dev-tools Network tab exports (HAR files) capturing intranet self-hosted Git host API calls, and copy-pasted documentation snippets in READMEs / wikis with real (live!) tokens left in. The blast radius of an undetected `token`-scheme leak therefore overlaps Bearer's blast radius in the worst case (full account access at the issuing service) while spanning a wider population of issuers than Bearer's OAuth-2.0 IdP scope.

**Sites closed (1 detector regex + 1 table entry):**

  * `src/utils/secret_scanner.py:_TOKEN_SCHEME_RE` — new regex appended to `_AUTH_SCHEME_DETECTORS` (placed LAST so more-specific schemes win via `covered_ranges` arbitration):
    ```python
    ...
    ```
    The body alphabet `[A-Za-z0-9_\-]` covers GitHub's PAT alphabet (URL-safe alphanumeric + underscore + hyphen — the same alphabet used by every `gh*_`-prefixed token family), Gitea/Forgejo's PAT alphabet (both inherit GitHub's shape per their REST API specs), and DigitalOcean's hex token shape. Standard base64 padding (`+/=`) is NOT in the alphabet because opaque API tokens in the GitHub-ecosystem `token` scheme are URL-safe alphanumeric, NOT base64 (Bearer's alphabet adds `.` for JWTs; Basic/Negotiate/NTLM use the standard base64 alphabet for their respective payloads). The 36+ char body floor is the structural disambiguator — the word `token` is common in English prose and code comments, but is essentially never followed by 36+ contiguous chars from the `[A-Za-z0-9_\-]` alphabet in natural text (English words break at whitespace and punctuation). The floor is calibrated to the GitHub PAT body length (`ghp_<36>` = 40 total chars, with the 36-char body part captured when the PAT prefix is absent — the legacy 40-char hex GitHub token from before April 2021 also exceeds the 36-char floor). The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer / Basic Auth / Negotiate / NTLM siblings.

  * `_AUTH_SCHEME_DETECTORS` table extension — the canonical `_scan_auth_scheme_credentials` helper (single source of truth for HTTP-auth-scheme detector processing, extracted by the 2026-05-16 Basic Auth round) processes the new entry uniformly via the existing iteration loop with the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1` so uniform-character-class bodies are NOT silently undetected) and `covered_ranges` mutation contract. No changes needed to `_scan_content` (C901 complexity unchanged) or to `_scan_auth_scheme_credentials` (no new branches). The structural invariant established by the 2026-05-16 Basic Auth round and confirmed by the 2026-05-16 Negotiate / NTLM rounds held end-to-end for the FIFTH HTTP auth-scheme detector: adding a new HTTP-auth-scheme detector requires only a new regex constant + a new tuple entry — the canonical processing helper handles it without modification.

**Severity:** **MEDIUM-HIGH** — detection-from-zero failure for a class of realistic GitHub-ecosystem (GitHub legacy 40-char hex tokens still in circulation on long-running services + every Git-host fork that mirrors GitHub's API shape: Gitea, Forgejo, Codeberg, Gogs) and DigitalOcean v1 API credential leaks. The high-severity case is the silent-undetection branch (all-letter bodies): the opaque API token (potentially granting full account access at the issuing service) sits committed in plaintext with NO detection at all. The attribution-drift branch is medium-severity (the body IS caught generically, but the issuer-family-specific recovery surface that determines which rotation flow applies — GitHub vs Gitea vs DigitalOcean vs other — is lost). Mitigated only by the requirement that the leaked credential live alongside a `token` auth-scheme literal in the same content blob; in practice this covers every leak through legacy curl examples, CI workflow templates, HAR exports of API-using intranet apps, `requests` debug output, and copy-pasted documentation. CodeQL / Semgrep / TruffleHog do not flag this drift because their default rules cover Bearer / Basic / canonical-prefix tokens (`ghp_`, `glpat-`) but not the de facto `token`-scheme literal that wraps opaque non-prefixed tokens (the `token` scheme is not registered as an HTTP auth-scheme in IANA, so generic auth-scheme detectors don't enumerate it).

**Fix:** Two-part change:
1. Add `_TOKEN_SCHEME_RE = re.compile(r"(?i)token\s+([A-Za-z0-9_\-]{36,})")` alongside `_BEARER_RE`, `_BASIC_AUTH_RE`, `_NEGOTIATE_RE`, and `_NTLM_RE` (single source of truth for the auth-scheme detector body shape).
2. Append `(_TOKEN_SCHEME_RE, "HTTP Token-Scheme Authentication Credential gefunden")` to `_AUTH_SCHEME_DETECTORS` LAST — the canonical iteration helper `_scan_auth_scheme_credentials` processes the new entry uniformly, and the LAST placement ensures more-specific auth-scheme detectors (Bearer / Basic / Negotiate / NTLM) win via `covered_ranges` arbitration when a token-shaped body sits inside one of their headers. No code changes elsewhere — the helper preserves byte-for-byte behaviour of the Bearer/Basic/Negotiate/NTLM detectors while extending coverage to the de facto `token` scheme.

Zero false-positive risk in the THIS codebase: a grep across `src/` `scripts/` `tests/` `docs/` confirmed ZERO existing `token\s+[A-Za-z0-9_\-]{36,}` matches before adding the detector. The body alphabet `[A-Za-z0-9_\-]` excludes the dots-and-base64-padding-required by JWTs / base64 payloads (which are caught by Bearer's `[A-Za-z0-9\-_.]` and Basic's `[A-Za-z0-9+/=]` alphabets respectively); the 36-char floor excludes natural English prose where the word `token` is followed by short identifiers; the `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`) catches any token-shaped string that does happen to follow a `token`-prefixed natural-language passage. The cross-detector boundary preserves the more-specific GitHub PAT (`ghp_`) attribution via `_KNOWN_TOKENS` running FIRST in `_scan_content` and the more-specific auth-scheme attribution (Bearer / Basic / Negotiate / NTLM) via the earlier table position in `_AUTH_SCHEME_DETECTORS`.

**Tests added:** `tests/test_sentinel_secret_scanner_token_scheme_drift.py` — 47 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × 46-char opaque mixed-class body) — proves attribution-drift closes for every RFC-7235-conforming `token` literal.
  2. Per-case silent-undetection PoCs (4 case variants × 40-char all-letter body) — proves `candidate.isalpha()` skip bypass closes for token-scheme-prefixed tokens.
  3. Legacy 40-char hex GitHub token PoC — proves backward compatibility for pre-April-2021 GitHub tokens still in circulation.
  4. Negative cases (16 natural-language passages, code-shape candidates, and a 35-char body just-below-floor) — proves false-positive surface is bounded.
  5. Two cross-detector boundary regression guards — proves GitHub PAT (`ghp_`) attribution and JWT attribution win over the new detector (the `_KNOWN_TOKENS` matching runs BEFORE auth-scheme detectors).
  6. Four regression guards — proves canonical Bearer, Basic Auth, Negotiate, and NTLM continue to detect after the token-scheme addition (the FIFTH HTTP auth-scheme detector in `_AUTH_SCHEME_DETECTORS`).
  7. Compiled-regex invariant — pins `_TOKEN_SCHEME_RE.flags & re.IGNORECASE` programmatically against future case-sensitivity regression.
  8. Two auth-scheme detector table invariants — pins `_TOKEN_SCHEME_RE` membership in `_AUTH_SCHEME_DETECTORS` AND its LAST-position ordering relative to the four more-specific detectors so a future regression that swaps the order fails fast.
  9. End-to-end emission-shape inventory (10 real-world emission patterns: curl debug logs lowercase/uppercase/title-case, browser HAR exports, Python/JSON fixtures, YAML manifests, GitHub Actions logs, urllib3 debug traces, Gitea curl examples, DigitalOcean v1 curl examples).
 10. Masking-contract test — proves the raw opaque API token body never appears unmasked in finding output (mirrors Bearer/Basic/Negotiate/NTLM masking-contract tests).

**Learning:** The HTTP-auth-scheme detector family extends BEYOND IETF-RFC-defined schemes (Bearer per RFC 6750, Basic per RFC 7617, Negotiate per RFC 4559) AND beyond vendor-spec-defined schemes ([MS-NLMP] NTLM) to include **de facto auth-scheme literals popularised by vendor REST API documentation** (the GitHub `token` scheme inherited by every Git-host fork) — the structural invariant ("HTTP-auth-scheme literals go to `_AUTH_SCHEME_DETECTORS` regardless of their underlying spec body and regardless of whether they are formally registered with IANA") proved end-to-end for the FIFTH detector. The 2026-05-16 Basic Auth round's `_AUTH_SCHEME_DETECTORS` tuple + `_scan_auth_scheme_credentials` helper architecture continues to hold byte-for-byte: adding a de facto auth-scheme (GitHub's `token`) required ONLY a new regex constant + a new tuple entry. ZERO changes to the iteration helper, ZERO changes to `_scan_content`, ZERO C901 complexity impact. **The body-length floor remains the per-scheme structural disambiguator** — the `token` scheme uses 36+ chars (matches GitHub PAT body width while excluding the common English word in prose). **The body alphabet remains determined by the scheme's typical-issuer encoding** — GitHub-ecosystem `token` schemes use URL-safe alphanumeric+hyphen+underscore (`[A-Za-z0-9_\-]`), NOT base64 (Bearer adds `.` for JWTs, Basic/Negotiate/NTLM use `+/=` for base64 payloads). **The placement order in `_AUTH_SCHEME_DETECTORS` matters for cross-detector boundary preservation** — the generic `token` scheme must come LAST so more-specific auth-scheme detectors (Bearer / Basic / Negotiate / NTLM) win via `covered_ranges` arbitration when a token-shaped body sits inside their headers. **The classification question — "de facto vendor literal in `_AUTH_SCHEME_DETECTORS` or `_KNOWN_TOKENS`?" — resolves to the auth-scheme table** when the literal IS positioned as an HTTP auth-scheme (the wire-protocol position determines placement, NOT whether the scheme is IANA-registered). The `token` literal is an HTTP auth-scheme in the GitHub-ecosystem wire protocol regardless of its absence from the IANA registry; the placement is in `_AUTH_SCHEME_DETECTORS` for uniformity with Bearer/Basic/Negotiate/NTLM.

**Next-round candidates (named-but-deferred):** **HOBA detector** (`(?i)HOBA\s+...`) — RFC 7486 HTTP Origin-Bound Auth; rare in practice but a complete auth-scheme enumeration would include it. **Mutual auth detector** (`(?i)Mutual\s+...`) — RFC 8120 HTTP Mutual Authentication using cryptographic keys; very rare. **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth STILL deferred pending body-structure-aware detector framework (the credential is split across the `nonce` / `response` / `cnonce` fields, so a single-regex anchor doesn't fully capture the secret — context-aware extraction needed; this remains the longest-deferred candidate from the auth-scheme family because the architectural lift is meaningfully larger than the per-detector additions of the past five rounds). **AWS4-HMAC-SHA256 detector** (`(?i)AWS4-HMAC-SHA256\s+Credential=...`) — AWS Sig v4 signed-request auth-scheme; the body is multi-field (`Credential` / `SignedHeaders` / `Signature`) like Digest, so similarly deferred pending body-structure-aware framework. **GitHub Apps JWT-Bearer detector** — GitHub Apps mint short-lived JWT tokens (RS256-signed with the app's private key) for `Bearer` scheme auth; the JWT detector already catches the canonical `eyJ.eyJ.SIG` shape via `_KNOWN_TOKENS`, but a leaked private RSA key (the JWT-signing material) is a higher-value secret; the PEM detector already catches the `-----BEGIN RSA PRIVATE KEY-----` envelope, so this is closed in practice but worth documenting as the structural anchor.

**Marker:** SENTINEL_TOKEN_SCHEME_DRIFT.

---

## 2026-05-16 - NTLM Detector Drift ([MS-NLMP] HTTP NTLM Authentication, Sibling Of Negotiate Round): The First-Priority Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-16 SPNEGO/Negotiate Round Closed In Full — A Leaked `Authorization: NTLM <base64(NTLMSSP-message)>` In Committed Source Carried Two Orthogonal Failure Modes (Attribution Drift For Long Type-3 Messages, Silent Undetection For All-Letter Bodies) That Lost The NTLM-Specific Recovery Surface (NetNTLMv2 Relay Via `ntlmrelayx`, Offline Cracking With `hashcat` Mode 5600 To Recover Plaintext Password) Entirely

**Vulnerability:** The 2026-05-16 SPNEGO/Negotiate round (`security(secret-scanner): add Negotiate detector per RFC 4559 SPNEGO`) named **NTLM detector** as the first-priority next-round adjacent-detector candidate (the deferred Digest Auth still requires a body-structure-aware framework). Pre-fix `src/utils/secret_scanner.py` had NO direct NTLM detector — the only coverage for a leaked `Authorization: NTLM <body>` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`) which matched the base64-encoded NTLMSSP message generically, BUT only when the body had ≥2 character categories AND was not all-letter (the `candidate.isalpha()` short-circuit in `_HIGH_ENTROPY_RE`'s loop suppresses LongCamelCaseClassName false positives). The Negotiate detector closed SPNEGO-wrapped NTLMSSP but NOT direct-NTLM, the common case for IIS with legacy non-Kerberos-capable clients, SMB-over-HTTP, WebDAV, SharePoint Windows-only intranets, and any backend whose NTLM auth precedes SPNEGO negotiation (per RFC 4559 §4 NTLM may appear as a standalone HTTP auth-scheme alongside Negotiate). Two orthogonal failure modes resulted: **(1) Attribution drift** — long NTLMSSP Type 3 (Authenticate) messages (350-1000+ bytes raw = 470-1500+ base64 chars carrying the NetNTLMv2 challenge-response + UserName/Workstation/DomainName fields) with mixed character classes lost the NTLM-specific attribution and landed as generic `Hochentropischer Token-String`, so incident-response triage had to GUESS the recovery surface (NTLM: rotate user password, audit domain controller for NetNTLMv2 relay attempts via `ntlmrelayx`, force user re-auth — the NTLMv2 hash is offline-crackable with `hashcat` mode 5600; SPNEGO/Kerberos: revoke service ticket at KDC; Bearer: revoke at IdP; Basic Auth: rotate password — four distinct IR flows that hinge on per-scheme attribution). **(2) Silent undetection** — all-letter base64 bodies trip `candidate.isalpha()` and are skipped by the entropy fallback. Pre-fix every leaked `NTLM <all-letter-body>` was SILENTLY UNDETECTED — the CI gate passed, the NTLMSSP message (potentially Type 3 with a recoverable NetNTLMv2 hash) sat committed in the public repo indefinitely. End-to-end PoC verified pre-fix across every case variation (`ntlm` / `NTLM` / `Ntlm` / `NtLm` / `nTlM`) and every body class (long-mixed 240-char NTLMSSP-prefixed, all-letter 62-char, canonical `TlRMTVNTUA`-prefixed NTLMSSP Type 1 shape): only `Verdächtige Zuweisung eines potentiellen Secrets` or `Hochentropischer Token-String` fired (when the variable name happened to carry a sensitive keyword or the body was mixed-class above 24 chars), losing all NTLM-specific attribution.

**Threat model:** A leaked direct-NTLM credential is structurally distinct from SPNEGO/Negotiate leaks in that **the body is a base64-encoded raw NTLMSSP message** per [MS-NLMP] §2.2 (Header: `0x4e 0x54 0x4c 0x4d 0x53 0x53 0x50 0x00` magic = `TlRMTVNTUA` in base64) — Type 1 Negotiate / Type 2 Challenge / Type 3 Authenticate. An attacker who recovers a Type 3 NTLMSSP body therefore holds: (a) **the NTLMv2 challenge-response** — the 16-byte NTProofStr + variable-length NTLMv2 client challenge, which together form a NetNTLMv2 hash; replayable via `ntlmrelayx` against any service in the same Windows domain that accepts NTLM authentication (SMB, LDAP, HTTP, MSSQL, RPC over named pipes); mitigated by SMB signing / LDAP channel binding / Extended Protection for Authentication (EPA) on the target, but those mitigations are not universal — most legacy Windows file shares and internal IIS apps still allow unsigned NTLM; (b) **the offline-cracking amplifier** — the NetNTLMv2 hash is subject to offline dictionary / brute-force / rule-based attack with `hashcat` (mode 5600) or `john` (`netntlmv2` format); modern GPUs (RTX 4090) sustain ~50 GH/s throughput for masking attacks, recovering weak passwords (≤10 chars with common patterns) within minutes; the plaintext password is the canonical password-reuse-amplifier surface (the user likely reuses it on email, VPN, SaaS); (c) **domain reconnaissance** — the Type 3 message carries the unencrypted `UserName`, `Workstation`, and `DomainName` fields, revealing a valid user-domain-workstation tuple for further attacks (lateral movement, password spraying with the recovered password, Kerberoasting once the user is on the radar); (d) **Type 1 negotiation flags** — the Type 1 message leaks the client's supported authentication features (NEGOTIATE_NTLM, NEGOTIATE_NTLM2_KEY, NEGOTIATE_SEAL, etc.), revealing the client's NTLM dialect and helping the attacker pick a downgrade strategy if the target supports multiple dialects. Real-world emission patterns include IIS HTTP request logs with `--debug` flag (the entire Authorization header is logged verbatim), browser dev-tools Network tab exports (`Save HAR with content`) for intranet NTLM-authenticated SharePoint / OWA / internal-app sites, Wireshark / tshark capture exports rendered as text (the common Windows-network-debug shape — SMB-over-HTTP / WebDAV / SharePoint captures are routinely shared in IR / on-call tickets), WinRM debug logs (`Set-PSDebug -Trace 2`) emitting the Authorization header during NTLM negotiate / challenge / authenticate round trips, `curl -v --ntlm -u user:pass` debug logs, Python `requests` with `requests-ntlm` debug mode, Spring Security `org.springframework.security.kerberos` debug logging (intercepts NTLM as a fallback when SPNEGO is unavailable), ELK Stack ingest of `WWW-Authenticate: NTLM` response headers, and Active Directory diagnostic event logs (Event ID 4624 with authentication type 3 / 5 / 7). The blast radius of an undetected NTLM Type 3 leak EXCEEDS a Basic Auth leak in the worst case — NTLM enables relay attacks even WITHOUT cracking the password, while a Basic Auth leak requires the attacker to actually use the recovered `user:password` pair.

**Sites closed (1 detector regex + 1 table entry):**

  * `src/utils/secret_scanner.py:_NTLM_RE` — new regex appended to `_AUTH_SCHEME_DETECTORS`:
    ```python
    ...
    ```
    The body alphabet `[A-Za-z0-9+/=]` is the canonical base64 alphabet per RFC 4648 §4 (standard base64; NTLMSSP messages always use standard base64 in HTTP Authorization headers per [MS-NLMP] §2.2 — the binary protocol is rendered into the textual HTTP scheme via standard base64, never the URL-safe `_-` variant). The 50+ char body floor is the structural disambiguator — real NTLMSSP messages are ALL above 50 chars (Type 1 = 40 bytes raw = 56 base64 chars minimum; Type 2 = 80-200 bytes raw = 108-272 base64 chars; Type 3 = 350-1000+ bytes raw = 470-1500+ base64 chars), so 50 is a safe conservative floor that catches every realistic NTLMSSP message including truncated Type 1 tokens in fragmented log lines. The acronym `NTLM` doesn't appear in natural English prose followed by 50+ contiguous chars from the base64 alphabet (English words break at whitespace and punctuation). The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer / Basic / Negotiate siblings.

  * `_AUTH_SCHEME_DETECTORS` table extension — the canonical `_scan_auth_scheme_credentials` helper (single source of truth for HTTP-auth-scheme detector processing, extracted by the 2026-05-16 Basic Auth round) processes the new entry uniformly via the existing iteration loop with the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1` so uniform-character-class bodies are NOT silently undetected) and `covered_ranges` mutation contract. No changes needed to `_scan_content` (C901 complexity unchanged) or to `_scan_auth_scheme_credentials` (no new branches). The structural invariant established by the 2026-05-16 Basic Auth round and confirmed by the 2026-05-16 Negotiate round held end-to-end for the FOURTH HTTP auth-scheme detector: adding a new HTTP-auth-scheme detector requires only a new regex constant + a new tuple entry — the canonical processing helper handles it without modification.

**Severity:** **MEDIUM-HIGH** — detection-from-zero failure for a class of realistic NTLM credential leaks in enterprise Windows environments (Active Directory, IIS legacy auth, WinRM, SharePoint, SMB-over-HTTP, WebDAV). The high-severity case is the silent-undetection branch (all-letter bodies): the NTLMSSP message (potentially Type 3 with a NetNTLMv2 hash recoverable to the plaintext password via offline cracking) sits committed in plaintext with NO detection at all. The attribution-drift branch is medium-severity (the body IS caught generically, but the NTLM-specific recovery surface that determines which IR playbook applies — rotate user password, audit domain controller for NetNTLMv2 relay attempts via `ntlmrelayx`, force user re-auth, offline-crack the hash with `hashcat` mode 5600 to recover plaintext — is lost). Mitigated only by the requirement that the leaked credential live alongside an `NTLM` auth-scheme literal in the same content blob; in practice this covers every leak through IIS debug logs, browser HAR exports of intranet sites, Wireshark/tshark SMB-over-HTTP captures, WinRM diagnostics, and curl/requests-ntlm debug output. CodeQL / Semgrep do not flag this drift because their default rules cover Bearer / Basic auth-schemes but not direct-NTLM (NTLM-specific detection requires a per-rule body-alphabet and body-length disambiguator, which is rule-table dependent — no industry-standard default).

**Fix:** Two-part change:
1. Add `_NTLM_RE = re.compile(r"(?i)NTLM\s+([A-Za-z0-9+/=]{50,})")` alongside `_BEARER_RE`, `_BASIC_AUTH_RE`, and `_NEGOTIATE_RE` (single source of truth for the auth-scheme detector body shape).
2. Append `(_NTLM_RE, "NTLM Authentication Credential gefunden")` to `_AUTH_SCHEME_DETECTORS` — the canonical iteration helper `_scan_auth_scheme_credentials` processes the new entry uniformly. No code changes elsewhere — the helper preserves byte-for-byte behaviour of the Bearer/Basic/Negotiate detectors while extending coverage to direct-NTLM.

Zero false-positive risk: the body alphabet `[A-Za-z0-9+/=]` is identical to `_BASIC_AUTH_RE`'s and `_NEGOTIATE_RE`'s (no `-_.` like Bearer's URL-safe-ish `token68` ABNF); the structural disambiguator (`\s+` between the auth-scheme literal and a 50+ char body) is identical to Negotiate's. The acronym `NTLM` is more selective than the English word `Negotiate` for natural-language false positives (Negotiate is a real English verb commonly appearing in prose; NTLM is a Microsoft technical acronym that almost never appears outside technical contexts). The `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`) catches any token-shaped string that does happen to follow an `ntlm`-prefixed natural-language passage — the same defence Bearer/Basic/Negotiate already rely on.

**Tests added:** `tests/test_sentinel_secret_scanner_ntlm_drift.py` — 39 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × 240-char NTLMSSP-prefixed long-mixed body) — proves attribution-drift closes for every RFC-7235-conforming NTLM literal.
  2. Per-case silent-undetection PoCs (4 case variants × 62-char all-letter body) — proves `candidate.isalpha()` skip bypass closes for NTLM-prefixed tokens.
  3. Negative cases (13 natural-language passages, code-shape candidates, and a 49-char body just-below-floor) — proves false-positive surface is bounded.
  4. Cross-detector ordering regression guard — proves JWT in Bearer header still yields JWT attribution (the `_KNOWN_TOKENS` matching runs BEFORE auth-scheme detectors).
  5. Three regression guards — proves canonical Bearer, Basic Auth, and Negotiate continue to detect after the NTLM addition (the FOURTH HTTP auth-scheme detector in `_AUTH_SCHEME_DETECTORS`).
  6. Compiled-regex invariant — pins `_NTLM_RE.flags & re.IGNORECASE` programmatically against future case-sensitivity regression.
  7. Auth-scheme detector table membership invariant — pins `_NTLM_RE` membership in `_AUTH_SCHEME_DETECTORS` so a future regression that adds the regex but forgets the table binding fails fast.
  8. End-to-end emission-shape inventory (9 real-world emission patterns: curl debug logs, IIS debug logs, browser HAR exports, Python/JSON fixtures, YAML config, WinRM debug traces, Wireshark text exports, WWW-Authenticate response headers).
  9. Masking-contract test — proves the raw base64 NTLMSSP body never appears unmasked in finding output (mirrors Bearer/Basic/Negotiate masking-contract tests).
 10. NTLMSSP Type 1 structural-prefix demonstration documenting the canonical `TlRMTVNTUA` base64 prefix (base64 of the ASCII `NTLMSSP\0` magic per [MS-NLMP] §2.2 Header) as an incident-response anchor distinguishing direct-NTLM from Kerberos AP-REQ (the `YII` prefix from the 2026-05-16 Negotiate round).

**Learning:** The HTTP-auth-scheme detector family extends BEYOND IETF-RFC-defined schemes to include vendor-specific HTTP auth-scheme literals ([MS-NLMP] NTLM) — the structural invariant ("HTTP-auth-scheme literals go to `_AUTH_SCHEME_DETECTORS` regardless of their underlying spec body") proved end-to-end for the FOURTH detector. The 2026-05-16 Basic Auth round's `_AUTH_SCHEME_DETECTORS` tuple + `_scan_auth_scheme_credentials` helper architecture continues to hold byte-for-byte: adding a vendor-defined auth-scheme (NTLM per [MS-NLMP]) required ONLY a new regex constant + a new tuple entry. ZERO changes to the iteration helper, ZERO changes to `_scan_content`, ZERO C901 complexity impact. **The body-length floor remains the per-scheme structural disambiguator** — NTLM uses 50+ chars (matches truncated Type 1 messages at 56 base64 chars while excluding the technical acronym in prose). **The body alphabet remains determined by the spec's encoding** — direct-NTLM, like Basic (RFC 7617) and Negotiate (RFC 4559), uses standard base64 `[A-Za-z0-9+/=]` per [MS-NLMP] §2.2 (the binary NTLMSSP wire format rendered to textual HTTP via standard base64). **The classification question — "vendor literal in `_AUTH_SCHEME_DETECTORS` or `_KNOWN_TOKENS`?" — resolves to the auth-scheme table** when the literal IS an HTTP auth-scheme (the wire-protocol position determines placement, NOT the spec authoring body). NTLM is an HTTP auth-scheme literal that happens to be defined in a Microsoft spec instead of an IETF RFC; the placement is in `_AUTH_SCHEME_DETECTORS` for uniformity with Bearer/Basic/Negotiate.

**Next-round candidates (named-but-deferred):** **GitHub `Token` scheme detector** (`(?i)token\s+[A-Za-z0-9_\-]{36,}`) — GitHub's older API alias for Bearer (`Authorization: token ghp_...`); the embedded GitHub PAT (`ghp_`-prefixed) already matches `_KNOWN_TOKENS` via the `ghp_` regex, so the `token` scheme detector is attribution-only for opaque non-GitHub tokens that happen to use the alias. **HOBA detector** (`(?i)HOBA\s+...`) — RFC 7486 HTTP Origin-Bound Auth; rare in practice but a complete auth-scheme enumeration would include it. **Mutual auth detector** (`(?i)Mutual\s+...`) — RFC 8120 HTTP Mutual Authentication using cryptographic keys; very rare. **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth STILL deferred pending body-structure-aware detector framework (the credential is split across the `nonce` / `response` / `cnonce` fields, so a single-regex anchor doesn't fully capture the secret — context-aware extraction needed; this remains the longest-deferred candidate from the auth-scheme family because the architectural lift is meaningfully larger than the per-detector additions of the past four rounds). **AWS4-HMAC-SHA256 detector** (`(?i)AWS4-HMAC-SHA256\s+Credential=...`) — AWS Sig v4 signed-request auth-scheme; the body is multi-field (`Credential` / `SignedHeaders` / `Signature`) like Digest, so similarly deferred pending body-structure-aware framework.

**Marker:** SENTINEL_NTLM_DRIFT.

## 2026-05-16 - SPNEGO/Negotiate Detector Drift (RFC 4559 HTTP SPNEGO Authentication, Sibling Of Basic Auth Round): The Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-16 Basic Auth Round Closed In Full — A Leaked `Authorization: Negotiate <base64(GSSAPI-token)>` In Committed Source Carried Two Orthogonal Failure Modes (Attribution Drift For 200+ Char Kerberos Tokens, Silent Undetection For All-Letter Bodies) That Lost The Kerberos-Specific Revocation Playbook (KDC Service Ticket Revocation, Force User Re-Auth, Audit Service Principal For Replay Within 8-10h Ticket Lifetime) Entirely

**Vulnerability:** The 2026-05-16 Basic Auth round (`security(secret-scanner): add Basic Auth detector per RFC 7617`) named **SPNEGO Negotiate detector** as the second-priority adjacent-detector candidate (after the deferred Digest Auth) sharing the same `(?i)` case-insensitivity contract on the auth-scheme literal per RFC 7235 §2.1 (referenced by RFC 4559 §4 — HTTP SPNEGO Authentication for Kerberos and NTLMSSP via GSSAPI). Pre-fix `src/utils/secret_scanner.py` had NO Negotiate detector at all — the only coverage for a leaked `Authorization: Negotiate <body>` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`) which matched the base64-encoded GSSAPI body generically, BUT only when the body had ≥2 character categories AND was not all-letter (the `candidate.isalpha()` short-circuit in `_HIGH_ENTROPY_RE`'s loop suppresses LongCamelCaseClassName false positives). Two orthogonal failure modes resulted: **(1) Attribution drift** — 200+ char Kerberos AP-REQ / AS-REP base64 bodies with mixed character classes (the common case — ASN.1 DER envelope + encrypted ticket combines digits, upper, lower, `+/=`) lost the SPNEGO-specific attribution and landed as generic `Hochentropischer Token-String`, so incident-response triage had to GUESS the revocation playbook (SPNEGO/Kerberos: revoke the service ticket at the KDC via Kerberos `kadmin` / Active Directory `setspn`, force user re-authentication via `klist purge` + `kinit`, audit the affected service principal for replay activity within the ticket's typical 8-10h lifetime per RFC 4120 §5.3 `EndTime` default; Bearer: revoke the opaque token at the issuing IdP; Basic Auth: rotate the user's password; opaque API key: revoke at the vendor dashboard — four entirely different IR flows that hinge on per-scheme attribution). **(2) Silent undetection** — all-letter base64 bodies trip `candidate.isalpha()` and are skipped by the entropy fallback. Pre-fix every leaked `Negotiate <all-letter-body>` was SILENTLY UNDETECTED — the CI gate passed, the encrypted Kerberos ticket sat committed in the public repo for the full ticket validity window (8-10h). End-to-end PoC verified pre-fix across every case variation (`negotiate` / `NEGOTIATE` / `NeGoTiAtE` / `nEgOtIaTe` / canonical `Negotiate`) and every body class (long-mixed 240-char, all-letter 62-char, canonical `YII`-prefixed Kerberos AP-REQ shape): only `Verdächtige Zuweisung eines potentiellen Secrets` or `Hochentropischer Token-String` fired (when the variable name happened to carry a sensitive keyword or the body was mixed-class above 24 chars), losing all SPNEGO-specific attribution.

**Threat model:** A leaked SPNEGO/Negotiate credential is structurally distinct from Bearer/Basic Auth leaks in that **the body is a base64-encoded ASN.1-DER GSSAPI token wrapping either a Kerberos AS-REP / AP-REQ message or an NTLMSSP Type 1/2/3 message** per RFC 4178 (SPNEGO) and [MS-NLMP] (NTLMSSP). An attacker who recovers a Negotiate body therefore holds: (a) **the encrypted service ticket** — replayable as the original user against the target service for the ticket's validity window (typical 8-10h KDC default per RFC 4120 §5.3); the 5-minute authenticator clock-skew check (RFC 4120 §3.2) does not fully mitigate replay on short timescales; (b) **the realm name** — the ASN.1 `realm` field is unencrypted in the AS-REQ message and identifies the target Kerberos realm for further attacks; (c) **the SPNEGO mech-types** — identifies the available authentication mechanisms (Kerberos / NTLMSSP / NEGOEX) at the target, helping attackers pivot to the weakest available; (d) **AS-REP roasting amplifier** — if the leaked token contains an AS-REP message with encrypted PA-DATA (less common but possible during error/debug logging when pre-authentication is disabled), the encrypted portion can be subjected to offline dictionary/brute-force attack with tools like `hashcat` (mode 18200 for AS-REP / 19600 for TGS-REP); (e) **NTLMSSP exposure** — when the Negotiate envelope wraps an NTLMSSP message, NTLMv2 responses leaked in Type 3 messages are vulnerable to NetNTLMv2 relay attacks (using `responder` / `ntlmrelayx`) and offline cracking with `hashcat` (mode 5600). Real-world emission patterns include IIS HTTP request logs with `--debug` flag (the entire Authorization header is logged verbatim per `UseAppPoolQueueLength` diagnostic mode), browser dev-tools Network tab exports (`Save HAR with content` captures the Authorization header verbatim into the HAR JSON), Wireshark/tshark capture exports rendered as text, WinRM debug logs (`Set-PSDebug -Trace 2`) emitting the Authorization header during the negotiation round trip, `curl -v --negotiate -u :` debug logs, Python `requests` with `requests-kerberos` debug mode (`logging.DEBUG` on the `requests_kerberos` logger), Spring Security `org.springframework.security.kerberos` debug logging, and ELK Stack ingest of `WWW-Authenticate` response headers. The blast radius of an undetected Negotiate leak is COMPARABLE to a Basic Auth leak in the worst case (full Kerberos credential exposure via AS-REP roasting recovers the user's password hash, which can be cracked offline to recover the plaintext password — feeding the same password-reuse amplifier chain).

**Sites closed (1 detector regex + 1 table entry):**

  * `src/utils/secret_scanner.py:_NEGOTIATE_RE` — new regex appended to `_AUTH_SCHEME_DETECTORS`:
    ```python
    ...
    ```
    The body alphabet `[A-Za-z0-9+/=]` is the canonical base64 alphabet per RFC 4648 §4 (standard base64; RFC 4559 §4 references the original base64 encoding per RFC 1421, which uses `+/=` not URL-safe `_-`). The 50+ char body floor is the structural disambiguator — real Kerberos AS-REP / AP-REQ tokens are 200-3000+ chars base64-encoded (the ASN.1 DER envelope alone is ~100 bytes, the encrypted ticket adds 200-2000+ bytes for typical service principals), so 50 is a safe conservative floor that catches even truncated tokens in fragmented log lines while preventing natural-language false positives (the English word "negotiate" appears commonly in code comments and prose, but never followed by 50+ contiguous chars from `[A-Za-z0-9+/=]` because English words break at whitespace and punctuation). The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer/Basic siblings.

  * `_AUTH_SCHEME_DETECTORS` table extension — the canonical `_scan_auth_scheme_credentials` helper extracted by the 2026-05-16 Basic Auth round (single source of truth for HTTP-auth-scheme detector processing) processes the new entry uniformly via the existing iteration loop with the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1` so uniform-character-class bodies are NOT silently undetected) and `covered_ranges` mutation contract. No changes needed to `_scan_content` (C901 complexity unchanged at 29) or to `_scan_auth_scheme_credentials` (no new branches). The structural invariant established by the 2026-05-16 Basic Auth round held end-to-end: adding a new HTTP-auth-scheme detector requires only a new regex constant + a new tuple entry — the canonical processing helper handles it without modification.

**Severity:** **MEDIUM-HIGH** — detection-from-zero failure for a class of realistic SPNEGO/Kerberos credential leaks in enterprise environments (Active Directory, IIS, WinRM, Spring Security with Kerberos integration). The high-severity case is the silent-undetection branch (all-letter bodies): the entire encrypted Kerberos ticket sits committed in plaintext with NO detection at all for the full ticket validity window (8-10h). The attribution-drift branch is medium-severity (the body IS caught generically, but the SPNEGO-specific reason that determines which IR playbook applies — KDC service ticket revocation via `kadmin` / Active Directory `setspn`, force user re-auth via `klist purge` + `kinit`, audit service principal for replay activity — is lost). Mitigated only by the requirement that the leaked credential live alongside a `Negotiate` auth-scheme literal in the same content blob; in practice this covers every leak through IIS debug logs, browser HAR exports, Wireshark/tshark captures, WinRM diagnostics, and curl/requests-kerberos debug output. CodeQL / Semgrep do not flag this drift because their default rules cover the Bearer auth-scheme but not Negotiate (the canonical Negotiate detector requires a per-rule body-alphabet and body-length disambiguator, which is rule-table dependent — no industry-standard default).

**Fix:** Two-part change:
1. Add `_NEGOTIATE_RE = re.compile(r"(?i)Negotiate\s+([A-Za-z0-9+/=]{50,})")` alongside `_BEARER_RE` and `_BASIC_AUTH_RE` (single source of truth for the auth-scheme detector body shape).
2. Append `(_NEGOTIATE_RE, "SPNEGO/Negotiate Authentication Token gefunden")` to `_AUTH_SCHEME_DETECTORS` — the canonical iteration helper `_scan_auth_scheme_credentials` (extracted by the 2026-05-16 Basic Auth round as a one-time refactor) processes the new entry uniformly. No code changes elsewhere — the helper preserves byte-for-byte behaviour of the Bearer/Basic detectors while extending coverage to Negotiate.

Zero false-positive risk: the body alphabet `[A-Za-z0-9+/=]` is identical to `_BASIC_AUTH_RE`'s (no `-_.` like Bearer); the structural disambiguator (`\s+` between the auth-scheme literal and a 50+ char body) is identical in shape to Basic's (and Bearer's, both at 16+ chars — Negotiate raised to 50+ because real GSSAPI tokens are massively longer and the higher floor further suppresses already-rare natural-language false positives). Natural-language false positives are bounded by the same logic that protects Bearer/Basic: an English sentence containing "Negotiate" cannot produce 50+ contiguous chars from the base64 alphabet because words are separated by whitespace. The `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`) catches any token-shaped string that does happen to follow a `negotiate`-prefixed natural-language passage — the same defence Bearer/Basic already rely on.

**Tests added:** `tests/test_sentinel_secret_scanner_negotiate_drift.py` — 36 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × 240-char long-mixed body) — proves attribution-drift closes for every RFC-7235-conforming Negotiate literal.
  2. Per-case silent-undetection PoCs (4 case variants × 62-char all-letter body) — proves `candidate.isalpha()` skip bypass closes for Negotiate-prefixed tokens.
  3. Negative cases (12 natural-language passages, code-shape candidates, and a 49-char body just-below-floor) — proves false-positive surface is bounded.
  4. Cross-detector ordering regression guard — proves JWT in Bearer header still yields JWT attribution (the `_KNOWN_TOKENS` matching runs BEFORE auth-scheme detectors).
  5. Bearer regression guard — proves canonical Bearer continues to detect after the Negotiate addition.
  6. Basic Auth regression guard — proves canonical Basic Auth (the 2026-05-16 sibling) continues to detect after the Negotiate addition.
  7. Compiled-regex invariant — pins `_NEGOTIATE_RE.flags & re.IGNORECASE` programmatically against future case-sensitivity regression.
  8. Auth-scheme detector table membership invariant — pins `_NEGOTIATE_RE` membership in `_AUTH_SCHEME_DETECTORS` so a future regression that adds the regex but forgets the table binding fails fast.
  9. End-to-end emission-shape inventory (8 real-world emission patterns: curl debug logs, IIS debug logs, browser HAR exports, Python/JSON fixtures, YAML config, WinRM debug traces, Wireshark text exports).
 10. Masking-contract test — proves the raw base64 GSSAPI body never appears unmasked in finding output (mirrors Bearer/Basic masking-contract tests). PLUS a Kerberos AP-REQ structural-prefix demonstration documenting the canonical `YII` base64 prefix (base64 of the ASN.1 SEQUENCE outer tag `0x60 0x82`) as an incident-response anchor for distinguishing Kerberos from NTLMSSP-wrapped Negotiate tokens (NTLMSSP base64 starts with `TlRMTVNTUA` = base64("NTLMSSP\0")).

**Learning:** The HTTP-auth-scheme detector family is a closed enumeration per the RFC 7235 §2.1 case-insensitivity contract. The 2026-05-16 Basic Auth round's `_AUTH_SCHEME_DETECTORS` tuple + `_scan_auth_scheme_credentials` helper architecture proved end-to-end: adding a third RFC-defined auth-scheme (Negotiate per RFC 4559) required ONLY a new regex constant + a new tuple entry. ZERO changes to the iteration helper, ZERO changes to `_scan_content`, ZERO C901 complexity impact. The structural invariant pinned by the Basic Auth round held byte-for-byte. **The body-length floor is the per-scheme structural disambiguator** — Bearer/Basic use 16+ chars (matches short Basic Auth credentials like base64("admin:password") = 20 chars), Negotiate uses 50+ chars (matches truncated GSSAPI tokens while excluding natural-language false positives where the English word "negotiate" might be followed by short identifier names). Future auth-scheme detectors choose the floor per the minimum realistic credential length for that scheme's RFC-defined structure. **Per-scheme body alphabet** is determined by the RFC's body encoding — Bearer (RFC 6750) allows `[A-Za-z0-9\-_.]` per `token68` ABNF (URL-safe-ish), Basic (RFC 7617) and Negotiate (RFC 4559 referencing RFC 1421) use standard base64 `[A-Za-z0-9+/=]`. Future auth-scheme detectors pick the body alphabet per the RFC.

**Next-round candidates (named-but-deferred):** **NTLM detector** (`(?i)NTLM\s+[A-Za-z0-9+/=]{50,}`) — Microsoft NTLM directly as an HTTP auth-scheme (without SPNEGO wrapping); used by IIS for legacy clients and SMB-over-HTTP scenarios. The literal `NTLM` is vendor-specific per [MS-NLMP] (not IETF-RFC-defined like Negotiate), creating a structural classification question: belongs to `_AUTH_SCHEME_DETECTORS` (because it IS an HTTP auth-scheme literal) OR `_KNOWN_TOKENS` (because [MS-NLMP] is a vendor spec, not an IETF RFC)? Per the structural invariant established by the Basic Auth round ("HTTP-auth-scheme literals go to `_AUTH_SCHEME_DETECTORS`"), NTLM should go in the auth-scheme table — the canonical processing path already handles it without modification. **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth still deferred pending body-structure-aware detector framework (the credential is split across the `nonce` / `response` / `cnonce` fields, so a single-regex anchor doesn't fully capture the secret — context-aware extraction needed). **GitHub `Token` scheme detector** (`(?i)token\s+[A-Za-z0-9_\-]{36,}`) — GitHub's older API alias for Bearer (`Authorization: token ghp_...`); the embedded GitHub PAT (`ghp_`-prefixed) already matches `_KNOWN_TOKENS` via the `ghp_` regex, so the `token` scheme detector is attribution-only for opaque non-GitHub tokens that happen to use the alias. **HOBA detector** (`(?i)HOBA\s+...`) — RFC 7486 HTTP Origin-Bound Auth; rare in practice but a complete auth-scheme enumeration would include it. **Mutual auth detector** (`(?i)Mutual\s+...`) — RFC 8120 HTTP Mutual Authentication using cryptographic keys; very rare.

**Marker:** SENTINEL_NEGOTIATE_DRIFT.

## 2026-05-16 - Basic Auth Detector Drift (RFC 7617 HTTP Basic Authentication, Sibling Of Bearer Case-Insensitivity Round): The Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-15 Bearer Case-Insensitivity Round Closed In Full — A Leaked `Authorization: Basic <base64(user:password)>` In Committed Source Carried Two Orthogonal Failure Modes (Attribution Drift For 24+ Char Bodies, Silent Undetection For Short Or All-Letter Bodies) That Lost The Plaintext `username:password` Recovery Surface Entirely

**Vulnerability:** The 2026-05-15 Bearer case-insensitivity round (`security(secret-scanner): pin Bearer detector to case-insensitive per RFC 7235`) named **Basic Auth detector** as the immediate adjacent-detector candidate sharing the same `(?i)` case-insensitivity contract on the auth-scheme literal per RFC 7235 §2.1 (referenced by RFC 7617 §2 — HTTP Basic Authentication). Pre-fix `src/utils/secret_scanner.py` had NO Basic Auth detector at all — the only coverage for a leaked `Authorization: Basic <body>` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`) which matched the base64-encoded body span generically, BUT only when the body was long enough (24+ chars) AND contained ≥2 character categories AND was not all-letter (the `candidate.isalpha()` short-circuit in `_HIGH_ENTROPY_RE`'s loop suppresses LongCamelCaseClassName false positives). Two orthogonal failure modes resulted: **(1) Attribution drift** — 24+ char base64 bodies with mixed character classes lost the Basic-Auth-specific attribution and landed as generic `Hochentropischer Token-String`, so incident-response triage had to GUESS the revocation playbook (Basic Auth: rotate the user's password AND audit downstream systems where the same password may be reused via password-reuse amplification; Bearer: revoke the opaque token at the issuing IdP; opaque API key: revoke at the vendor dashboard). **(2) Silent undetection** — short bodies (16-23 chars, e.g. `YWRtaW46cGFzc3dvcmQ=` = `admin:password` is only 20 chars) fall BELOW the entropy fallback's 24-char minimum entirely; all-letter base64 bodies trip `candidate.isalpha()` and are skipped by the entropy fallback. Pre-fix every leaked `Basic <short-or-all-letter-body>` was SILENTLY UNDETECTED — the CI gate passed, the plaintext `username:password` pair (trivially recovered via `base64.b64decode(body).decode()`) sat committed in the public repo indefinitely. End-to-end PoC verified pre-fix across every case variation (`basic` / `BASIC` / `BaSiC` / `bAsIc` / canonical `Basic`) and every body class (short, long-mixed, all-letter): only `Verdächtige Zuweisung eines potentiellen Secrets` fired (and only when the variable name happened to carry a sensitive keyword), losing all Basic-Auth-specific attribution.

**Threat model:** A leaked Basic Auth credential carries whatever scope the issuing service grants the `username:password` pair. The structurally critical distinction from Bearer Auth is that **the body is a reversible base64 encoding of the cleartext `username:password` string** — `base64.b64decode("YWRtaW46cGFzc3dvcmQ=").decode("ascii")` returns `"admin:password"`. An attacker who recovers a Basic Auth body therefore holds (a) the username (which may be a system administrator account, service account, or canonical project owner — directly identifying the leaking entity), (b) the plaintext password (which may be reused across other systems by the same user — the canonical real-world IR-amplifier for credential leaks), and (c) by inference, the existence of an HTTP-Basic-Auth-protected service that the credential pair authenticates to. Real-world emission patterns include curl `-u user:pass` debug logs (Basic Auth is curl's default `--user` shape), httpie `--auth=user:pass` debug logs, ASP.NET / Spring Boot / Django Basic Auth Authorization headers committed to test fixtures, hand-typed examples in documentation or migration guides, and hostile-PR fragments mimicking existing canonical-case detector evasion. The blast radius of an undetected Basic Auth leak is HIGHER than an undetected Bearer leak: a leaked Bearer is opaque so the attacker can only use it as-is until revocation, but a leaked Basic Auth provides the *cleartext password* that may be reused across other systems by the same user (password reuse is the dominant real-world account-takeover amplifier per every published breach analysis: LinkedIn 2012, Adobe 2013, Yahoo 2013/2014, Equifax 2017, the Marriott Starwood 2018 chain).

**Sites closed (1 detector regex + 1 helper extraction):**

  * `src/utils/secret_scanner.py:_BASIC_AUTH_RE` — new regex tuple appended to a new `_AUTH_SCHEME_DETECTORS` table:
    ```python
    ...
    ```
    The body alphabet `[A-Za-z0-9+/=]` is the canonical base64 alphabet per RFC 4648 §4 (standard base64; URL-safe RFC 4648 §5 uses `_-` instead of `+/` but is rare in Authorization headers per RFC 7617 §2 which prescribes standard base64). The 16+ contiguous body length is the structural disambiguator against natural-language false positives — sentences like "Basic understanding of..." do NOT have 16+ contiguous chars from the base64 alphabet following the literal (English words break at spaces and punctuation). The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer round.

  * `src/utils/secret_scanner.py:_scan_auth_scheme_credentials` — new helper extracted from `_scan_content` to keep that function at its existing C901 complexity baseline (29) while still adding the new detector. The helper iterates `_AUTH_SCHEME_DETECTORS` uniformly, applies the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1`) that the original Bearer loop used (so uniform-character-class bodies are NOT silently undetected), and mutates `covered_ranges` in place so subsequent detector loops correctly suppress the same span. Future detectors (RFC 7616 Digest, RFC 4559 SPNEGO, RFC 7486 HOBA) extend `_AUTH_SCHEME_DETECTORS` without changing the helper or `_scan_content` — single point of audit for every HTTP-auth-scheme detector.

**Severity:** **MEDIUM-HIGH** — detection-from-zero failure for a class of realistic Basic Auth credential leaks. The high-severity case is the silent-undetection branch (short bodies < 24 chars OR all-letter bodies): the entire `username:password` pair sits committed in plaintext with NO detection at all. The attribution-drift branch is medium-severity (the body IS caught generically, but the Basic-Auth-specific reason that determines which rotation playbook applies — rotate the user's password, audit password-reuse cross-system — is lost). Mitigated only by the requirement that the leaked credential live alongside a `Basic` auth-scheme literal in the same content blob; in practice this covers every leak through automation-generated fixtures, lowercase-normalising loggers, hostile-PR-introduced fragments, and curl/httpie debug logs. CodeQL / Semgrep do not flag this drift because their default rules cover the Bearer auth-scheme but not Basic (the canonical Basic detector requires a per-rule body-alphabet disambiguator, which is rule-table dependent — no industry-standard default).

**Fix:** Three-part change:
1. Add `_BASIC_AUTH_RE = re.compile(r"(?i)Basic\s+([A-Za-z0-9+/=]{16,})")` alongside `_BEARER_RE` (single source of truth for the auth-scheme detector body shape).
2. Add `_AUTH_SCHEME_DETECTORS` tuple binding each regex to its Basic-Auth-specific reason — placing both detectors in a single iterable so future auth-scheme additions (Digest, SPNEGO, HOBA) follow the same shape without modifying `_scan_content`.
3. Extract `_scan_auth_scheme_credentials(content, covered_ranges, line_resolver)` helper from the inline Bearer loop in `_scan_content`. The helper preserves the original Bearer detector's behaviour byte-for-byte (same `_looks_like_secret` filter, same `is_covered` semantics, same mutation of `covered_ranges`) while routing both detectors through one canonical implementation. The extraction REDUCES `_scan_content`'s C901 complexity by 2 (eliminates one inline detector loop) while ADDING the Basic detector — net cyclomatic complexity at the function level: unchanged (still 29, matches `.c901-baseline.txt`).

Zero false-positive risk: the body alphabet `[A-Za-z0-9+/=]` is strictly narrower than `_BEARER_RE`'s `[A-Za-z0-9\-_.]` (no `-`, no `_`, no `.`); the structural disambiguator (`\s+` between the auth-scheme literal and a 16+ char body) is identical to Bearer's. Natural-language false positives are bounded by the same logic that protects Bearer: an English sentence containing "Basic" cannot produce 16+ contiguous chars from the base64 alphabet because words are separated by whitespace. The `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`) catches any token-shaped string that does happen to follow a `basic`-prefixed natural-language passage — the same defence Bearer already relies on.

**Tests added:** `tests/test_sentinel_secret_scanner_basic_auth_drift.py` — 32 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × long-mixed body) — proves attribution-drift closes for every RFC-7235-conforming Basic literal.
  2. Per-case silent-undetection PoCs (4 case variants × short body) — proves short-body credentials (below entropy fallback's 24-char minimum) gain attribution.
  3. Per-case all-letter body PoCs (4 case variants × 40-char all-letter body) — proves `candidate.isalpha()` skip bypass closes.
  4. Negative cases (7 natural-language passages) — proves false-positive surface is bounded.
  5. Cross-detector ordering regression guard — proves JWT in Bearer header still yields JWT attribution (the `_KNOWN_TOKENS` matching runs BEFORE auth-scheme detectors).
  6. Bearer regression guard — proves canonical Bearer continues to detect after the refactor.
  7. Compiled-regex invariant — pins `_BASIC_AUTH_RE.flags & re.IGNORECASE` programmatically.
  8. End-to-end emission-shape inventory (6 real-world emission patterns: curl debug logs, JSON fixtures, YAML config, Python dicts).
  9. Masking-contract test — proves the raw base64 body never appears unmasked in finding output (mirrors Bearer's masking-contract test).
 10. Decoded-credential demonstration — documents the threat-model rationale (the body decodes back to `admin:password`, distinct from opaque Bearer).

**Learning:** The HTTP-auth-scheme detector family is a closed enumeration per the RFC 7235 §2.1 case-insensitivity contract. Pinning every detector in a shared `_AUTH_SCHEME_DETECTORS` table with a uniform processing path (single helper `_scan_auth_scheme_credentials`) defeats the drift mode where one detector gets the `(?i)` flag and a sibling does not. **The structural invariant for future auth-scheme detectors: any new HTTP-auth-scheme literal added MUST be a `_AUTH_SCHEME_DETECTORS` entry, NOT a separate `_KNOWN_TOKENS` entry or inline `_scan_content` loop.** The `_KNOWN_TOKENS` table is for VENDOR-prefixed tokens (canonical issuer prefix anchors the credential to its revocation flow — `ghp_`, `glpat-`, `xkeysib-`, etc.); the `_AUTH_SCHEME_DETECTORS` table is for IETF-RFC-defined auth-scheme literals (canonical HTTP authentication scheme anchors the credential to its RFC-defined recovery semantics — `Bearer` is opaque per RFC 6750, `Basic` is reversible base64 per RFC 7617). The closing-checklist invariant for future Sentinel rounds: every detector regex whose anchor is an HTTP-RFC-defined auth-scheme literal MUST be added to `_AUTH_SCHEME_DETECTORS` (NOT `_KNOWN_TOKENS`), inherit the `(?i)` inline flag per RFC 7235 §2.1, and route through `_scan_auth_scheme_credentials`. The 16+ char body-alphabet structural disambiguator + the `is_assignment=True` `_looks_like_secret` filter together prevent natural-language false positives without per-detector bikeshedding.

**Next-round candidates (named-but-deferred):** **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth; the credential is split across the `nonce` / `response` / `cnonce` fields, so a single-regex anchor doesn't fully capture the secret — context-aware extraction needed, deferred pending body-structure-aware detector framework. **GitHub `Token` scheme detector** (`(?i)token\s+[A-Za-z0-9_\-]{36,}`) — GitHub's older API alias for Bearer (`Authorization: token ghp_...`); the embedded GitHub PAT (`ghp_`-prefixed) already matches `_KNOWN_TOKENS` via the `ghp_` regex, so the `token` scheme detector is attribution-only for opaque non-GitHub tokens that happen to use the alias. **SPNEGO detector** (`(?i)Negotiate\s+[A-Za-z0-9+/=]{50,}`) — RFC 4559 Kerberos via SPNEGO; the body is a base64-encoded GSSAPI token, body lengths typically much longer than Basic Auth (200+ chars per Kerberos ticket). **HOBA detector** (`(?i)HOBA\s+...`) — RFC 7486 HTTP Origin-Bound Auth; rare in practice but a complete auth-scheme enumeration would include it.

**Marker:** SENTINEL_BASIC_AUTH_DRIFT.

## 2026-05-16 - Default Env-File Candidate Symlink-Following Bypass: `_default_env_file_candidates` Skipped Symlink-Aware Resolution For Its Three *Built-In* Default Paths (`<base_dir>/.env`, `<base_dir>/data/secrets.env`, `<base_dir>/config/secrets.env`) — A Hostile PR Landing A Symlink At Any Of The Three Documented Default Paths Pointing Outside `base_dir` Was A Single-Precondition Arbitrary-File-Read Primitive Whose Blast Radius Equalled The `WIEN_OEPNV_ENV_FILES` Bypass (KEY=VALUE Pairs From `/etc/environment` / `~/.aws/credentials` Loaded Into `os.environ`) But With Half The Preconditions

**Vulnerability:** Sibling-drift closure of the `WIEN_OEPNV_ENV_FILES` symlink-following bypass (the previous entry in this journal). The pre-fix `_default_env_file_candidates` applied `Path.resolve(strict=False)` + `relative_to(base_dir)` containment ONLY to the env-controlled `WIEN_OEPNV_ENV_FILES` extras; the THREE built-in default paths (`<base_dir>/.env`, `<base_dir>/data/secrets.env`, `<base_dir>/config/secrets.env`) were kept as bare lexical `base_dir / "..."` Path objects with NO symlink-aware check. The bare lexical Path opens its target via `path.open` (which follows symlinks) so a symlink planted at any default location pointing OUTSIDE `base_dir` — e.g. `<base_dir>/.env -> /etc/environment`, `<base_dir>/data/secrets.env -> ~/.aws/credentials`, `<base_dir>/config/secrets.env -> /proc/self/environ` — turned every default candidate into an arbitrary-file-read primitive. Pre-fix `load_env_file(<symlink>)` opened the link (`path.exists()` and `path.is_file()` both follow symlinks), `read_capped_text` followed the link to the outside target (capped at `MAX_ENV_FILE_BYTES`), `_parse_env_file` parsed every `KEY=VALUE` line, and the loader wrote each variable into `os.environ` (`override=False` default). End-to-end PoC verified pre-fix across all THREE default paths simultaneously: `SENTINEL_DEFAULT_TRIPLE_DOT_ENV_LEAK` + `SENTINEL_DEFAULT_TRIPLE_DATA_SECRETS_LEAK` + `SENTINEL_DEFAULT_TRIPLE_CONFIG_SECRETS_LEAK` from three distinct outside-base files all landed in `os.environ` in a single `load_default_env_files()` call. **Severity MEDIUM** — the single-precondition shape is meaningfully different from the env-controlled round which needed TWO preconditions (filesystem planting + env var injection). The default-candidate branch needs only ONE precondition: filesystem write to any of the three publicly-documented default paths via (a) a hostile PR submitted against the repository — Git tracks symlinks as files with the `120000` mode, and merging the PR lands the link in `main` where every subsequent cron tick re-materialises it; (b) a pre-compromised CI runner or multi-tenant shared workspace where another tenant's job leaves a symlink at the documented default path; (c) a misconfigured development environment that accidentally generates a symlink pointing outside the project tree. Code review may not flag a new symlink as suspicious unless reviewers specifically inspect the mode bits — git diff UI shows symlinks as ordinary text files with the link target as the file content, easily mistaken for a tiny config snippet.

**Learning:** The structural lesson is that when a security defence is added to ONE branch of a multi-branch function, every sibling branch that hits the SAME downstream sink must be audited in lockstep. The previous round (`WIEN_OEPNV_ENV_FILES` bypass) added `Path.resolve(strict=False)` + `relative_to(base_dir)` to the env-controlled branch but left the default-candidate branch as bare lexical Path objects — same downstream sink (`load_env_file` → `read_capped_text` → `_parse_env_file` → `os.environ`), same blast radius, half the preconditions. The asymmetry was non-obvious because the default candidates LOOKED safe by construction (`base_dir / ".env"` is lexically inside `base_dir`, so why would a containment check matter?) — but the lexical-vs-resolved distinction the previous round established is precisely the gap: `<base_dir>/.env` is lexically inside `base_dir` regardless of whether the filesystem entry is a regular file or a symlink to `/etc/environment`. **Any path-containment defence at a security boundary must apply uniformly to every path that flows into the same sink** — the sink (here: `load_env_file`) is the canonical "I will follow symlinks" point; every path reaching the sink must have been resolved + checked. The fix extracts the resolve-and-check logic into a single `_resolve_within_base(candidate, base_dir)` helper that BOTH branches route through. Pinning a helper rather than parallel inline checks defeats the drift mode that produced this very vulnerability: future maintenance that touches one branch cannot silently bypass the defence on the other. Mathematical invariant: a function that returns candidates flowing into a symlink-aware sink must define a "candidate validity" predicate that is closed under composition — every candidate-producing branch must hit the SAME predicate, NOT a parallel inline approximation.

**Prevention:** Extract every path-containment defence into a named helper (here: `_resolve_within_base`) so every candidate-producing branch routes through one canonical implementation. The structural invariant test `test_default_env_file_candidates_routes_through_resolver` in `tests/test_sentinel_default_env_file_symlink_bypass.py` greps the function source for the helper name and fails if a future refactor drops the routing — pinning the routing prevents the parallel-inline regression that produced this round. Companion PoC test exercises all three default candidate paths (parametrised) plus a triple-simultaneous-leak scenario verifying that NO subset of the three planted symlinks leaks into `os.environ`. Future audits should walk `git grep -nE 'def _.*candidates' src/` to enumerate every candidate-list builder and verify each candidate flows through the canonical resolver. Generalisation: when an existing security helper is extracted from one branch, audit every sibling branch that hits the same downstream sink — the audit grep is `git grep -nE 'load_env_file\(' src/` for this specific sink; future audits of new sink families should adapt the pattern. The new `_resolve_within_base` helper also serves as a single point of audit: any future change to the containment semantics (e.g. tightening the strict=False default, adding a per-component symlink hop limit, switching from `relative_to` to a stricter prefix-bytes check) propagates to every caller automatically.

## 2026-05-16 - `WIEN_OEPNV_ENV_FILES` Symlink-Following Bypass: `_default_env_file_candidates` Used `os.path.abspath` (Lexical-Only Normalisation) Instead of `Path.resolve()` (Symlink-Aware) — A Symlink Planted Inside `base_dir` Pointing OUTSIDE (e.g. `<base_dir>/decoy.env -> /etc/environment`) Passed the `relative_to(base_dir)` Containment Check And Let `load_env_file` Read Arbitrary Filesystem Locations Via The Env-Controlled `WIEN_OEPNV_ENV_FILES` Path List
**Vulnerability:** `src/utils/env.py:_default_env_file_candidates` accepted `WIEN_OEPNV_ENV_FILES` entries by resolving them via `Path(os.path.abspath(candidate))` and checking containment via `relative_to(base_dir)`. `os.path.abspath` performs ONLY lexical normalisation (joins relative paths against `os.getcwd()` and resolves `.`/`..` parts via string manipulation); it does NOT follow symlinks. So a symlink planted inside `base_dir` pointing OUTSIDE `base_dir` — e.g. `<base_dir>/decoy.env -> /etc/environment`, `<base_dir>/decoy.env -> ~/.aws/credentials`, `<base_dir>/decoy.env -> /proc/self/environ` — passed the lexical containment check verbatim (the lexical path `<base_dir>/decoy.env` IS inside `base_dir`) while resolving to a filesystem destination FAR outside the intended boundary. Pre-fix `load_env_file(decoy.env)` then opened the symlink, followed it to the outside target, and `read_capped_text` read the target's bytes (up to `MAX_ENV_FILE_BYTES`). `_parse_env_file` parsed every `KEY=VALUE` line and the loader wrote each variable into `os.environ` (when `KEY` is not already set, per the `override=False` default). End-to-end PoC verified pre-fix: `STOLEN_SYMLINK_LEAK=pwned_value` from `/tmp/outside/stolen.env` landed in `os.environ` despite the symlink target lying entirely outside `base_dir`. **Severity MEDIUM** — defense-in-depth violation that breaks the explicit invariant the `relative_to(base_dir)` check was added to enforce; the two preconditions (filesystem planting + env var injection) are sometimes obtainable independently (a hostile PR that lands a symlink in `main` + a leaked CI env that sets `WIEN_OEPNV_ENV_FILES`).
**Learning:** The canonical path-containment pattern across `src/` already uses `Path.resolve()` consistently at the symlink-aware boundary: `read_secret` (`src/utils/env.py:188-189`) calls `Path(cred_dir).resolve()` then `(base_dir / name).resolve()` before `relative_to`; `validate_path` (`src/feed/config.py:211`) calls `path.resolve()` before iterating the allow-listed bases. `_default_env_file_candidates` was the **outlier sibling** that used `os.path.abspath` — the lexical-only sibling primitive that looks shape-equivalent but breaks the symlink invariant. The asymmetry is non-obvious: both produce absolute, normalised paths, but only `.resolve()` traverses symlinks. Any `relative_to(base_dir)` check at a SECURITY boundary (where the question is "where on the filesystem does this path actually point?") MUST resolve symlinks; only `.resolve()` answers that question. `os.path.abspath` answers a DIFFERENT question ("what does this path string look like when normalised lexically?") which is correct for thread-lock-key normalisation (`src/utils/locking.py:178`) but wrong for filesystem-boundary checks. The mathematical invariant: `Path.resolve()` is the unique canonical containment primitive — any sibling site that needs to answer "is this path actually inside the allowed sub-tree?" must use it. Audit grep for the dual: `git grep -nE 'os\.path\.abspath\(' src/` enumerates every site to inspect. `src/utils/files.py:safe_path_join` also uses `os.path.abspath` but is reached via the `_cache_file` regex-constrained provider name (`^[a-zA-Z0-9_-]+$`), so the symlink-planting precondition reduces to filesystem write access to `_CACHE_DIR` (game-over already) — the env.py site is the higher-priority closure because the env-controlled list IS the attack surface.
**Prevention:** Replace `Path(os.path.abspath(candidate))` with `candidate.resolve(strict=False)` at every env-controlled / network-controlled / user-controlled path-containment boundary. The `strict=False` argument preserves the pre-fix behaviour for non-existent paths (the lexical part resolves and the missing tail is appended) while still following every existing symlink hop along the path. Catch `OSError` (symlink loops via `ELOOP`, I/O errors) and `RuntimeError` (CPython infinite-loop guard on some Python versions) so the boundary fails closed — the secure default for any filesystem state that prevents full resolution. The companion `tests/test_sentinel_env_file_symlink_bypass.py` pins seven attack scenarios (direct symlink-to-outside, nested-symlink-chain, symlink-loop, pathsep-mixed legitimate-and-bypass, absolute-path-outside regression, `../` traversal regression, legitimate-symlink-staying-inside-base acceptance) PLUS a structural invariant test that fails any future refactor that re-introduces an `os.path.abspath(...)` call at the candidate boundary. Future audits should walk `git grep -nE 'os\.path\.abspath\([^)]*candidate' src/` to enumerate any drift sibling that creeps back in.

## 2026-05-16 - Entropy Heuristic Long-Base64 Detection Gap: `_looks_like_secret` Uniqueness Floor `len(set) >= len // 4` Was Mathematically Unsatisfiable For Multi-Hundred-Char Base64 Credentials (AWS STS Session Tokens, GCP Service-Account Bodies, Long Opaque OAuth Tokens) Whose Alphabet Plateaus At ~64 Distinct Chars While The Requirement Demanded 71+
**Vulnerability:** `src/utils/secret_scanner.py:_looks_like_secret` rejected any candidate whose distinct-character count fell below `max(6, len(candidate) // 4)`. For candidates longer than ~256 chars whose alphabet is the base64 ceiling (`[A-Za-z0-9+/=_-]` — at most 65 unique characters total), the `len // 4` requirement is mathematically impossible: a 287-char token has at most 65 unique chars but needs 71+ to pass. The gate applies to BOTH the entropy fallback (`_HIGH_ENTROPY_RE` → `_looks_like_secret(is_assignment=False)`) AND the assignment heuristic (`_SENSITIVE_ASSIGN_RE` → `_looks_like_secret(is_assignment=True)`), so a real-world AWS STS Session Token leaked in `.env`, log lines, JSON fixtures, or any plaintext context was **silently undetected entirely** — no finding produced by ANY detection branch, CI gate passes, credential commits to `main` history, and remains exploitable for the natural session-token validity window (15 min to 12 h depending on the originating session, long enough for credential pivoting via `iam:CreateAccessKey` or other privilege-escalation flows). The gap generalises to every long-base64 credential class: GCP service-account JSON `private_key` body (covered structurally by `_PEM_RE` for the `-----BEGIN PRIVATE KEY-----` block, but the bare-body shape was uncovered), Auth0 / Microsoft Graph access tokens > 256 chars, custom-IdP opaque tokens, and any future vendor token whose canonical format exceeds the breakpoint. **Severity HIGH** for the AWS-session-token case — silent CI gate bypass on a credential class with multi-hour validity and full IAM-scope blast radius.
**Learning:** The previous entropy-heuristic rounds (`_BEARER_RE` case-insensitivity, `xoxd-` uniform-body, etc.) all focused on the *category-count* axis (`min_categories=2`); none audited the *unique-character-count* axis. The two axes fail differently: the category-count gap silently undetects uniform-class bodies regardless of length; the uniqueness-count gap silently undetects diverse bodies *only above a length breakpoint* that ties to the alphabet ceiling. Any future detector design must check BOTH axes when reasoning about the entropy fallback's coverage envelope. The 32-char floor in the post-fix uniqueness gate is the breakpoint at which the alphabet-ceiling-bounded ratio diverges from the original `len // 4` ratio (`alphabet_size ≈ 64`, so `len // 4 > 32` at `len > 128`). The mathematical relationship `unique_chars <= min(len, alphabet_size)` is the load-bearing invariant: any uniqueness gate must be capped at `alphabet_size` to remain satisfiable across the realistic length range.
**Prevention:** When designing entropy-based detection heuristics, explicitly model the alphabet-ceiling constraint. For any candidate that matches an alphabet-limited regex (`[A-Za-z0-9+/=_-]{N,}`, `[a-f0-9]{N,}`, `[0-9]{N,}`), the uniqueness requirement should be `min(max(6, len // 4), alphabet_size)`. Audit grep: `git grep -nE 'len\(set\(.*\)\)\s*<' src/` enumerates every uniqueness-gated heuristic. Add a parametrised regression test that exercises strings of various lengths (24, 100, 287, 500, 1000) with the alphabet-bounded unique-char count to verify the cap holds across the long-tail. The fix shape (`max(6, min(len(candidate) // 4, 32))`) preserves behaviour for short candidates (length 24-127, where `len // 4` dominates) and only loosens the requirement for longer candidates where the alphabet ceiling makes the original ratio mathematically impossible. False-positive surface is bounded by the contiguous-match requirement of `_HIGH_ENTROPY_RE` (natural-language text breaks at punctuation) and the `_is_binary` filter that already skips committed binary files; remaining false-positive class (long base64 data URIs inline in HTML/Markdown) is addressable via `.secret-scan-ignore` if it materialises.

## 2026-05-16 - Secret Scanner Drift Round 15: Slack Workflow Token Family Closure (`xoxe-` / `xoxe.xoxb-` / `xoxe.xoxp-` V2 Token Rotation Refresh + `xoxc-` Browser Session + `xoxd-` Cookie Session) — The Three Adjacent-Prefix Candidates Round 14's Closing Checklist Explicitly Named-But-Deferred; Pre-Fix `xoxd-<uniform-body>` Was Silently Undetected Entirely (Entropy Fallback's `min_categories=2` Rejects Single-Class Bodies, `_KNOWN_TOKENS` Has No Entry, Assignment Heuristic Requires Sensitive Variable Name) And `xoxe-`/`xoxc-` Bare Leaks Lost Slack-Specific Issuer Attribution To Generic "Hochentropischer Token-String" Findings

**Vulnerability:** Round 14 (PR #1520, AWS ABIA prefix) closed the AWS credential-prefix family at 4-of-4 and explicitly named the next-round adjacent-prefix candidates: **Slack workflow tokens (`xoxe-` refresh, `xoxc-` browser, `xoxd-` cookie/session — adjacent-PREFIX candidates if the Slack family expands beyond the four already covered).** The pre-Round-15 Slack family covered four prefixes (`xoxb-` Bot / `xoxp-` User / `xoxa-` OAuth / `xoxr-` Legacy Refresh) and three canonical Slack token prefixes remained outside `_KNOWN_TOKENS`: the modern V2 token-rotation refresh format (`xoxe-` direct + `xoxe.xoxb-`/`xoxe.xoxp-` chained), the browser session token (`xoxc-`), and the cookie session token (`xoxd-`).

A leaked Slack workflow token fails the canonical detection branches in `_scan_content`:

1. **`_PEM_RE`** — no PEM markers; no match.

2. **`_KNOWN_TOKENS`** — pre-fix no entry covers `xoxe-`/`xoxc-`/`xoxd-`; no match.

3. **`_AWS_ID_RE`** — wrong issuer family; no match.

4. **`_BEARER_RE`** — bare Slack tokens lack the `Bearer ` prefix; no match.

5. **`_SENSITIVE_ASSIGN_RE`** — only fires when the variable name carries a sensitive keyword. For bare token leaks in log lines, JSON fixtures without sensitive keys, comments, documentation snippets, or arbitrary text — NO match.

6. **`_HIGH_ENTROPY_RE`** — the body alphabet `[0-9a-zA-Z\-]` lies inside the entropy alphabet `[A-Za-z0-9+/=_-]`, so the body span matches generically as `Hochentropischer Token-String` — BUT this **loses the Slack-specific issuer attribution** (revocation flow at api.slack.com/apps/<app>/oauth for `xoxe-` rotation; https://slack.com/account/sessions for `xoxc-`/`xoxd-` browser-cookie auth — distinct from every other vendor's). For uniform-character-class bodies (all-lowercase / all-uppercase / all-digit, common for hash-derived or poorly-seeded RNG tokens), `_looks_like_secret` requires `min_categories=2` and returns `False` — the credential is silently undetected entirely (verified pre-fix for `xoxd-` with 32 uppercase chars). The chained `xoxe.xoxb-<body>` shape additionally trips up the strict bare `xoxb-` regex (which requires a `[0-9]{10,}-[0-9]{10,}-[a-zA-Z0-9]{24}` body that the rotation shape `1-<base64body>` does NOT match) — so no detection branch ever fires for the canonical Slack V2 token-rotation refresh token in the wild.

**Threat model:** A real-world Slack workflow token (V2 rotation refresh token from `oauth.v2.access` with `grant_type=refresh_token`, browser/cookie session token extracted from Slack web client DevTools by unofficial tools like slackdump / slack_cleaner) leaked alongside non-sensitive context (debug log, JSON fixture, hostile-PR fragment, hand-typed documentation snippet) slips past the secret scanner CI gate entirely or lands without issuer-specific attribution. Per-prefix blast radius:

  * **`xoxe-` / `xoxe.xoxb-` / `xoxe.xoxp-`** (Token Rotation Refresh): holder can mint fresh `xoxb-`/`xoxp-` access tokens with the rotation chain's identity and scopes until revocation. Slack's V2 rotation gives 12-hour access-token TTL but refresh tokens are long-lived (multi-month), so a leaked refresh token grants persistent access until manual revocation via api.slack.com/apps/<app>/oauth > Reinstall App. Multi-month blast radius.

  * **`xoxc-`** (Browser Session Token): full user-level session auth with **no scope restrictions** — equivalent to the user being logged in via the web client. An attacker holding `xoxc-` can browse every DM / private channel / sensitive file the user can see, post messages as the user, and exfiltrate the workspace's full message history via cookie-authenticated `/api/conversations.list` / `conversations.history` endpoints. Revocation flow at https://slack.com/account/sessions ("Sign out all other sessions") — distinct from the OAuth-app revocation flow.

  * **`xoxd-`** (Cookie Session Token): companion to `xoxc-`, the `d` cookie value from the browser session. Used in conjunction with `xoxc-` for direct browser-style API calls. A leak grants the same SESSION-LEVEL access as `xoxc-`; both tokens typically leak together via DevTools cookie extraction.

**Sites closed (3 new `_KNOWN_TOKENS` entries):**

  1. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `Slack Token Rotation Refresh Token gefunden` matching `(?<![A-Za-z0-9])xoxe(?:-|\.xox[bp]-)[0-9a-zA-Z\-]{20,}(?![A-Za-z0-9])`. Covers BOTH the direct shape (`xoxe-<body>`) AND the chained shape (`xoxe.xoxb-<body>` / `xoxe.xoxp-<body>`) per https://api.slack.com/authentication/rotation. The non-capturing alternation `(?:-|\.xox[bp]-)` between the prefix and body handles both shapes in a single regex while keeping the body strict (`[0-9a-zA-Z\-]` — no dots in the post-prefix span).
  2. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `Slack Browser Session Token gefunden` matching `(?<![A-Za-z0-9])xoxc-[0-9a-zA-Z\-]{20,}(?![A-Za-z0-9])`. Permissive 20+ char body mirrors the existing `xoxa-`/`xoxr-` shape (Slack session tokens use a similar structured-with-dashes format).
  3. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `Slack Cookie Session Token gefunden` matching `(?<![A-Za-z0-9])xoxd-[0-9a-zA-Z\-]{20,}(?![A-Za-z0-9])`. Same permissive body shape; mutually exclusive prefix.

**Placement invariant:** All three new entries are placed BEFORE the existing `xoxb-` / `xoxp-` / `xoxa-` / `xoxr-` entries in the `_KNOWN_TOKENS` list. This ordering matters because `_scan_content` iterates `_KNOWN_TOKENS` in list order and `is_covered` suppresses overlapping later matches — so the larger `xoxe.xoxb-<body>` / `xoxe.xoxp-<body>` chained span correctly anchors at the rotation attribution, suppressing any inner-span match the permissive `xoxa-`/`xoxr-` regexes might claim on the post-`xoxe.` portion. The strict bare `xoxb-`/`xoxp-` regexes don't match the rotation body shape anyway (`1-<base64body>` ≠ `[0-9]{10,}-[0-9]{10,}-[a-zA-Z0-9]{24,32}`), so the canonical bare tokens still receive their canonical attribution.

**Severity:** **HIGH-MEDIUM** — silent-undetection for uniform-body `xoxd-` / `xoxe-` (the high-severity branch where the credential never triggers a CI alert and remains exploitable until manual audit) + attribution-drift for mixed-body `xoxe-`/`xoxc-`/`xoxd-` (medium-severity branch where the entropy fallback caught the body span but lost the Slack-specific issuer attribution that determines which revocation playbook applies). The chained `xoxe.xoxb-`/`xoxe.xoxp-` shape is the worst-case real-world emission (Slack's canonical V2 rotation format), and the strict bare `xoxb-`/`xoxp-` regexes don't match it — pre-fix EVERY V2 rotation-refresh-token leak in the wild bypassed the Slack detector entirely.

**Fix:** Three new `_KNOWN_TOKENS` entries appended to `src/utils/secret_scanner.py`, positioned right before the existing `xoxb-` entry so `is_covered` anchors the chained-shape attribution correctly.

Companion `tests/test_sentinel_secret_scanner_drift_round15.py` (26 tests): per-prefix detection PoCs (direct `xoxe-`, chained `xoxe.xoxb-`, chained `xoxe.xoxp-`, `xoxc-`, `xoxd-`), per-prefix plaintext-context PoCs (log line / JSON fixture without sensitive variable name — silent-undetection branch demonstration), per-prefix negative cases (short bodies < 20 chars correctly NOT matched, mid-word prefixes correctly NOT matched via lookbehind anchor), regression guards for ALL four existing Slack prefixes (xoxb- / xoxp- / xoxa- / xoxr- retain canonical attribution), seven-prefix coexistence test (every Slack prefix produces a distinct finding with correct attribution), cross-vendor boundary regression (GitHub PAT / GitLab PAT / Stripe / Google API keys not mis-attributed to Slack patterns), inventory invariants (source-grep enforces `xoxe`/`xoxc-`/`xoxd-` presence in `_KNOWN_TOKENS` — future regressions that drop a pattern fail the invariant immediately), per-prefix masking-contract tests (raw value never appears unredacted in `Finding.match`), and the silent-undetection-via-uniform-body PoC that directly exercises the `_looks_like_secret` `min_categories=2` rejection path that the entropy fallback hits for all-uppercase bodies. Every test fails pre-fix and passes post-fix.

**Learning:** **The Slack token-prefix family is enumerated, NOT pattern-derived — every Slack token prefix MUST be explicitly listed in `_KNOWN_TOKENS` because the issuer-specific attribution is the determinant for which Slack revocation flow applies (api.slack.com/apps/* for OAuth-app tokens; slack.com/account/sessions for browser-cookie session tokens; a workspace's "Reinstall App" flow for rotation chains). Each prefix has a structurally distinct revocation playbook — losing the prefix-specific attribution to a generic high-entropy finding silently degrades the incident-response triage.** The seven Slack prefixes per Slack's published token taxonomy:

  * `xoxb-` (Bot User Token) — covered Round 1; strict shape; OAuth-app reinstall to revoke.
  * `xoxp-` (User Token) — covered Round 1; strict shape; OAuth-app reinstall to revoke.
  * `xoxa-` (OAuth App Configuration Token) — covered Round 1; permissive shape; api.slack.com/apps/ rotation.
  * `xoxr-` (Legacy Refresh Token) — covered Round 1; permissive shape; api.slack.com/apps/ rotation.
  * `xoxe-` / `xoxe.xoxb-` / `xoxe.xoxp-` (V2 Rotation Refresh) — Round 15 (THIS), api.slack.com/apps/<app>/oauth > Reinstall App.
  * `xoxc-` (Browser Session) — Round 15 (THIS), slack.com/account/sessions sign-out.
  * `xoxd-` (Cookie Session) — Round 15 (THIS), slack.com/account/sessions sign-out.

The closing-checklist invariant: every token whose vendor publishes a distinct revocation flow MUST receive its vendor-specific attribution; reliance on the entropy fallback's generic finding silently degrades the IR triage even when the entropy alphabet does happen to match the body. The chained-shape `xoxe.xoxb-`/`xoxe.xoxp-` case demonstrates the **ordering invariant**: when a new prefix's body region overlaps with an existing detector's body region, the new entry MUST be placed BEFORE the existing one in `_KNOWN_TOKENS` to ensure `is_covered`-based suppression yields the correct (more-specific) attribution. The Round-12 Shopify family closure already established this ordering invariant for `shpat_`/`shpss_`/`shppa_`/`shpca_` — Round 15 confirms the invariant generalises to the rotation-chain encoding shape.

**Next-round candidates (named-but-deferred):** The Slack issuer family is now closed at 7-of-7 documented prefixes. The following adjacent next-round candidates apply the same enumeration-not-derivation contract: **Basic Auth detector** (`(?i)Basic\s+[A-Za-z0-9+/=]{16,}` per RFC 7617) — named by the 2026-05-15 Bearer case-sensitivity round; closes the second auth-scheme literal in the RFC 7235 family. **GCP service account key** (typically a JSON file with an embedded `-----BEGIN PRIVATE KEY-----` block) — covered structurally by `_PEM_RE`; bucket-(b) deferred. **Azure SAS tokens** (`?sv=YYYY-MM-DD&...&sig=<base64>`) — adjacent context candidate, deferred pending query-string-aware extraction. **Cloudflare API token** (`<40 char alphanumeric>` — bucket-(b), no canonical prefix). **Azure storage account key** (`<88 char base64>` — bucket-(a) candidate pending shape-disambiguator analysis to avoid false-positive noise). **Heroku Platform API key** (UUID format — bucket-(b), shape would collide with operator-named UUIDs without context). **Datadog API key** (`<32 lowercase hex>` — bucket-(b), no canonical prefix; shape collides with operator-named hashes).

**Marker:** SENTINEL_SLACK_WORKFLOW_TOKEN_DRIFT.

## 2026-05-16 - Secret Scanner Drift Round 14: AWS STS Service Bearer Token (`ABIA<16>`) — The Fourth Credential Prefix In The AWS 4-Char Unique-Identifier Family That `_AWS_ID_RE` Explicitly Enumerated As Named-But-Uncovered (Round 13 Closing Checklist Listed Only `AKIA`/`ASIA`/`ACCA`); Pre-Fix Bare `ABIA<16>` Tokens Slipped Through Every Detection Branch (Entropy Fallback's 24-Char Minimum Rejects The 20-Char ABIA Shape; `_AWS_ID_RE` Enumerates Only 3 Of 4 Prefixes; Assignment Heuristic Loses AWS-Specific Attribution)

**Vulnerability:** The `_AWS_ID_RE = re.compile(r"(?<![A-Za-z0-9])(AKIA|ASIA|ACCA)[A-Z0-9]{16}(?![A-Za-z0-9])")` regex in `src/utils/secret_scanner.py` enumerates only **three of the four** AWS 4-character credential prefixes documented in the AWS IAM "Unique identifiers" reference. The missing fourth — `ABIA` (AWS STS service bearer token, issued by `sts:GetServiceBearerToken` for service-to-service authentication on behalf of an AWS user) — follows the identical 4+16=20 char format but is silently excluded from the enumeration. Per gitleaks / trufflehog / detect-secrets / aws-secret-detector default rules, ABIA is the fourth AWS credential prefix that grants direct API access (alongside AKIA/ASIA/ACCA); only the AWS-non-credential prefixes (AGPA group, AIDA IAM user, AIPA EC2 instance profile, ANPA SNS access point, ANVA account, APKA public key, AROA role, ARPA resource, ASCA certificate) are deliberately omitted as identifiers-not-credentials.

A leaked `ABIA<16>` token in committed source / log artefacts / CI debug snippets / hostile-PR fragments fails ALL six detection branches in `_scan_content`:

1. **`_PEM_RE`** — no PEM markers; no match.

2. **`_KNOWN_TOKENS`** — pre-fix no entry covered `ABIA`; no match.

3. **`_AWS_ID_RE`** — pre-fix the regex enumerates only `(AKIA|ASIA|ACCA)`; `ABIA` is silently excluded.

4. **`_BEARER_RE`** — requires the literal `Bearer ` keyword before the body; a bare ABIA token without that prefix does not match.

5. **`_SENSITIVE_ASSIGN_RE`** — only fires when the *variable name* carries a sensitive keyword (`key`/`secret`/`token`/etc.). For bare token leaks in log lines, JSON fixtures without sensitive keys, comments, documentation snippets, or arbitrary text — no match.

6. **`_HIGH_ENTROPY_RE`** — requires `{24,}` chars from `[A-Za-z0-9+/=_-]`. The ABIA token format is exactly **20 chars** (4-char prefix + 16-char body). The 20-char shape falls **below** the 24-char entropy threshold; no match.

Net result pre-fix: a bare `ABIAV2EXAMPLE12345AB` leaked into a log line, JSON fixture, error message, comment, or documentation snippet is **silently undetected** by every detection branch — the CI gate passes, the credential sits in the public repository indefinitely, and the issuing user's full AWS scope (the bearer token's authorized API access window) is exposed to every consumer. Even when the token IS embedded in a sensitive-keyword assignment (`AWS_BEARER_TOKEN = "ABIA…"`), the only finding emitted is the generic `Verdächtige Zuweisung eines potentiellen Secrets` — losing the AWS-specific issuer attribution that incident-response playbooks key off (revocation flow at IAM > STS service bearer token management — distinct from the IAM > Access keys flow used to revoke `AKIA` tokens).

Pre-fix PoC (verified):
```python
abia_token = 'ABIAV2EXAMPLE123ABCD'  # 20 chars, real AWS-credential format
content = f'2026-05-15T14:32:01Z DEBUG sts.client received bearer token {abia_token} for service auth'
_scan_content(content)  # → []  EMPTY — silently undetected
```

**Threat model:** A real-world AWS STS service bearer token (issued by `sts:GetServiceBearerToken` for any AWS service that supports bearer-token-based auth — AWS CodeCatalyst, IoT Device Defender, certain Bedrock APIs, federated workload-identity flows) leaked alongside non-sensitive context (CloudTrail debug log line, AWS SDK debug trace with `AWS_DEBUG=true`, hand-typed documentation example, hostile-PR fragment demonstrating service-to-service auth) slips past the secret scanner CI gate entirely. Bearer tokens carry whatever scope `sts:GetServiceBearerToken` was authorized to grant — typically the same scope as the issuing user's IAM policy, restricted to the target service. The high-severity case is the silent-undetection branch: a credential committed in any non-assignment context never triggers a CI alert, never gets rotated, and remains exploitable until the bearer token's TTL expires (typically 12 hours but can be extended via STS regional endpoints).

The 20-char shape exclusion from `_HIGH_ENTROPY_RE`'s 24-char minimum is the structural root cause — every AWS credential prefix family member is exactly 20 chars by AWS spec, so the explicit enumeration in `_AWS_ID_RE` is the *only* line of defence. Missing one prefix from that enumeration creates a complete detection-from-zero gap for that prefix family. Pre-fix every real-world ABIA emission shape (CloudTrail `ResponseElements.bearerToken`, AWS SDK retry/debug log line, JSON fixture with non-sensitive key, documentation example) bypassed the AWS detector. Post-fix every emission shape — across every context — yields the AWS-STS-Service-Bearer-Token-specific attribution.

**Sites closed (1 detector regex added to `_KNOWN_TOKENS`):**

  * `src/utils/secret_scanner.py:_KNOWN_TOKENS` — single `(re.compile(...), reason)` tuple appended to the table:
    ```python
    (
    ...
    ```
    Strict `[A-Z0-9]{16}` body alphabet (uppercase + digits, mirroring `_AWS_ID_RE`) anchors against false positives on lowercase or mixed-case strings happening to start with `ABIA` (English words / placeholder identifiers). The `(?<![A-Za-z0-9])` lookbehind prevents matching mid-word occurrences. Distinct attribution from the generic `AWS Access Key ID gefunden` reason so the report identifies WHICH AWS credential type leaked and the operator can rotate via the correct STS revocation flow rather than the IAM access-key rotation flow.

The KNOWN_TOKENS processing branch runs **before** `_AWS_ID_RE` in `_scan_content`, so `is_covered` correctly anchors the ABIA detection at the more-specific issuer attribution and the existing AKIA/ASIA/ACCA matches via `_AWS_ID_RE` are unaffected (regression guards in the companion test confirm).

**Severity:** **HIGH** — detection-from-zero failure for an AWS credential prefix that grants direct API access. The 20-char format places ABIA tokens **below** the entropy fallback's 24-char minimum (every other AWS prefix in the family is also 20 chars but they are explicitly enumerated, so the enumeration is the only line of defence). Every bare ABIA leak in a non-assignment context is silently undetected; assignment-context leaks lose the AWS-specific issuer attribution. CodeQL / Semgrep do not flag this drift because the AWS unique-identifier prefix taxonomy is encoded in AWS documentation, not in any static-analysis catalogue.

**Fix:** Single-tuple addition to `_KNOWN_TOKENS`. Zero false-positive risk: the strict `[A-Z0-9]{16}` body alphabet rejects lowercase / mixed-case / hyphen / underscore content (every AWS-credential body is uppercase + digits per spec); the `(?<![A-Za-z0-9])` lookbehind prevents mid-word matches; the explicit 16-char body length matches the AWS spec exactly (no quantifier slop).

Companion `tests/test_sentinel_secret_scanner_drift_round14.py` (10 tests): plaintext-context PoC (silent-undetection branch), JSON-fixture PoC (no sensitive variable name), assignment-context PoC (attribution-drift branch), three negative cases (short body, lowercase body, mid-word ABIA), three regression guards (AKIA/ASIA/ACCA still receive canonical `AWS Access Key ID gefunden` attribution — no collision), and an inventory invariant pin (`ABIA` and `AWS STS Service Bearer Token` strings present in `secret_scanner.py` source — fails on regression to pre-fix shape).

**Learning:** **The AWS 4-character credential-prefix family is enumerated, NOT pattern-derived — every credential prefix MUST be explicitly listed in `_AWS_ID_RE` (or `_KNOWN_TOKENS`) because the 20-char AWS format falls below `_HIGH_ENTROPY_RE`'s 24-char minimum, so the enumeration is the only line of defence.** The four AWS credential prefixes per AWS IAM "Unique identifiers" reference are: `AKIA` (long-term IAM access key) + `ASIA` (STS short-term access key) + `ACCA` (context-specific credentials) + `ABIA` (STS service bearer token). The non-credential AWS prefixes (`AGPA` group, `AIDA` IAM user, `AIPA` EC2 instance profile, `ANPA` SNS access point, `ANVA` account, `APKA` public key, `AROA` role, `ARPA` resource, `ASCA` certificate) are deliberately omitted — they are identifiers-not-credentials and would create false-positive noise in policy/IAM documentation files without IR value. The closing-checklist invariant for future Sentinel rounds is: **any new vendor whose token format is shorter than 24 characters MUST be explicitly enumerated (cannot rely on the entropy fallback as a safety net).** This applies equally to short-format tokens like AWS prefixes (20 chars), some Twilio Auth Tokens (32 chars but one-category hex — would fail `min_categories=2`), and any future vendor with a `<short prefix><uniform body>` shape.

**Next-round candidates (named-but-deferred):** The AWS credential-prefix family is now closed at 4-of-4. The following adjacent next-round candidates apply the same enumeration-not-derivation contract: **GCP service account key** (typically a JSON file with a 24+ char private key fingerprint inside, but no canonical short prefix — covered structurally by `_PEM_RE` for the embedded `-----BEGIN PRIVATE KEY-----` block; bucket-(b) deferred). **Azure SAS tokens** (`?sv=YYYY-MM-DD&...&sig=<base64>` — adjacent context candidate, deferred pending query-string-aware extraction; the `&sig=` parameter body matches `_HIGH_ENTROPY_RE` generically but loses the Azure-specific issuer attribution that anchors revocation at the Storage Account access-keys page). **Cloudflare API token** (`<40 char alphanumeric>` — bucket-(b), no canonical prefix). **Azure storage account key** (`<88 char base64>` — covered by `_HIGH_ENTROPY_RE` structurally but loses Azure attribution; bucket-(a) candidate pending shape-disambiguator analysis to avoid false-positive noise). **Slack workflow tokens** (`xoxe-` refresh token, `xoxc-` browser token, `xoxd-` cookie/session token — adjacent-PREFIX candidates if the Slack family expands beyond the four already covered).

**Marker:** SENTINEL_AWS_ABIA_PREFIX_DRIFT.

## 2026-05-15 - Bearer Token Detector Case-Sensitivity Drift (RFC 7235 §2.1 Auth-Scheme Case-Insensitive Contract): `_BEARER_RE` Matched Only Canonical-Case `Bearer` Literal — A Leaked `BEARER <body>` / `bearer <body>` / `BeArEr <body>` Authorization-Header Fragment With a Uniform-Character-Class Body (All-Lowercase / All-Uppercase / All-Digit) Was SILENTLY UNDETECTED Entirely (Both Bearer-Specific AND Entropy-Fallback Branches), And a Mixed-Case Body Lost the Bearer-Token-Specific Issuer Attribution to a Generic "Hochentropischer Token-String" Finding

**Vulnerability:** The `_BEARER_RE = re.compile(r"Bearer\s+([A-Za-z0-9\-_.]{16,})")` regex in `src/utils/secret_scanner.py` matched the `Bearer` literal **case-sensitively**, but RFC 7235 §2.1 mandates that the HTTP auth-scheme component is matched **case-insensitively** by every conforming receiver. A leak of `BEARER <body>` / `bearer <body>` / `BeArEr <body>` — a legitimate canonical HTTP Authorization-header shape on the wire (curl's `--header "Authorization: bearer …"` lowercase form, httpie's debug-log uppercase form, YAML/JSON fixtures canonicalised to lowercase or uppercase for consistency with surrounding keys, hand-typed examples in documentation / log snippets / hostile-PR fragments) — fails both Bearer-specific detection branches in `_scan_content`:

1. **`_BEARER_RE.finditer(content)`** — pre-fix matched only the canonical `Bearer` form. `BEARER xyz` / `bearer xyz` / `BeArEr xyz` yields ZERO matches; the Bearer-Token attribution (`Bearer-Token wirkt echt`) is lost.

2. **`_HIGH_ENTROPY_RE.finditer(content)`** — the entropy fallback matches the body span as a generic high-entropy hit BUT applies `_looks_like_secret(candidate)` with `is_assignment=False`, which requires `min_categories=2` (multiple character categories — lowercase + uppercase, or lowercase + digit). For **uniform-character-class bodies** (all-lowercase legacy / hash-derived tokens, all-uppercase UUID-style tokens converted via `.upper()`, all-digit numeric keys), `_looks_like_secret` returns `False` and the entropy fallback emits ZERO findings — the leaked Bearer credential is **silently undetected** entirely.

Pre-fix dataflow (PoC confirmed):

  - Hostile or accidentally-leaked source carries `{"authorization": "bearer abcdefghijklmnopqrstuvwxyz"}` (24 lowercase chars — a real RSA-256 KID hash, a poorly-seeded `random.choices(ascii_lowercase, k=24)` test fixture, or a hand-typed all-lowercase placeholder that happens to be 24+ chars).
  - `_BEARER_RE` (case-sensitive) does NOT match `bearer` → no Bearer-Token finding.
  - `_HIGH_ENTROPY_RE` matches the 24-lowercase span, but `_looks_like_secret("abcdefghijklmnopqrstuvwxyz")` returns `False` (categories = 1: only lowercase) → no entropy-fallback finding.
  - `scan_repository(...) → []` — the CI gate passes; the committed credential sits in the public repository indefinitely.

For mixed-case bodies (`bearer AbCdEfGhIjKlMnOpQrStUvWx0123`) the entropy fallback DID match (3 categories), but the finding's reason is the generic `Hochentropischer Token-String` — the Bearer-Token-specific attribution that incident-response triage keys off (which auth flow leaked, which revocation playbook applies — Bearer is the canonical OAuth 2.0 / OIDC / OpenAPI bearer scheme, distinct from cookie-session / API-key-header / JWT-cookie auth flows) is silently dropped.

**Threat model:** A real-world Bearer token (OAuth 2.0 access token, OIDC ID token, JWT issued by an internal IdP, GitHub PAT used in `Authorization: token …` aliased to `bearer`, every modern API authentication flow on the wire) leaked alongside a non-canonical-case Bearer literal in committed source / log artefacts / CI debug snippets / hostile-PR fragments slips past the secret scanner CI gate entirely (silent-undetection branch) or lands without issuer-specific attribution (drift branch). Bearer tokens carry whatever scope the issuing IdP / OAuth client granted — typically `read` / `write` / `admin` capabilities against the issuing service's REST API, plus any `aud`-scoped downstream services for delegated tokens. The high-severity case is the silent-undetection branch: a credential committed with `bearer <uniform-body>` shape (realistic for tokens derived from `hex(hmac(...))`, `base64(uuid)`, or any single-character-class generator) never triggers a CI alert, never gets rotated, and remains exploitable until manual code audit.

Pre-fix every real-world Bearer emission shape with non-canonical case (curl `-v` debug log shape `< Authorization: BEARER <tok>`, JSON fixture `{"Authorization": "bearer <tok>"}`, YAML config `authorization: 'bearer <tok>'`, Python-dict log line `{'Authorization': 'BEARER <tok>'}`) bypassed the Bearer-Token detector. Post-fix every emission shape — across every case variant — yields the Bearer-Token-specific attribution.

**Sites closed (1 detector regex + 1 line of code):**

  * `src/utils/secret_scanner.py:_BEARER_RE` — single-line change: add `(?i)` inline flag so the `Bearer` literal is matched case-insensitively per RFC 7235 §2.1. The body capture group `[A-Za-z0-9\-_.]{16,}` already accepts both cases of letters; the inline-flag change only relaxes the matching of the auth-scheme literal, preserving the structural disambiguation contract (`\s+` + 16+ contiguous chars from the body alphabet) against natural-language false positives.

**Severity:** **MEDIUM-HIGH** — detection-from-zero failure for a class of realistic Bearer-token leaks. The high-severity branch is the silent-undetection case (uniform-body tokens slip past both detectors); the attribution-drift branch is medium-severity (body still caught generically, but the Bearer-Token reason that determines which revocation playbook applies is lost). Mitigated only by the requirement that the leaked token live alongside a non-canonical-case Bearer literal in the same content blob; in practice this covers every leak through automation-generated fixtures, lowercase-normalising loggers, and hostile-PR-introduced fragments that mimic existing canonical-case detector evasion. CodeQL / Semgrep do not flag this drift because the auth-scheme case-sensitivity invariant is encoded in RFC 7235 §2.1, not in any static-analysis catalogue.

**Fix:** Single-line code change:
```python
_BEARER_RE = re.compile(r"(?i)Bearer\s+([A-Za-z0-9\-_.]{16,})")
```
Zero false-positive risk: the existing structural disambiguator (`\s+` between the auth-scheme literal and a 16+ char body from `[A-Za-z0-9\-_.]`) prevents natural-language matches. A sentence like `"The bearer of bad news is rarely welcome."` does not trigger because `of` is short — no 16+ contiguous chars from `[A-Za-z0-9\-_.]` follow the `bearer` literal. The downstream `_looks_like_secret(candidate, is_assignment=True)` heuristic (entropy + unique-char check) provides the second-layer filter for any token-shaped string that does happen to follow a `bearer`-prefixed natural-language sentence.

Companion `tests/test_sentinel_secret_scanner_bearer_case_drift.py` (36 tests): per-case PoCs for every variation (`bearer` / `BEARER` / `BeArEr` / `bEaReR` / `Bearer` regression guard), per-body-class PoCs (mixed-body for attribution-drift demonstration, lowercase/uppercase/digit-body for silent-undetection demonstration), end-to-end Authorization-header emission inventory (curl `>` debug log, curl `<` debug log, JSON fixture, Python dict, YAML config — every real-world emitter that may canonicalise case differently), false-positive negative cases (natural-language sentences containing `bearer`/`BEARER`/`Bearer` without a token-shaped body), canonical-case regression guard, masking-contract per-case test (pinning that `_mask_secret` redacts the body regardless of auth-scheme case), cross-vendor boundary regression (JWT in a lowercase `bearer` header still yields the JWT-specific reason because `_KNOWN_TOKENS` matching runs BEFORE `_BEARER_RE` in `_scan_content`), and a compiled-regex invariant test that pins `_BEARER_RE.flags & re.IGNORECASE` programmatically — a future regression that reverts to the case-sensitive shape fails the invariant immediately.

**Learning:** **The case-insensitivity invariant of HTTP auth-scheme literals (RFC 7235 §2.1) is a CROSS-CUTTING property that applies to every authentication-scheme detector in the secret scanner, NOT just `_BEARER_RE`.** The `_KNOWN_TOKENS` entries closed by Rounds 1-13 are all issuer-prefixed (the prefix is the canonical sentinel published by the vendor — `glpat-` / `ghp_` / `shpat_` / `xkeysib-` etc.) and the vendor's own documentation fixes the case at the prefix level (GitHub PATs are always lowercase prefix + mixed-case body — that's GitHub's published format). But the *non-issuer-prefixed* detectors (`_BEARER_RE` here; future `_BASIC_AUTH_RE` for RFC 7617 Basic; `_DIGEST_AUTH_RE` for RFC 7616 Digest; `_NEGOTIATE_RE` for RFC 4559 SPNEGO; `_TOKEN_SCHEME_RE` for GitHub's older `Authorization: token <pat>` shape) match the HTTP auth-scheme literal directly — and RFC 7235 §2.1 mandates case-insensitive matching for every auth-scheme. **The closing-checklist invariant for future Sentinel rounds: every detector regex whose anchor is an HTTP-RFC-defined auth-scheme literal MUST use the `(?i)` inline flag (or `re.IGNORECASE` flag arg) on the auth-scheme component. The 16+ char body-alphabet structural disambiguator (`\s+([A-Za-z0-9\-_.]{16,})`) is sufficient to prevent natural-language false positives — the case-insensitivity widening does NOT increase the false-positive rate per-match.**

The closing-checklist also clarifies the relationship between `_BEARER_RE` and the `_KNOWN_TOKENS` issuer-specific patterns: `_KNOWN_TOKENS` matching runs FIRST in `_scan_content`, so a JWT (`eyJ...`) embedded in a Bearer header yields the JWT-specific reason (`JSON Web Token (JWT) gefunden`) via `_KNOWN_TOKENS`, and the Bearer-Token reason fires only for *opaque* Bearer tokens that don't match any specific issuer prefix (true Round 1 base case). The case-insensitivity widening preserves this ordering — the JWT regression test in this PR confirms that lowercase `bearer eyJ...` still yields the JWT-specific reason, not a Bearer-Token finding.

**Next-round candidates (named-but-deferred):** **Basic Auth detector** (`(?i)Basic\s+[A-Za-z0-9+/=]{16,}`) — RFC 7617 HTTP Basic Authentication credential leak (`Authorization: Basic <base64(user:password)>`). The base64-encoded body falls inside `_HIGH_ENTROPY_RE`'s alphabet so the body matches generically, but the Basic-Auth-specific attribution (decode the base64 to recover the user:password pair, distinct revocation flow — rotate the user's password and any service-account credential) is lost. Adjacent-detector candidate with the same `(?i)` case-insensitivity contract as this round. **Digest Auth detector** (`(?i)Digest\s+username="..."`) — RFC 7616 multi-field challenge-response auth; the credential is split across the `nonce` / `response` / `cnonce` fields, so a single-regex anchor doesn't fully capture the secret — context-aware extraction needed, deferred pending body-structure-aware detector framework. **GitHub `Token` scheme detector** (`(?i)token\s+[A-Za-z0-9_\-]{36,}`) — GitHub's older API alias for Bearer (`Authorization: token ghp_...`); the embedded GitHub PAT (`ghp_`-prefixed) already matches `_KNOWN_TOKENS` via the `ghp_` regex, so the `token` scheme detector is attribution-only for opaque non-GitHub tokens that happen to use the alias.

**Marker:** SENTINEL_BEARER_CASE_DRIFT.

## 2026-05-15 - Secret Scanner Drift Round 13: Adjacent-Prefix Candidates Named-But-Deferred By Round 12 (WooCommerce REST API `ck_<32+ alphanumeric>` Consumer Key + `cs_<32+ alphanumeric>` Consumer Secret Pair + Mailchimp API Key `<32 hex>-us<region>` With Datacenter-Suffix Disambiguator) — Closes The E-Commerce-Platform SMB Tier (Shopify Round 12 + WooCommerce Round 13 = 6 Of 6 Documented Prefixes) AND The Marketing-Email-Platform Tier (Mailgun Round 12 + Mailchimp Round 13 = Two Highest-Volume Vendors)

**Vulnerability:** Round 12 (PR closing Mailgun + Square + Shopify family) closed the e-commerce-platform sub-landscape via the four Shopify prefixes (`shpat_` / `shpss_` / `shppa_` / `shpca_`) and explicitly named six next-round targets. Four of those (Magento 2 admin integration token, BigCommerce API token, SES SMTP credentials, Postmark API key) are bucket-(b) (no canonical prefix — covered structurally by the entropy fallback, no anchor for issuer-specific attribution, deferred indefinitely). The remaining two are adjacent-PREFIX candidates that THIS round closes: **WooCommerce REST API credentials** (`ck_<32+ alphanumeric>` consumer key + `cs_<32+ alphanumeric>` consumer secret pair) and **Mailchimp API keys** (`<32 hex>-us<region>` with the `-us<digit>` datacenter suffix as the structural disambiguator).

Pre-fix dataflow gap surfaced two orthogonal failure modes the existing scanner table did NOT cover:

1. **WooCommerce keys with uniform-character-class bodies were SILENTLY UNDETECTED.** The entropy fallback's `_looks_like_secret` heuristic requires multiple character categories (`int categories < min_categories: return False`). A real WooCommerce key generated with a poor RNG seed, or a test-fixture key like `ck_aaaaaa...` / `cs_BBBBB...` (all-lowercase / all-uppercase / all-digit body), fails the entropy check entirely. The PoC `tests/test_sentinel_secret_scanner_drift_round13.py:test_secret_scanner_detects_woocommerce_consumer_key` proves this pre-fix: a `ck_<32 'a' chars>` body produces ZERO findings — the scanner walks past the committed credential and the CI gate passes. Mixed-case bodies (real `wp_generate_password(32, false)` output) were caught only by the generic "Verdächtige Zuweisung eines potentiellen Secrets" assignment heuristic, which depends on the *variable name* carrying a sensitive keyword (`KEY` / `SECRET`). A WooCommerce credential committed under a non-sensitive-keyword variable name (`WC_CK = "..."` instead of `WC_CONSUMER_KEY = "..."`) bypasses BOTH detection branches and remains entirely undetected.

2. **Mailchimp API keys with the `<32 hex>-us<region>` shape match the entropy fallback as one generic finding, losing the issuer-specific attribution.** The entropy fallback's `[A-Za-z0-9+/=_-]{24,}` regex matches the entire string (every char in the alphabet — the dash IS inside the entropy alphabet, the digits IS inside, the `us` literal IS inside), so the full `<body>-us<N>` span gets flagged as one "Hochentropischer Token-String" finding. The Mailchimp-specific attribution that incident-response keys off (revocation flow at mailchimp.com/account/api/, distinct from Mailgun's app.mailgun.com control plane — both are marketing-email-platform vendors but their revocation flows live on entirely different domains) is silently lost.

**Threat model:** A WooCommerce / Mailchimp credential committed to a public repository (intentional misconfig, accidental copy-paste from a config sandbox, hostile PR slipping the credential past code review, leaked from a developer's local `.env` synced to a branch) carries the full blast radius of the issuing account:

  * **WooCommerce credential pair leak** (`ck_:cs_` together is the canonical Basic Auth shape — the standard documentation recommends storing both in the same config block, so a leak of either typically implies a leak of both): full WooCommerce REST API scope as assigned to the issuing API user. Default `read_write` role grants read+write on every product, order, customer, coupon, refund, draft order, shipping zone, tax rate, and webhook configuration; the `read` and `write` tier roles are narrower but still grant data exfiltration / mutation of the corresponding subset. The blast radius is structurally identical to a Shopify Admin API token (`shpat_`, Round 12) for the e-commerce dashboard — write access to every order means the attacker can issue arbitrary refunds, modify shipping zones to mis-route in-flight orders, edit product prices to enable arbitrage, and exfiltrate the full customer-PII database via the `/wp-json/wc/v3/customers` endpoint.

  * **Mailchimp API key leak**: full Mailchimp scope. Read/write every audience / list / contact / campaign / automation / signup form / template / report. Export the full subscriber CSV (PII exfiltration — name + email + opt-in timestamp + custom fields). Send mass mailings AS the issuing account — phishing amplification leveraging the account's authenticated sender domain and existing SPF/DKIM/DMARC reputation, hard to detect at the recipient gateway because the From: address is legitimately signed. Modify webhook URLs to redirect subscription/unsubscription events to attacker-controlled endpoints (lets the attacker silently harvest every new opt-in). Create new transactional API keys via Mandrill (Mailchimp's transactional sub-product) for persistence — even after the operator rotates the primary key, the Mandrill key continues to grant Mandrill-tier send authority. Blast radius is structurally identical to Mailgun (`key-<32 hex>`, Round 12) — both grant mass-mail-from-authenticated-domain scope — but their control planes are entirely distinct.

**Sites closed (3 new `_KNOWN_TOKENS` entries):**

  1. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `WooCommerce Consumer Key gefunden` matching `(?<![A-Za-z0-9])ck_[a-zA-Z0-9]{32,}(?![A-Za-z0-9])`. Covers modern `wp_generate_password(32, false)` (32 alphanumeric body), intermediate 40-char-body, AND legacy `wc_rand_hash()` (64-char lowercase-hex body) in one regex via the `{32,}` lower bound + greedy + lookahead anchor.
  2. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `WooCommerce Consumer Secret gefunden` matching `(?<![A-Za-z0-9])cs_[a-zA-Z0-9]{32,}(?![A-Za-z0-9])`. Sibling regex with identical body shape; distinct reason so the report calls out *which* half of the credential pair leaked.
  3. `src/utils/secret_scanner.py:_KNOWN_TOKENS` — `Mailchimp API Key gefunden` matching `(?<![A-Za-z0-9])[a-f0-9]{32}-us[0-9]{1,3}(?![A-Za-z0-9])`. The `-us<1-3 digit>` suffix is unique to Mailchimp's datacenter routing convention; no other major API vendor uses this suffix pattern.

**Severity:** **MEDIUM** — issuer-attribution drift; the underlying tokens were already partially detected (mixed-case WC bodies via "Verdächtige Zuweisung"; Mailchimp via entropy fallback) but lost the per-issuer reason that determines which admin UI the operator must visit to revoke the credential. The high-severity case is the WooCommerce uniform-character-class body branch where the credential was SILENTLY UNDETECTED entirely — that branch hardens the scanner against a class of low-entropy-but-real keys (poorly-seeded RNG, test fixtures committed as real-looking credentials, hand-typed placeholder keys that happen to be 32+ chars uniform).

**Fix:** Three new `_KNOWN_TOKENS` entries appended to `src/utils/secret_scanner.py`:
  * `re.compile(r"(?<![A-Za-z0-9])ck_[a-zA-Z0-9]{32,}(?![A-Za-z0-9])"), "WooCommerce Consumer Key gefunden"` — `{32,}` lower bound covers every documented WC body length (32/40/64) in one regex; `[a-zA-Z0-9]` body alphabet matches `wp_generate_password(N, false)` exactly.
  * `re.compile(r"(?<![A-Za-z0-9])cs_[a-zA-Z0-9]{32,}(?![A-Za-z0-9])"), "WooCommerce Consumer Secret gefunden"` — sibling regex.
  * `re.compile(r"(?<![A-Za-z0-9])[a-f0-9]{32}-us[0-9]{1,3}(?![A-Za-z0-9])"), "Mailchimp API Key gefunden"` — strict 32-char lowercase-hex body + `-us<1-3 digit>` suffix matches Mailchimp's canonical documented format.

Companion `tests/test_sentinel_secret_scanner_drift_round13.py` (16 tests): per-issuer detection PoCs (WooCommerce ck_, WooCommerce cs_, Mailchimp), per-issuer env-config detection (`KEY=VALUE` shapes), legacy 64-hex WooCommerce variant coverage, lookbehind-anchor negative case (`track_<body>` correctly NOT matched), per-issuer negative cases (short placeholder strings, non-hex bodies, wrong suffix), credential-pair detection (both halves of the WC ck_/cs_ pair flagged in the same scan), three-digit datacenter coverage for Mailchimp, cross-vendor boundary regression (no collisions with Stripe / Shopify / Mailgun patterns), and a masking-contract test that pins the per-secret `_mask_secret` redaction. Every test fails pre-fix and passes post-fix.

**Learning:** **WooCommerce credentials surfaced a previously-undocumented detection-gap shape: uniform-character-class bodies bypass the entropy fallback's `_looks_like_secret` heuristic entirely.** Every prior Sentinel round assumed the entropy fallback would catch any token the `_KNOWN_TOKENS` table missed; the WC uniform-body shape proves that assumption wrong — a poorly-seeded RNG, a test fixture committed as a real-looking key, or a hand-typed placeholder that happens to be 32+ chars of one character class slips past both detection branches. **The closing-checklist invariant for future Sentinel rounds is therefore: every adjacent-PREFIX candidate gets a `_KNOWN_TOKENS` entry, NOT just to add issuer-specific attribution but ALSO to anchor detection against the entropy-bypass class.** The Mailchimp closure follows the same pattern as Round 11/12 (per-issuer attribution accelerates IR triage when revocation flows diverge — mailchimp.com vs. app.mailgun.com for the marketing-email-platform tier; admin.shopify.com/* vs. wp-admin > WooCommerce > Settings > Advanced > REST API for the e-commerce SMB tier). The e-commerce SMB tier is NOW FULLY ENUMERATED (Shopify 4 prefixes + WooCommerce 2 prefixes = 6 of 6 documented prefixes); the enterprise tier (Magento) remains bucket-(b) and is deferred indefinitely.

**Next-round candidates (named-but-deferred):** Magento 2 admin integration token (`Bearer <40 hex>` — no canonical prefix, bucket-(b)); BigCommerce API token (`<32-64 base64url>` — no canonical prefix, bucket-(b)); SES SMTP credentials (no canonical prefix, bucket-(b)); Postmark API key (`<UUID-shape>` — no canonical prefix, bucket-(b)); Algolia Admin API key (`<32 hex>` with embedded `application_id` parameter — adjacent context candidate, deferred pending application_id-aware extraction); Pinecone API key (`<UUID-shape>` — bucket-(b)). The marketing-email-platform tier (Mailgun Round 12 + Mailchimp Round 13) is now closed for the two highest-volume vendors; Postmark closure would extend the tier to the transactional-mail-vendor sub-landscape but requires either a canonical prefix (which Postmark doesn't have — it uses UUID-shape) or context-aware extraction (server-token reference shape) that the current scanner architecture doesn't support.

**Marker:** SENTINEL_SECRET_SCANNER_DRIFT_ROUND_13.

## 2026-05-15 - GitHub API URL Validator HTTPS-Only Drift: `_is_trusted_github_api` Accepted `http://api.github.com` (and HTTP-on-Allowlisted-GHE-Host), Leaking the `FEED_GITHUB_TOKEN` Bearer Credential Over Plaintext HTTP — Closes the Last Validator In the Family That Drifted From the Canonical HTTPS-Only Shape Established for `_validated_vor_base_url` / `_validated_oebb_url` / `_validated_wl_base` / `validate_public_feed_url`

**Vulnerability:** The 2026-05-10 *HTTPS-only Provider URL Drift* round pinned every credential-bearing / content-publishing provider validator to the `parsed.scheme.lower() == "https"` contract (the canonical shape established by `validate_public_feed_url` in `src/utils/http.py`). The auto-issue submission endpoint validator (`src/feed/reporting.py:_is_trusted_github_api`, gating every `submit()` call that POSTs a `Authorization: Bearer <FEED_GITHUB_TOKEN>` header to GitHub's issues API) was left at the looser `parsed.scheme.lower() not in ("http", "https")` shape, which accepts both `http` and `https`. An attacker who controls `FEED_GITHUB_API_URL` (or its legacy `GITHUB_API_URL` alias — intentional CI misconfig, leaked secrets-store entry, attacker-crafted `.env`, hostile-PR-introduced env block in the Actions workflow) sets the value to `http://api.github.com` (or `http://<allowlisted-ghe-host>/api/v3` when an operator has opted-in to a GHE host via `FEED_GITHUB_ENTERPRISE_HOSTS`). Pre-fix dataflow:

1. `_GithubIssueConfig.from_env` reads the env var verbatim, strips the trailing slash, and stores `http://api.github.com`.
2. `_is_trusted_github_api("http://api.github.com")` returns `True` (the scheme check rejects only non-`http`/non-`https` schemes; the hostname + empty-path check matches).
3. `submit()` constructs `http://api.github.com/repos/owner/name/issues`, attaches the `Authorization: Bearer <FEED_GITHUB_TOKEN>` header, and POSTs over plaintext HTTP via `request_safe`.
4. Any on-path attacker — compromised network hop, BGP hijack, hostile public WiFi gateway, malicious transparent proxy, MITM on the client's egress — captures the bearer token verbatim from the cleartext `Authorization` header on the FIRST request, before any cross-scheme redirect would let `_safe_rebuild_auth` strip the header on the redirected request.

**Threat model:** The bearer token is sourced from `read_secret("FEED_GITHUB_TOKEN")` / `read_secret("GITHUB_TOKEN")` and grants whatever scope the issuing PAT / fine-grained token / App installation token / GitHub-Actions-injected `GITHUB_TOKEN` carries — typically `issues:write` + `contents:read` in default configurations, but CI environments routinely grant broader permissions (`contents:write`, `pull-requests:write`, `actions:read`). The leaked token's blast radius is therefore the full set of `permissions:` granted by the operator's workflow YAML. The GitHub REST API has been HTTPS-only since 2014 — `api.github.com` either rejects HTTP connections or 301-redirects to HTTPS — but the bearer token is already on the wire BEFORE either response, captured by any on-path adversary the moment the TCP-level POST body containing the headers leaves the cron runner.

**Sites closed (1 validator + 2 callsite branches):**

  1. `src/feed/reporting.py:_is_trusted_github_api` — the public-`api.github.com` branch AND the GHE-allowlist branch both inherit the HTTPS-only pin (one scheme check at the top of the function gates both branches, so a single `parsed.scheme.lower() != "https"` early-return closes the credential-leak vector across every operator deployment shape).

**Severity:** **HIGH** — direct bearer-token leak over plaintext HTTP under an attacker-controllable env-var override. Mitigated only by the requirement that the attacker control the env var; in CI environments where the env var is set from a leaked secrets-store entry or a typo-squat allowlist, the leak is automatic on every feed-build cron tick that hits an error path.

**Fix:** Tighten `_is_trusted_github_api`'s scheme check from `not in ("http", "https")` to `!= "https"`. Single-line code change at the validator entry-point; the rest of the function (hostname + path matching, GHE allowlist consultation) is unchanged. Pinning HTTPS-only here is safe with zero false-positive risk:
  - `api.github.com` has never accepted plaintext HTTP in production (HTTPS-only since 2014).
  - GitHub Enterprise Server canonical installations terminate TLS at the GHE-managed load balancer — production GHE deployments do not expose `/api/v3` over HTTP.

Companion `tests/test_sentinel_github_api_url_https_drift.py` (14 tests):
  * **Validator-level PoCs (8 tests):** every `http://`/`HTTP://` variant of `api.github.com` and the GHE allowlist branch is rejected; the HTTPS canonical forms continue to be trusted (regression coverage for the production happy path).
  * **End-to-end PoCs (2 tests):** `submit()` with `FEED_GITHUB_API_URL=http://api.github.com` AND `FEED_GITHUB_API_URL=http://<allowlisted-ghe-host>/api/v3` are both observed to abort without any outbound HTTP call (`responses.calls` is empty) and emit the canonical `kein bekannter GitHub-Endpunkt` warning.
  * **Inventory invariant (1 test):** audit walker enumerates every URL validator across the project that gates a credential-bearing or content-publishing request — `_validated_vor_base_url`, `_validated_oebb_url`, `_validated_wl_base`, `validate_public_feed_url`, `_is_trusted_github_api` — and asserts the canonical `http://` form of each is rejected. A future sixth validator that accepts `http://` fails the test until the canonical shape is restored.

In lockstep with the validator fix, the existing `tests/test_reporting_github_host_allowlist.py:test_is_trusted_github_api_rejects_untrusted_endpoints` parametrize block had a misleading comment ("Plain HTTP (not allowed for credential transport)") attached to an `ftp://api.github.com` URL — the comment described the intent but the test never actually exercised the HTTP scheme. Both `http://api.github.com` and `http://api.github.com/` are now added to the explicit reject list to pin the post-fix HTTPS-only contract against future drift.

**Learning:** **HTTPS-only validator shape is a CROSS-CUTTING invariant** that applies to every URL validator gating a credential-bearing or content-publishing request — not just the provider-side URL validators that Round 4 (2026-05-10) closed. The 2026-05-10 round enumerated three siblings (`_validated_vor_base_url` / `_validated_oebb_url` / `_validated_wl_base`) and the `validate_public_feed_url` publishing-side validator, but the GitHub auto-issue submitter's `_is_trusted_github_api` was missed — the validator was already host-pinned (the harder problem) and the scheme check was already there (the easier problem), so the drift looked surface-level identical to the canonical shape at code-review time. The miss was structural: the scheme check used `not in (...)` (excluding obviously-wrong schemes like `ftp://` and `data:`) rather than `==` (positively asserting the secure scheme). **The closing-checklist invariant for future URL validators: every validator whose downstream caller attaches a credential (bearer token, basic auth, signed query parameter) OR whose output is interpolated into a published artefact MUST use the `parsed.scheme.lower() == "https"` positive-assertion shape — not the looser `not in ("http", "https")` exclusion shape that accidentally allows plaintext HTTP.** The inventory-walker test in this PR pins the invariant programmatically — a future sixth credential-bearing endpoint added to the project that uses the looser shape fails the test on the first pytest run.

**Marker:** SENTINEL_GITHUB_API_HTTPS_DRIFT.

## 2026-05-15 - Secret Scanner Drift Round 12: Adjacent-Prefix Candidates Named-But-Deferred By Round 11 (Mailgun `key-<32 hex>` + Square `EAAA<base64>` Access Tokens) + E-Commerce-Platform Sub-Landscape Opened (Shopify Family — `shpat_` Admin API / `shpss_` Shared Secret / `shppa_` Private App / `shpca_` Custom App)

**Vulnerability:** Round 11 (PR closing GitLab developer-tooling tier + CircleCI) closed the CI/CD execution-tier sub-landscape and explicitly named five next-round targets. Three of those (AWS Secret Access Keys, Cloudflare API Tokens, Heroku API keys) are bucket-(b) (no canonical prefix — covered structurally by the entropy fallback, no anchor for issuer-specific attribution, deferred indefinitely). The remaining two are adjacent-PREFIX candidates that THIS round closes: **Mailgun private API keys** (`key-<32 lowercase hex>`) and **Square Access Tokens** (`EAAA<60+ base64url body>`). Both prefixes are unambiguous in their issuer context and previously fell through to the generic high-entropy fallback ``[A-Za-z0-9+/=_-]{24,}`` as one ``Hochentropischer Token-String`` finding — the prefix-specific attribution that incident-response triage keys off (rotation flow, revocation URL, blast-radius estimate) was silently lost.

In lockstep with the Round-11 named candidates, this round opens the **e-commerce-platform sub-landscape** via the **Shopify access-token family** (4 prefixes — Admin API `shpat_`, shared secret `shpss_`, private app `shppa_`, custom app `shpca_`). Each prefix has a 32-char lowercase-hex body and grants a different scope on the issuing store:

  1. **`shpat_<32 hex>` Admin API access token** — full custom-app installed scope on the store: read/write every product, customer, order, inventory item, fulfilment, refund, draft order, gift card, abandoned checkout, customer-segment, and webhook configuration; modify shop settings; initiate refunds and create new staff accounts. Blast radius is the highest in the e-commerce-platform tier — structurally identical to `glpat-` (Round 1) for the e-commerce dashboard. Revocation: admin.shopify.com/apps/<app>/edit.
  2. **`shpss_<32 hex>` shared secret** — HMAC-SHA256 webhook signature verification credential. A leak lets a network adversary forge webhook payloads (order fulfilment, refund processing, cart-abandonment automation, customer-data sync, gift-card issuance) that the app's signature verification accepts. Structurally identical to Stripe's `whsec_` (Round 1) but for the e-commerce dashboard. Rotation: app's webhook-settings page.
  3. **`shppa_<32 hex>` private-app access token** — legacy shape (deprecated 2022-01-01 for new stores; still issued for existing installations). Same blast radius as `shpat_`. Revocation: admin.shopify.com/admin/apps/private (distinct flow from custom-app revocation).
  4. **`shpca_<32 hex>` custom-app access token** — modern replacement (post-2022) for the deprecated `shppa_` private-app flow. Same blast radius as `shpat_`. Revocation: admin.shopify.com/admin/settings/apps/development (distinct flow from private-app revocation).

Pre-fix every Mailgun / Square / Shopify token matched the generic entropy fallback as a single `Hochentropischer Token-String` finding (the dash in `key-`, underscore in `shp*_`, and entire base64url body in `EAAA<body>` all lie inside or adjacent to the entropy alphabet); post-fix each token receives its issuer-specific German attribution string (`Mailgun Private API Key gefunden` / `Square Access Token gefunden` / `Shopify Admin API Access Token gefunden` / `Shopify Shared Secret gefunden` / `Shopify Private App Access Token gefunden` / `Shopify Custom App Access Token gefunden`) that incident-response playbooks key off. The Shopify per-prefix attribution is the most operationally valuable enhancement: the four prefixes have FOUR DISTINCT revocation flows on admin.shopify.com, and precise attribution lets the operator land on the correct admin page instantly during an active incident.

**Fix:** Six new `_KNOWN_TOKENS` entries appended to `src/utils/secret_scanner.py`:
  * `re.compile(r"(?<![A-Za-z0-9])key-[a-f0-9]{32}(?![A-Za-z0-9])"), "Mailgun Private API Key gefunden"` — strict 32-char lowercase-hex body matches Mailgun's canonical documented format; `[a-f0-9]` (not `[A-Za-z0-9]`) avoids overlap with arbitrary operator-supplied `key-...` placeholder values that don't accidentally land in the hex alphabet AND clock in at exactly 32 chars.
  * `re.compile(r"(?<![A-Za-z0-9])EAAA[A-Za-z0-9_\-]{60,}(?![A-Za-z0-9])"), "Square Access Token gefunden"` — 60-char body lower bound rejects short `EAAA`-prefixed fragments (operator placeholders, accidentally-truncated tokens) while accepting every legitimate Square access token (real tokens are 64+ chars).
  * Four Shopify entries with `shp(at|ss|pa|ca)_[a-fA-F0-9]{32}` regexes — strict 32-char hex body matches Shopify's canonical documented format; the four prefixes are mutually exclusive at the fifth-character level.

Companion `tests/test_sentinel_secret_scanner_drift_round12.py` (16 tests): per-issuer detection PoCs (Mailgun + Square + 4× Shopify), per-issuer env-config detection (`KEY=VALUE` shapes), per-issuer negative cases (short placeholder strings, non-hex bodies), Shopify-family mutual-exclusion test, cross-vendor boundary regression (no collisions with JWT / Stripe / GitLab patterns), and a masking-contract test that pins the per-secret `_mask_secret` redaction. Every test fails pre-fix (the entropy fallback's body diversity check rejects the all-zeros test body, OR returns only the generic `Hochentropischer Token-String` reason; the issuer-specific reason is absent) and passes post-fix (the new regex returns the issuer-specific reason at the prefix boundary).

**Learning:** The closing-checklist sweep continues to be the canonical pattern for issuer-prefix audits. **Adjacent-prefix candidates (Mailgun, Square) anchor immediately** via a same-PR regex addition — distinct from bucket-(b) candidates (AWS, Cloudflare, Heroku) that cannot anchor specific attribution and remain at the entropy fallback indefinitely. **The Shopify family closure opens the e-commerce-platform tier** as a new sub-landscape, mirroring the GitLab developer-tooling-tier closure pattern (multi-prefix issuer taxonomy enumerated iteratively). **Per-prefix attribution within a vendor family is operationally critical** when revocation flows diverge: Shopify has FOUR DISTINCT revocation URLs on admin.shopify.com (per-prefix), and a single-vendor "Shopify Token gefunden" attribution would force the operator to scan four admin pages during an active incident — the per-prefix reason cuts triage time from minutes to seconds. Closing-checklist invariant for future Sentinel rounds: **whenever a new vendor family is added, enumerate ALL documented prefixes in the same PR** (Shopify's 4 prefixes here mirror GitLab's 9-prefix enumeration); deferring siblings to "next round" risks the inventory drifting against upstream's published taxonomy.

**Next-round candidates (named-but-deferred):** Magento 2 admin integration token (`Bearer <40 hex>` — no canonical prefix, bucket-(b)); BigCommerce API token (`<32-64 base64url>` — no canonical prefix, bucket-(b)); WooCommerce REST API key (`ck_<32 hex>` + `cs_<32 hex>` pair — adjacent prefix candidate); Mailchimp API key (`<32 hex>-us<region>` — adjacent prefix candidate, with the region suffix as the structural disambiguator); SES SMTP credentials (no canonical prefix, bucket-(b)); Postmark API key (`<UUID-shape>` — no canonical prefix, bucket-(b)). The e-commerce-platform tier (opened this round) is the natural continuation point: WooCommerce + BigCommerce close the SMB tier; Magento closes the enterprise tier.

**Marker:** SENTINEL_SECRET_SCANNER_DRIFT_ROUND_12.

## 2026-05-15 - Network/Env/Sidecar Non-Finite Literal Drift (Symmetric Companion of PR #1503's Committed-Reader Closure): 18 JSON Parser Sites Outside the Round-1503 Inventory Accepted `NaN` / `Infinity` / `-Infinity` / `1e1000` Literals — Network-Tainted HTTP Responses (13 Sites) + Env-Tainted `BOUNDINGBOX_VIENNA` (2 Sites) + Disk-Sidecar State Readers Missed by PR #1503 (3 Sites) — All Closed Via New `loads_finite()` Wrapper + Direct Hook Pinning

**Vulnerability:** PR #1503 closed the on-disk reader-side gap for the eleven committed-state-file readers (`read_capped_json` + 10 callers across `src/`). The drift this round closes: **every JSON parser site OUTSIDE the Round-1503 inventory** was left at lenient-mode `json.loads(content)` / `response.json()` and accepts the non-finite literal family (`NaN` / `Infinity` / `-Infinity` literal tokens + `1e1000` scientific-notation overflow → `float('inf')` via the default `parse_float=float` hook). Three orthogonal taint channels:

1. **Network-tainted (13 sites)** — every HTTP response parsed into the build pipeline: `src/providers/wl_fetch.py:_get_json` (Wiener Linien API), `src/places/client.py:_post` + `_format_error_message` (Google Places 200-branch + error-formatter), `src/places/hafas_client.py:_fetch_hafas_location` (HAFAS Mgate LocMatch), `src/places/osm_client.py:OSMOverpassClient._fetch_payload` (OSM Overpass), `src/feed/reporting.py:_GithubIssueReporter.submit` (GitHub API issue submission, error + success branches), `scripts/check_overpass_status.py:_evaluate_response` (Overpass smoke probe), `scripts/verify_vor_access_id.py` (VOR access-id verifier), `scripts/update_baustellen_cache.py:_load_json_from_content` (Wien Baustellen), `scripts/update_stammstrecke_hbf.py` (VAO `/departureBoard` Hbf), `scripts/update_stammstrecke_status.py` (VAO `/trip` legacy + error envelope decode).

2. **Env-tainted (2 sites)** — the `BOUNDINGBOX_VIENNA` override parsed via `json.loads(env_str)` in `scripts/fetch_google_places_stations.py:_parse_bounding_box` + `scripts/update_station_directory.py:_parse_bounding_box`. A leaked CI env / compromised secret store / hostile operator plants `{"min_lat": NaN, ...}`; the downstream `float(data["min_lat"])` coercion lifts NaN into the `BoundingBox` dataclass — every place-fetch URL constructed from the bounds carries the non-finite, and the writer-pin round-trip crashes the next persist.

3. **Disk-sidecar state readers missed by PR #1503 (3 sites)** — `src/build_feed.py:_read_state_capped` (the second `data/first_seen.json` reader on the save-path merge — same file as the Round-1503 `_load_state` but separate function), `scripts/update_stammstrecke_status.py:_load_pending_trips` (`cache/stammstrecke/pending_trips.json`), `scripts/update_stammstrecke_status.py:_load_recently_finalised` (`cache/stammstrecke/recently_finalised.json`).

**Threat model:** A compromised upstream / DNS-hijack / MITM / leaked-CI-env / hostile-operator plants a non-finite literal at the taint source. The lenient-mode parser returns a Python structure with `float('nan')` / `float('inf')` inside, which then poisons (a) **comparisons** (`nan != nan` is `True` — breaks every dedup invariant and timestamp ordering), (b) **arithmetic** (`nan + x` is `nan` — silently corrupts latency averages, retention windows, delay calculations), and (c) **round-trip back to the writer** — the writer-pin (`allow_nan=False` from Round 1485/1487/1488/1491) catches the non-finite at serialisation time and raises `ValueError`, crashing the cron pipeline mid-write with no recovery.

**Fix:** Added new `loads_finite()` wrapper in `src/utils/files.py` (thin shim over `json.loads(raw, parse_constant=_reject_non_finite_constant, parse_float=_reject_non_finite_float)`). At each enumerated site: `json.loads(raw)` / `_json_lib.loads(content)` replaced with `loads_finite(raw)` / `loads_finite(content)`; every `response.json()` call replaced with `response.json(parse_constant=_reject_non_finite_constant, parse_float=_reject_non_finite_float)` (requests' `Response.json(**kwargs)` forwards to `json.loads` internally). All existing `except (ValueError, json.JSONDecodeError, RecursionError)` handlers catch the rejection transparently — no per-callsite `except` widening needed.

**Learning:** When the writer side gets pinned (`allow_nan=False`), the reader side MUST be pinned symmetrically — at EVERY taint channel: on-disk state files (PR #1503), network upstreams, env-controlled config, AND every state sidecar reader (not just the named "main" readers). The audit walker (`tests/test_sentinel_json_audit_walker.py`) enforces `RecursionError` tolerance at every `json.loads` callsite but does NOT enforce `parse_constant` + `parse_float` pinning — that's a per-channel inventory test (`tests/test_sentinel_committed_reader_non_finite_drift.py` for the Round-1503 channel; `tests/test_sentinel_network_tainted_non_finite_drift.py` for THIS round's network/env/sidecar channel). Future taint channels (e.g. stdin-piped JSON, subprocess stdout JSON, IPC sockets) need their own enumeration + inventory pin file. The canonical `loads_finite()` wrapper (`src/utils/files.py`) is now the canonical entrypoint for any new JSON parsing in the project — new callsites should import it rather than calling `json.loads()` directly.
...
**Vulnerability:** Round 10 (PR #1493) closed the three CI/CD-infrastructure-tier GitLab prefixes (`glrt-` / `gldt-` / `glagent-`) and explicitly named the four developer-tooling-tier siblings (`glft-` / `glimt-` / `glcbt-` / `glsoat-`) + the CircleCI prefix (`CCIPAT_`) as the named-but-deferred next-round targets. Re-running the closing-checklist sweep produces five additional issuer prefixes whose canonical formats silently bypass specific attribution in the post-Round-10 `_KNOWN_TOKENS` table:
  ...
  1. **GitLab Feed Token** (`glft-<20 chars from [A-Za-z0-9_-]>`): issued automatically for every user via `Settings > Access Tokens > Feed token` for personal RSS/Atom-feed authentication against the GitLab REST API. A leak grants the issuing user's read scope to the activity stream — visible issues, MRs, comments, project metadata; for an admin user the feed exposes the entire instance's project taxonomy.
  2. **GitLab Incoming Mail Token** (`glimt-<25+ chars from [A-Za-z0-9_-]>`): embedded in the reply-by-email `Reply-To: noreply+<token>@<instance>.gitlab.com` header. A leak lets a network adversary post comments / MR replies / issue updates **as the issuing user** by sending crafted email to the GitLab inbound-mail relay — full impersonation within the user's commenting scope.
  3. **GitLab CI Build Token** (`glcbt-<partition_prefix>_<body>`): per-build CI token exposed to the job as `CI_JOB_TOKEN`. The structured `<partition_prefix>_<body>` shape (1-3 alphanumeric chars + literal `_` + 20+ char body) is unique among GitLab prefixes and is the structural disambiguator from the flat 20-char-body `glpat-` / `glrt-` / `gldt-` siblings. A leak during the job's lifetime grants the attacker REST-API impersonation as the job: download package/container-registry artefacts, trigger downstream pipelines via the canonical job-token auth flow.
  4. **GitLab Scoped OAuth Access Token** (`glsoat-<20+ chars from [A-Za-z0-9_-]>`): issued by SCIM-integrated SSO providers (Okta / OneLogin / AzureAD / Google Workspace) when an OAuth application provisions a scoped access token. A leak grants the OAuth application's scoped capabilities for the issuing user — typically `read_user` / `read_repository` / `api`. Revocation lives at gitlab.com/-/profile/applications (distinct from the gitlab.com/-/user_settings/personal_access_tokens PAT revocation flow).
  5. **CircleCI Personal API Token** (`CCIPAT_<32+ chars from [A-Za-z0-9_-]>`): issued via app.circleci.com/settings/user/tokens for full CircleCI REST-API v2 access. The `CCIPAT_` prefix was added in 2023 to replace the legacy unprefixed 40-char-alphanumeric CircleCI tokens. A leak grants the issuing user's full CircleCI organisation scope: read every project's pipeline configuration (which embeds inline env-var references to other vendors' tokens — AWS keys, Docker registry creds, third-party API tokens), trigger arbitrary pipelines on attacker-controlled branches, exfiltrate build artifacts, manage SSH keys for project deployments. Blast radius is structurally identical to the Buildkite User Access Token (`bkua_`, Round 8).

Pre-fix every one of these five tokens is matched by the generic high-entropy fallback `[A-Za-z0-9+/=_-]{24,}` as a single `Hochentropischer Token-String` finding — the body and the dashes/underscores all lie inside the entropy alphabet. Post-fix each token receives its issuer-specific German attribution string (`GitLab Feed Token gefunden` / `GitLab Incoming Mail Token gefunden` / `GitLab CI Build Token gefunden` / `GitLab Scoped OAuth Access Token gefunden` / `CircleCI Personal API Token gefunden`) that incident-response playbooks key off (rotation flow, revocation URL, blast-radius estimate). The revocation flows live at gitlab.com/-/user_settings/personal_access_tokens (Feed / Incoming Mail), the per-pipeline job cancellation flow (CI Build), gitlab.com/-/profile/applications (Scoped OAuth), and app.circleci.com/settings/user/tokens (CircleCI) respectively — each distinct from every other vendor's, so issuer-specific attribution accelerates IR triage.

**Learning:** **The GitLab token family is now fully enumerated (9 of 9 documented prefixes covered)** after three rounds of closing-checklist sweeps: Round 1 (`glpat-`), Round 5 (`glptt-`), Round 10 (`glrt-` / `gldt-` / `glagent-`), Round 11 (`glft-` / `glimt-` / `glcbt-` / `glsoat-`). The pattern generalises: **vendors with a multi-prefix token taxonomy require iterative closure rounds whenever the upstream documented prefix list grows past the project's `_KNOWN_TOKENS` coverage**. The closing-checklist sweep should re-run on every Sentinel round to catch newly-shipped prefixes — the platform-side defence shape introduces unique prefixes between major releases (GitLab introduced new prefixes between 14.0 / 15.0 / 16.0; CircleCI introduced `CCIPAT_` in 2023; GitHub's `ghs_` shipped alongside Actions secrets). Detection-from-zero is already covered by the entropy fallback; the issuer attribution is what determines whether IR triage takes minutes or hours. Next-round candidates: the bucket-(b) (no canonical prefix) tier — AWS Secret Access Keys, Cloudflare API Tokens, Heroku API keys; the adjacent-prefix candidates — Mailgun `key-<32 hex>`, Square `EAAA<base64>` Access Tokens.

## 2026-05-15 - Secret Scanner Drift Round 10: GitLab CI/CD Infrastructure Tier Closure — `glrt-` (Runner Authentication Token) + `gldt-` (Deploy Token) + `glagent-` (Cluster Agent for Kubernetes Token) — Three Adjacent High-Impact GitLab Issuer Prefixes That Round 9 (New Relic NRAK/NRRA/NRII) Did Not Enumerate

**Vulnerability:** Round 9 (PR #1492) closed the three New Relic prefixes (NRAK / NRRA / NRII) for the **observability tier** sub-landscape and named **Datadog / PagerDuty / Honeycomb** as deferred bucket-(b) candidates (no canonical prefix). Re-running the issuer-prefix audit against the **GitLab token family** — currently covered only for Personal Access Tokens (`glpat-`) and Pipeline Trigger Tokens (`glptt-`) — surfaced **three additional GitLab prefixes** whose canonical formats silently bypass specific attribution in the post-Round-9 `_KNOWN_TOKENS` table:

  1. **GitLab Runner Authentication Token** (`glrt-<20 chars from [A-Za-z0-9_-]>`): issued via project / group / instance Runner registration in GitLab 15.6+ (the post-16.0 default replacing the legacy unprefixed registration-token shape). A leak lets a network adversary **register a rogue Runner** against the issuing scope; the runner drains the CI job queue and every CI job (with every protected-branch-scoped secret) lands on attacker-controlled hardware. Blast radius identical to Buildkite Agent Token (`bkat_`, Round 7).
  2. **GitLab Deploy Token** (`gldt-<20 chars from [A-Za-z0-9_-]>`): issued via project / group settings > Repository > Deploy Tokens in GitLab 16.0+. A leak grants Container Registry / Package Registry read/write access; a tampered image pushed via a leaked deploy token persists across every downstream pull, bypassing the source-repository security gate entirely.
  3. **GitLab Cluster Agent for Kubernetes Token** (`glagent-<50+ chars from [A-Za-z0-9_-]>`): issued via project / group settings > Operate > Kubernetes clusters > GitLab Agent in GitLab 14.0+. A leak lets a network adversary **register a rogue Agent for Kubernetes**; the agent runs `kubectl` inside the target cluster under the configured impersonation account, reading / mutating every Kubernetes resource the agent's RBAC binding permits.

Pre-fix every one of these three tokens is matched by the generic high-entropy fallback `[A-Za-z0-9+/=_-]{24,}` as a single `Hochentropischer Token-String` finding — the body and the dashes both lie inside the entropy alphabet. Post-fix each token receives its issuer-specific German attribution string (`GitLab Runner Authentication Token gefunden` / `GitLab Deploy Token gefunden` / `GitLab Cluster Agent Token gefunden`) that incident-response playbooks key off (rotation flow, revocation URL, blast-radius estimate). The revocation flows live at gitlab.com/<scope>/-/runners, gitlab.com/<project>/-/settings/repository#js-deploy-tokens, and gitlab.com/<project>/-/settings/cluster_agents respectively — each distinct from every other vendor's, so issuer-specific attribution accelerates IR triage.

**Learning:** Treat `_KNOWN_TOKENS` as an issuer-keyed table, not a list. **Every vendor with a multi-prefix token family (GitHub: ghp/gho/ghu/ghs/ghr/github_pat; Slack: xoxb/xoxp/xoxa/xoxr; Stripe: sk_live/sk_test/rk_live/rk_test/whsec; New Relic: NRAK/NRRA/NRII) eventually needs a closing-checklist sweep to enumerate every shipped prefix**. GitLab's nine documented prefixes (glpat / glptt / glrt / gldt / glagent / glft / glimt / glcbt / glsoat) were covered for two of nine before this round — closing the three CI/CD-infrastructure-tier siblings (`glrt-` / `gldt-` / `glagent-`) leaves the four developer-tooling-tier siblings (`glft-` / `glimt-` / `glcbt-` / `glsoat-`) as the named-but-deferred next-round target. Whenever a new GitLab token type is rolled out by upstream (the platform-side defence shape introduced unique prefixes between 14.0 / 15.0 / 16.0 with backward-compatible legacy shapes), the scanner's `_KNOWN_TOKENS` table needs a same-PR widening. Detection-from-zero is already covered by the entropy fallback; the issuer attribution is what determines whether IR triage takes minutes or hours.
...
**Vulnerability:** Eight sibling sites interpolated operator-controlled env values via the bare ``%r`` repr conversion in a ``logger.warning`` call:
  ...
  1. `scripts/update_station_directory.py:_parse_radius` (line 793 pre-fix) — `PLACES_RADIUS_M`.
  2. `scripts/update_station_directory.py:_parse_max_results` (line 804 pre-fix) — `PLACES_MAX_RESULTS`.
  3. `scripts/update_station_directory.py:_parse_float` (line 817 pre-fix) — generic float env (`REQUEST_TIMEOUT_S`, `MERGE_MAX_DIST_M`).
  4. `scripts/update_station_directory.py:_parse_int` (line 827 pre-fix) — generic int env (`REQUEST_MAX_RETRIES`).
  5. `scripts/update_baustellen_cache.py:_resolve_data_url` (line 352 pre-fix) — `BAUSTELLEN_DATA_URL` unknown-host warning.
  6. `scripts/update_baustellen_cache.py:main` (line 626 pre-fix) — `BAUSTELLEN_TIMEOUT` non-integer warning.
  7. `src/build_feed.py:_read_optional_non_negative_int` line 370 (pre-fix) — per-provider env (`PROVIDER_<SLUG>_MAX_AGE_DAYS` etc.) on the parse-failure WARNING branch.
  8. `src/build_feed.py:_read_optional_non_negative_int` line 378 (pre-fix) — same env on the negative-value WARNING branch.

While Python's `repr()` escapes the C0 control set (`\n`, `\r`, `\x1b`, the rest of U+0000-U+001F + U+007F), the 8-bit C1 band (U+0080-U+009F), the soft-hyphen / BiDi marks / zero-width family / Cf-Format primitives (escaped as `\u…` or `\x…`), and the supplementary-plane Tag block (escaped as `\U…`), it leaves all **256 code points in Unicode general-category `Mn` (Mark, nonspacing)** — specifically the Variation Selector ranges **U+FE00–U+FE0F (16 codes) + U+E0100–U+E01EF (240 codes)** — in the output **verbatim**. These code points are all members of the canonical `src/utils/logging.py:_INVISIBLE_DANGEROUS_RE` always-strip floor — added in the 2026-05-11 "Tag-Character / Variation-Selector Drift" round explicitly to defeat them. Severity: **MEDIUM** — operator-facing log-injection / Trojan-Source / cache-key-collision primitive on the cron-pipeline log streams. No code execution.

Threat positions and reach: an operator-controlled, leaked-CI-runner, compromised-secret-store, or `.env` file value carrying any Variation Selector flows verbatim into:

  * `record.args[N]` — what every pre-`SafeFormatter` consumer iterates over (custom `RecordFactory`-style adapters, structured log emitters that read args separately from msg, third-party SDKs that hook into the `logging` module's record stream).
  * `record.getMessage()` — what pytest's `caplog.text`-based assertions see (caplog uses its OWN `logging.Formatter`, NOT the project's `SafeFormatter`); what any non-`SafeFormatter` `logging.Handler` formats (e.g. an early-failure path that fires the WARNING BEFORE `configure_logging` / `setup_script_logging` runs, the GitHub Actions log viewer's pass-through adapter, rsyslog with Python logging bridges, ELK / Datadog / Splunk shippers that don't install the project's formatter).
  * The aggregated cron log captured by `update-cycle.yml` in `$log_dir/<script>.log` (e.g. `baustellen.log`) and surfaced in the workflow run UI.

The drift fingerprint: the canonical pattern (pinned by PRs #1456, #1468, #1472, #1473, #1475 — "Path-Log Sibling Drift" rounds 1-3) is `sanitize_log_arg(raw)` + `%s`, which strips the canonical `_INVISIBLE_DANGEROUS_RE` union (including all 256 Variation Selectors) plus ANSI escape codes, redacts the secret-keys union, and escapes `\n`/`\r`/`\t` at the args level (BEFORE the value reaches `record.args` / `record.getMessage()`). The pre-fix `%r` shape pushes the sanitisation responsibility to the formatter layer — which works for `SafeFormatter` consumers, but is a structural drift from the canonical contract and leaks through every alternate consumer.

**Learning:** The 2026-05-13 Round 2 closing-checklist named `scripts/update_station_directory.py` (17 WARNING/INFO sinks) as already-fixed, and the 2026-05-14 Round 3 closure named `scripts/update_baustellen_cache.py` (5 sinks). Neither round audited the surviving `%r` shape — `%r` was treated as equivalent to `%s + sanitize_log_arg` because of `repr()`'s escape-by-default behaviour on most attack bytes. The Variation Selector gap (Python's `repr` considers `Mn`-class characters "printable") is the failure mode that the prior rounds did not enumerate. The structural invariant the rest of the canonical defence pins is "every operator-controlled value reaches `record.args` already-sanitised" — `%r` only partially satisfies this contract.

**Prevention:** Eight coordinated edits, all pinned by `tests/test_sentinel_env_repr_log_injection.py` (97 PoC + inventory tests):

 1-4. Replace `%r` with `%s + sanitize_log_arg(raw)` in `_parse_radius` / `_parse_max_results` / `_parse_float` / `_parse_int` at `scripts/update_station_directory.py` (the helper is already imported at module top).
 5-6. Replace `%r` with `%s + sanitize_log_arg(text)` / `%s + sanitize_log_arg(timeout_raw)` at `scripts/update_baustellen_cache.py:_resolve_data_url` and `scripts/update_baustellen_cache.py:main` (the helper is newly imported alongside the existing `utils.ids` / `utils.serialize` imports).
 7-8. Replace `%r` with `%s + sanitize_log_arg(raw)` at `src/build_feed.py:_read_optional_non_negative_int` on both the parse-failure and negative-value WARNING branches (the helper is already imported at module top alongside the existing `sanitize_log_arg(str(exc))` site).

The PoC pins:
 * Per-primitive caplog walker across 13 canonical-floor primitives × 7 sites = 91 per-code-point bypass tests.
 * Baseline regression-guard `test_repr_leaks_variation_selector_into_record_args` — fails the day Python's `repr()` starts escaping Variation Selectors and the fix becomes redundant.
 * Whole-category structural invariant `test_variation_selector_range_inventory_leaks_through_repr` — enumerates all 256 Variation Selectors and asserts each one passes through `repr()` verbatim, codifying the threat model so a future Python release that closes the gap is detected.
 * Source-grep inventory `test_inventory_no_env_repr_log_drift_in_canonical_sites` — fails on any future `logger.warning("... %r", raw_env_value)` drift in the three canonical sites.
 * Positive invariants: safe ASCII correlation tokens still survive the fix (operators can still distinguish "bad PLACES_RADIUS_M" from "bad PLACES_MAX_RESULTS" in the log); ANSI escape sequences and newline log-forgery primitives are stripped at the args level (defence-in-depth on top of `repr()`'s own escaping).

**Audit invariant for next round:** A future audit walker can enumerate every `logger.<level>(<fmt>, raw_env_value)` call site in `src/` and `scripts/` where `<fmt>` contains `%r` AND `raw_env_value` originates from `os.getenv` / `os.environ` / `env.get`. Today: the three canonical sites above are clean post-fix. The remaining `%r` callsites in the codebase emit hardcoded probe queries (`PROBE_QUERY`), exception-type names (`type(exc).__name__`), tile coordinates (`tile`), upstream-derived but non-string identity values (`value` from internal state files), or first-seen ISO timestamps that have already been validated against `datetime.fromisoformat` — none flow operator-controlled env strings. The next-round audit shape: `grep -rn 'log\(ger\)?\.\(info\|warning\|error\|debug\)([^)]*%r' src/ scripts/ | xargs -I{} verify-not-env-controlled`.

## 2026-05-14 - Companion-Writer Drift: Five `data/stations.json` Writers + the Unified Cache Writer Missed the `allow_nan=False` Defence Pin Established at `src/places/merge.py:write_stations` (Round 1485); Plus the Baustellen Parser `_build_location` Accepted Non-Finite (`NaN`/`+Inf`/`-Inf`) and Out-of-WGS84-Range Coordinates from a Compromised `data.wien.gv.at` Upstream

**Vulnerability:** The 2026-05-14 coordinate-finite/range round (PR #1485, commit `31570fe`) pinned `allow_nan=False` on `src/places/merge.py:write_stations` as defence-in-depth for the parser-level finite floor at `src.places.hafas_client._extract_first_location` and `src.places.client.GooglePlacesClient._parse_place`. The drift this round closes: **five additional writers all land payloads in the same on-disk artefact (`data/stations.json`) but only the canonical `write_stations` carried the writer-defence pin.** Plus the cache writer in `src/utils/cache.py:write_cache` (Baustellen feed lands `coordinates: {lat, lon}` in committed `cache/baustellen_*/events.json`) lacked the pin too. Total drifted sites:

  1. `scripts/update_station_directory.py:write_json` (line 2041 pre-fix) — primary OEBB Excel writer; the local `_coerce_float_value` parser accepts `float('nan')` / `float('inf')` via the literal CSV strings `"nan"` / `"inf"` / `"infinity"`.
  2. `scripts/enrich_station_aliases.py:_write_stations_payload` (line 794 pre-fix) — re-reads `data/stations.json` and rewrites it after alias enrichment; a planted upstream NaN survives Python's `json.loads` lenient mode and flows verbatim.
  3. `scripts/update_all_stations.py:_write_stations_payload` (line 538 pre-fix) — orchestrator temp-file writer that is copy-back'd to `data/stations.json`.
  4. `scripts/update_wl_stations.py:merge_into_stations` (line 1226 pre-fix) — WL OGD CSV merger; local `_coerce_float` accepts non-finite literals.
  5. `scripts/fetch_google_places_stations.py:_write_if_changed` (line 317 pre-fix) — Google-Places-only manual escape hatch.
  6. `scripts/fetch_google_places_stations.py:_dump_changes` (line 292 pre-fix) — diff-dump writer reviewed by operators.
  7. `src/utils/cache.py:write_cache` (line 435 pre-fix) — unified cache writer; Baustellen carries `coordinates: {lat, lon}` into committed `cache/baustellen_*/events.json`.

Additionally the Baustellen parser `scripts/update_baustellen_cache.py:_build_location` (line 509 pre-fix) did `lon, lat = float(coordinates[0]), float(coordinates[1])` without `math.isfinite()` or WGS84-range guards — the fourth-tier ingest parser missing the canonical floor pinned at HAFAS / Google Places / OSM siblings. Severity: **MEDIUM** — public-artefact data-integrity attack with multi-vector attacker positions (compromised upstream, MITM/DNS hijack, planted on-disk cache that survives a re-read cycle). No code execution, no credential leak.

**Learning:** Same drift family as the prior "feed_path JSON-sink drift" — when a canonical writer-side defence is established at one sink, every parallel sink that lands data in the SAME on-disk artefact (or a parallel artefact committed to the same cron pipeline) must inherit the pin. The audit walker for this round's inventory invariant (`test_inventory_every_committed_artefact_writer_pins_allow_nan_false`) enumerates every committed-to-main JSON writer that may carry float coordinates and asserts each pins `allow_nan=False`. A future writer added to the cron commit set without the pin fails this test on the first pytest run after merge. The structural invariant is "every writer that lands a JSON artefact in a path the cron pipeline commits to `main` MUST pin `allow_nan=False`" — same shape as the prior "every reachable string sink applies the canonical attack-byte scrub" rounds.

**Prevention:** Eight coordinated edits, all pinned by `tests/test_sentinel_companion_writer_allow_nan_drift.py` (50 PoC + inventory tests):
 1-6. Pass `allow_nan=False` to each of the six listed station writers (writer-side defence-in-depth).
 7. Pass `allow_nan=False` to `src/utils/cache.py:write_cache` (writer-side defence-in-depth for cache).
 8. Apply the canonical `math.isfinite` + WGS84-range check at `scripts/update_baustellen_cache.py:_build_location` (parser-level floor for the fourth-tier ingest). Mirrors the canonical scrub pinned at `src/places/hafas_client._extract_first_location` / `_parse_place` / `filter_complete_places`.

**Audit invariant for next round:** A future audit walker can enumerate every writer call site that emits an on-disk artefact committed to `main` by any cron workflow (`update-cycle.yml`, `update-stations.yml`, `update-google-places-stations.yml`, `update-vor-cache.yml`, `manual-full-refresh.yml`) and assert each pins `allow_nan=False`. Today: every committed-to-main writer that may carry floats is enumerated; non-coord-carrying writers (`_write_quarantine_file`, `_write_heartbeat_file`, `_write_request_count_file`, `MonthlyQuota.save_atomic`) use `ensure_ascii=True` which is orthogonal but also a defence layer. The inventory test `test_inventory_every_committed_artefact_writer_pins_allow_nan_false` codifies the contract; the parser-side invariant `test_inventory_baustellen_parser_pins_canonical_finite_floor` pins the Baustellen scrub.

## 2026-05-14 - Two Canonical-Floor Drifts: (a) `build_feed_health_payload` Wrote `report.feed_path` (Env-Controlled `OUT_PATH`) Verbatim Into `docs/feed-health.json` With `ensure_ascii=False`, Bypassing Trojan-Source / 8-bit-C1 / Tag-Block / Variation-Selector Strip; (b) `filter_complete_places` (OSM, Third Ingest Tier) Only Checked `math.isnan` — Did NOT Apply the Canonical `math.isfinite` + WGS84-Range Check Pinned by HAFAS / Google Places Siblings

**Vulnerability:** Two structural canonical-floor drifts left over after the prior round:

  1. **JSON-sink Trojan-Source drift** (`src/feed/reporting.py:build_feed_health_payload` line 759 pre-fix). The function emitted `report.feed_path` (POSIX form of the env-controlled `OUT_PATH` resolved by `src.feed.config.resolve_env_path`) VERBATIM into the JSON payload's `run.feed_path` field. `validate_path` only checks the path resolves under one of `ALLOWED_ROOTS` (`docs/`, `data/`, `log/`); it does NOT strip the canonical CVE-2021-42574 attack-byte union. The companion writer `write_feed_health_json` calls `json.dump(payload, f, ensure_ascii=False)`, so the bytes survive as raw UTF-8 in the published `docs/feed-health.json` artefact (committed to `main` by every `update-cycle.yml` cron tick, served via GitHub Pages + raw.githubusercontent.com, consumed by SIEMs / LLM-driven downstream services). The companion Markdown sink (`render_feed_health_markdown` line 593) and the GitHub-Issue body (`_GithubIssueReporter._build_body` line 1131) BOTH correctly route `feed_path` through `src.utils.text.safe_markdown_codespan` (the canonical inline-code-span helper that strips the canonical floor + collapses whitespace + replaces backticks + caps length). Only the JSON sink was the structural drift — every sibling sanitiser in the codebase covers the canonical floor at this exact shape (provider-controlled / env-controlled string flowing into a public artefact via `ensure_ascii=False` JSON dump), but this one sink emitted the bytes raw. Attack alphabet covered: BiDi RLO (U+202E) / Tag block (U+E0000–U+E007F) / Variation Selectors / zero-width family (U+200B–U+200D) / SOFT HYPHEN (U+00AD) / Cf-class Format characters / 8-bit C1 (U+009B CSI, U+009D OSC) / ASCII C0 controls. Severity: **MEDIUM** — Trojan-Source visual deception + steganographic data smuggling + LLM prompt-injection smuggling + cache-key collision primitive on a public-served, machine-consumed artefact. No code execution.

  2. **OSM `filter_complete_places` finite/range drift** (`src/places/osm_client.py` line 617 pre-fix). The function checked `math.isnan(lat) or math.isnan(lon)` only. The HAFAS sibling `_is_valid_wgs84_coord` and the Google Places `_parse_place` both apply `math.isfinite(...)` + WGS84-range (`-90 ≤ lat ≤ 90`, `-180 ≤ lon ≤ 180`); the OSM filter relied on the upstream `_iter_stations` `BoundingBox.contains` gate to incidentally reject `±Inf` (every comparison with `±inf` falls outside any finite envelope) — the prior 2026-05-14 round explicitly named this as the "incidental" defence. The function is exported in `src.places.osm_client.__all__` and the docstring claims "non-empty name and finite coordinates" but the code did NOT enforce the "finite" half. Defence-in-depth threat shape: a future refactor that relaxes the bounding box (e.g. switches to a multi-country envelope so the project ingests Praha / Budapest / Roma fallbacks), removes `_iter_stations` from the call chain, or a third-party caller that invokes `filter_complete_places` on places of unknown provenance bypasses the indirect protection and lands non-finite or out-of-range coordinates into the canonical merge / writer chain. Severity: **LOW-MEDIUM** — structural inconsistency rather than an exploited bypass today; closes the third-tier drift so the canonical-floor invariant is uniform across all three ingest tiers (OSM, HAFAS, Google Places).

**Learning:** Two related shapes of the same canonical-floor drift family. The JSON-sink drift is the same shape as the Trojan-Source rounds where every reachable string sink applies the canonical attack-byte scrub — `feed_path` is "field that the Markdown sink already scrubs but the parallel JSON sink does not." A reliable audit: enumerate every `(field, markdown-sink-path, json-sink-path)` tuple where the field flows into both renderers, and check that BOTH paths apply the canonical floor. The OSM filter drift is the "third-tier-with-incidental-protection" shape — when a sibling's defence relies on an upstream gate (here, `BoundingBox.contains`) rather than its own boundary check, the structural invariant ("every ingest tier applies the canonical floor BEFORE the data lands in the committed artefact") is satisfied only by accident; a refactor of the upstream gate breaks the assumption silently.

**Prevention:** Two coordinated boundary defences, both pinned by inventory tests:
 1. **JSON sink** (`src/feed/reporting.py:build_feed_health_payload`): apply `_CONTROL_CHARS_RE.sub("", report.feed_path)` to `feed_path` (with a `None`-guard) before assigning it into the JSON payload — same regex as the existing duplicate-summary scrub at lines 739/741, so a future widening of `_CONTROL_CHARS_RE` automatically covers both sinks. Pinned by `tests/test_sentinel_feed_health_json_feed_path_drift.py` with 41 per-primitive PoCs across the canonical-floor inventory + a combined-payload PoC + 7 legitimate-content invariants + `None`-safety + an inventory invariant that greps the function source for `_CONTROL_CHARS_RE`.
 2. **OSM filter** (`src/places/osm_client.py:filter_complete_places`): replace the `math.isnan(...)` check with `math.isfinite(...)` (the strict superset that catches `NaN` AND `±Inf`) plus a WGS84-range check, mirroring the HAFAS / Google Places parser-boundary defences. Pinned by `tests/test_sentinel_osm_filter_finite_range_drift.py` with 12 per-primitive PoCs across the non-finite / out-of-range inventory + 9 legitimate-coordinate invariants (Vienna, boundary corners, distant ÖBB termini) + a NaN regression invariant + two inventory invariants (source-grep + runtime).

**Audit invariant for next round:** A future audit walker can enumerate every `(field, markdown-sink, json-sink)` triple where a single source field flows into both `render_feed_health_markdown` AND `build_feed_health_payload` (and similarly for every "field × Markdown × JSON × Issue-body × log-line" sink fan-out), then assert that ALL render paths apply the canonical floor. The inventory of fields to walk: `report.feed_path`, `report.run_id`, `report.exception_message`, `entry.detail`, `entry.status`, `entry.fetch_type`, `entry.name`, `dup.dedupe_key`, `dup.titles[*]`, `report.warnings[*]`, `report.errors[*]`, `report.durations.keys()`. Today: `feed_path` is the only fan-out where the JSON sink was a drift — the others are all sanitised at construction (`clean_message`, `add_warning`, `add_error_message`) or are constants ("ok"/"empty"/"error"/"disabled" / hardcoded provider names / "cache"/"network" / timestamps / float durations). The next round's audit walker can codify this as `test_inventory_feed_health_payload_fields_all_sanitised_or_constant`.

## 2026-05-14 - Coordinate Ingest Drift: `_extract_first_location` (HAFAS) and `_parse_place` (Google Places) Accept Non-Finite (`NaN`/`+Inf`/`-Inf`) and Out-of-WGS84-Range Coordinates from a Compromised Upstream, Persisting Them Verbatim Into the Public `data/stations.json` Artefact

**Vulnerability:** The HAFAS parser `src.places.hafas_client._extract_first_location` (added 2026-05-11, PR #1482) and the Google Places parser `src.places.client.GooglePlacesClient._parse_place` both validated coordinate types via `isinstance(value, int | float)` only — neither applied a `math.isfinite()` check or a WGS84-range check on the post-`float()` lat/lon pair. The OSM sibling `src.places.osm_client.filter_complete_places` defends against `NaN` via `math.isnan` at line 617 AND incidentally against `±Inf` via `BoundingBox.contains` (lines 516-517 reject every comparison with `±inf`). `json.loads` accepts the non-standard `NaN` / `Infinity` / `-Infinity` JSON literals in Python's default lenient mode, so a compromised HAFAS upstream (`fahrplan.oebb.at`), compromised Google Places upstream (`places.googleapis.com`), or any MITM on either connection could plant `crd: {x: NaN, y: Infinity}` (HAFAS) or `location: {latitude: NaN, longitude: Infinity}` (Google Places). The poisoned coordinates flow straight through `scripts/update_station_directory._enrich_with_hafas` (which commits `hit["lat"]` / `hit["lon"]` onto `station.extras` without post-validation) and `src.places.merge.merge_places` (which writes `place.latitude` / `place.longitude` onto `station["latitude"]` / `["longitude"]`) into `src.places.merge.write_stations`. The writer called `json.dumps` with the default `allow_nan=True`, so the resulting on-disk JSON carried the non-standard `NaN` / `Infinity` literals — invalid per RFC 8259, broken in every strict downstream consumer (`JSON.parse` in every modern browser, `serde_json` in Rust, `encoding/json` in Go), silently visualised at impossible map locations by the lenient consumers that do accept them. Same shape covered the out-of-WGS84-range integer-overflow attack (`x: 999999999999` post-scale produces `lon=999999.999999` — geographically impossible). Public sinks impacted: `data/stations.json` committed to `main` on every weekly `update-stations.yml` cron tick, served from the GitHub web UI / `raw.githubusercontent.com` mirror / `https://origamihase.github.io/wien-oepnv/stations.json` Pages mirror; operator-facing `log/diagnostics.log` lines that emit the same lat/lon values verbatim through every JSON-aware log shipper (Datadog, Splunk HEC, the GitHub Actions log viewer). Severity: **MEDIUM** — public-artefact data-integrity attack with two attacker positions and a four-line bypass surface. No code execution, no credential leak, no SSRF.

**Learning:** The OSM-first / HAFAS-second / Google-Places-third enrichment cascade lands data in the same on-disk artefact (`data/stations.json`) via three independent parsers, but the canonical floor that pins the data shape was only applied at one of the three (OSM). The drift fingerprint: an `isinstance(value, int | float)` type-narrowing that doesn't continue with a finite check or a domain-range check. The structural invariant the rest of the codebase pins is "every ingest tier applies the canonical floor BEFORE the data lands in the committed artefact" — same shape as the Trojan-Source rounds where every reachable string sink applies the canonical attack-byte scrub. A new ingest tier introduced without the canonical floor reaches the artefact unfiltered.

**Prevention:** Three coordinated boundary scrubs, all pinned by `tests/test_sentinel_coordinate_nan_inf_range_drift.py`:
 1. **HAFAS parser** (`src/places/hafas_client.py`): extracted the WGS84 finite/range check into a module-level helper `_is_valid_wgs84_coord(lat, lon)` so the main parser carries a single new branch (C901 stays under the 15-cap). The helper rejects any non-finite or out-of-WGS84-range pair before constructing the `HafasLocation`.
 2. **Google Places parser** (`src/places/client.py`): mirrored the same finite + WGS84-range check on the post-`float()` lat/lon pair, with operator-facing `LOGGER.warning` lines for visibility.
 3. **Writer defence-in-depth** (`src/places/merge.py`): pinned `allow_nan=False` on the `json.dumps` call so a future bypass of either parser floor surfaces as a loud `ValueError` at write time rather than silently corrupting the artefact with non-standard JSON literals.

The inventory invariant `test_inventory_every_places_parser_validates_finite_range` enumerates every public coordinate-emitting parser in `src.places` and asserts each rejects the canonical inventory of non-finite / out-of-range pairs; the writer-side invariant `test_inventory_writer_pins_allow_nan_false_contract` greps the `write_stations` source for the `allow_nan=False` pin. A future fourth ingest tier added without the canonical floor fails the inventory test; a future writer change that drops the `allow_nan=False` pin fails the writer invariant test.

## 2026-05-14 - Cf-Format Drift: Canonical `_INVISIBLE_DANGEROUS_RE` and Eight Sibling Sanitisers Skipped 13 Disjoint Bands (44 Code Points) of Unicode Format-Category (`Cf`) Characters — Most Notably `U+00AD` SOFT HYPHEN, the `U+0600`–`U+0605` Arabic Prefix Band, `U+206A`–`U+206F` Deprecated BiDi Controls, `U+FFF9`–`U+FFFB` Interlinear Annotation, and the Supplementary-Plane Kaithi / Egyptian-Hieroglyph / Shorthand-Format / Musical-Symbol Format Bands

**Vulnerability:** The canonical always-strip floor `_INVISIBLE_DANGEROUS_RE` (`src/utils/logging.py`) and its eight sibling validators (`_CONTROL_CHARS_RE` ×2, `_TROJAN_SOURCE_PRIMITIVES_RE`, `_MARKDOWN_NORMALISE_UNSAFE_RE`, `_CSV_CONTROL_CHARS_RE`, `_CONTROL_RE`, `_UNSAFE_CHARS_RE`, `_UNSAFE_URL_CHARS` ×2) covered ~119 of the 163 Unicode `Cf` (Format) code points after the prior 2026-05-14 "Zero-Width Format Drift" round. 44 `Cf`-class code points across 13 disjoint bands remained uncovered: `U+00AD` SOFT HYPHEN; `U+0600`–`U+0605` ARABIC NUMBER SIGN / SIGN SANAH / FOOTNOTE MARKER / SIGN SAFHA / SIGN SAMVAT / NUMBER MARK ABOVE; `U+06DD` ARABIC END OF AYAH; `U+070F` SYRIAC ABBREVIATION MARK; `U+0890`–`U+0891` ARABIC POUND / PIASTRE MARK ABOVE; `U+08E2` ARABIC DISPUTED END OF AYAH; `U+206A`–`U+206F` deprecated INHIBIT/ACTIVATE SYMMETRIC SWAPPING / INHIBIT/ACTIVATE ARABIC FORM SHAPING / NATIONAL/NOMINAL DIGIT SHAPES BiDi controls; `U+FFF9`–`U+FFFB` INTERLINEAR ANNOTATION ANCHOR / SEPARATOR / TERMINATOR; `U+110BD` and `U+110CD` KAITHI NUMBER SIGN / NUMBER SIGN ABOVE; `U+13430`–`U+13438` EGYPTIAN HIEROGLYPH VERTICAL/HORIZONTAL JOINERS + INSERT AT TOP/BOTTOM START/END + OVERLAY MIDDLE + BEGIN/END SEGMENT; `U+1BCA0`–`U+1BCA3` SHORTHAND FORMAT LETTER OVERLAP / CONTINUING OVERLAP / DOWN STEP / UP STEP; `U+1D173`–`U+1D17A` MUSICAL SYMBOL BEGIN/END BEAM/TIE/SLUR/PHRASE. The single most-impactful omission is `U+00AD` SOFT HYPHEN: it renders zero-width unconditionally in browsers / RSS readers / terminals / IDE preview / GitHub web UI when not at a line-break opportunity, but is stored as a real character in every downstream byte-equality / hash / GUID dedup key. Used in real-world attacks since 2018 (CVE-2018-19165 IDN homographs, CVE-2021-43616 npm package-name spoofing, the 2023 "Sneaky Text" research). A planted upstream payload `"Wien Hbf<U+00AD>evil"` reaches the published RSS feed `docs/feed.xml`, the operator-facing Markdown report `docs/feed-health.md`, the auto-submitted GitHub Issue body, the published `docs/sitemap.xml`, the CSV stats ledger `data/stats/<kind>_YYYY.csv`, the canonical `data/stations.json` directory, every `cache/<provider>/events.json` sidecar, every `data/*.json` state file, and `log/diagnostics.log` verbatim — visually identical to `"Wien Hbfevil"` in every consumer but byte-distinct for every dedup key. The `U+0600`–`U+0605` / `U+06DD` / `U+070F` / `U+0890`–`U+0891` / `U+08E2` Arabic-prefix bands have zero advance width per UAX #9; `U+206A`–`U+206F` are documented zero-width deprecated BiDi controls; `U+FFF9`–`U+FFFB` are CJK ruby annotation formatting controls (zero-width except in dedicated ruby renderers); the supplementary-plane Kaithi / Egyptian-Hieroglyph / Shorthand-Format / Musical-Symbol bands are documented zero-width formatting controls with no consumer in the Vienna ÖPNV pipeline. Total smuggling alphabet doubled: from ~119 covered `Cf` code points pre-fix to all 163 post-fix.

**Learning:** The 2026-05-14 prior round explicitly named the prevention rule: "when an 'invisible-character' round lands, run `python3 -c \"import unicodedata; [print(hex(cp)) for cp in range(0x110000) if unicodedata.category(chr(cp))=='Cf']\"` and diff the result against the canonical regex to enumerate the next gap." This round closes that gap completely — every Unicode `Cf` code point that was not in the floor is now covered. The structural invariant pinned by the new test `test_canonical_invisible_dangerous_re_covers_every_unicode_cf_character` enumerates every `Cf` code point in Unicode 16.0 and asserts the floor matches each one, so any future Unicode-spec addition of a new Format-category code point fails the test on the first pytest run after the new `unicodedata` ships, surfacing the next drift family programmatically. The drift family is not "look for new BiDi controls" but the Unicode general-category invariant `unicodedata.category(ch) == "Cf"` — a structural test that future-proofs the floor against Unicode 17 / 18 / etc. additions.

**Prevention:** Two additive coverage layers, both pinned by `tests/test_sentinel_cf_format_drift_round.py`:
 1. **Per-band PoC inventory** (29 spot-check code points across the 13 bands × 9 sibling sanitiser sites = 261 per-code-point bypass tests + 174 end-to-end tests) — fails pre-fix on every sanitiser site that does not match the new Cf bands.
 2. **Whole-category structural invariant** (`test_canonical_invisible_dangerous_re_covers_every_unicode_cf_character`) — enumerates every Unicode `Cf` code point and asserts the canonical floor matches each one. The sibling-superset invariant `test_sibling_regex_covers_canonical_invisible_dangerous_set` (already present from the prior round) inherits the new coverage automatically.
The fix shape mirrors every prior canonical-floor round: extend `_INVISIBLE_DANGEROUS_RE` with the new bands (`­`, `؀-؅`, `۝`, `܏`, `࢐࢑`, `࣢`, widen `⁠-⁩` to `⁠-⁯`, `￹-￻`, `\U000110bd\U000110cd`, `\U00013430-\U00013438`, `\U0001bca0-\U0001bca3`, `\U0001d173-\U0001d17a`) and widen each of the eight sibling validators in lockstep. Additive only against the invisible-character family — every pre-fix-covered code point still matches post-fix; legitimate German (ä/ö/ü/Ä/Ö/Ü/ß), CJK, and emoji content stays untouched.

## 2026-05-14 - Zero-Width Format Drift: Canonical `_INVISIBLE_DANGEROUS_RE` and Eight Sibling Sanitisers Skipped `U+180E` (MONGOLIAN VOWEL SEPARATOR) and the `U+2060`–`U+2064` Band (WORD JOINER, FUNCTION APPLICATION, INVISIBLE TIMES, INVISIBLE SEPARATOR, INVISIBLE PLUS) — All Six Are Cf-Class Zero-Width Format Characters That Survived Every Trojan-Source / Markdown / CSV / RSS / URL / JSON Sink in the Codebase
**Vulnerability:** The canonical always-strip floor `_INVISIBLE_DANGEROUS_RE` (`src/utils/logging.py`) and its eight sibling validators (`_CONTROL_CHARS_RE` ×2, `_TROJAN_SOURCE_PRIMITIVES_RE`, `_MARKDOWN_NORMALISE_UNSAFE_RE`, `_CSV_CONTROL_CHARS_RE`, `_CONTROL_RE`, `_UNSAFE_CHARS_RE`, `_UNSAFE_URL_CHARS` ×2) covered the BiDi formatting controls, line/paragraph separators, the Tag block, and the Variation Selectors — but jumped from `‮` (RLO) to `⁦` (LRI), leaving the `⁠`–`⁤` band uncovered, and never included the legacy `᠎`. All six omitted code points are in Unicode general category `Cf` (Format), the same class as ZWSP/ZWNJ/ZWJ already in the floor, and all six are documented zero-width primitives in published "invisible Unicode steganography" research. A planted upstream payload (provider compromise, OSM Overpass cache poisoning, VOR ReST hijack, MITM, planted CI cache file) carrying a WORD JOINER between two visible tokens — `"Hauptbahnhof⁠evil"` — survived every sink: the JSON sidecar scrubber, the Markdown writer, the CSV stats ledger, the RSS XML serialiser, the URL boundary validator, the stations directory validator, and both `strip_control_chars` paths in the log sanitiser. Public sinks impacted: `docs/feed.xml`, `docs/feed_health.json`, `docs/feed-health.md`, `docs/sitemap.xml`, `data/stats/<kind>_YYYY.csv`, `data/stations.json`, `cache/<provider>/events.json`, GitHub Issue body submitted by `submit_auto_issue`. The `⁠`–`⁤` band specifically is the canonical "Sneaky Text" alphabet — combinations of WJ + INVISIBLE TIMES + INVISIBLE SEPARATOR + INVISIBLE PLUS encode arbitrary bytes that survive copy-paste from any of the above artefacts into an LLM context window (invisible to humans, fully visible to the model). `᠎` defeats every "strip ZWSP family" filter that relies on the `\u200x` band alone.

**Learning:** The 2026-05-11 Tag-Character / Variation-Selector Drift round closed plane-14 and the BMP Variation Selector band but did not audit the small `⁠`–`⁤` gap inside the BiDi region nor the legacy `᠎`. The `Cf` (Format) general category is the structural invariant that ties the omitted band to the family already in the floor: every code point in `Cf` is documented zero-width by Unicode, has no readability value, and has no legitimate consumer in this codebase's data path. The drift family is not "look for new BiDi controls" but "look for any Cf-class code point that is NOT in the floor." A `unicodedata.category(ch) == "Cf"` query against every code point not currently in the regex enumerates the next round's candidates programmatically.

**Prevention:** The fix shape mirrors every prior canonical-floor round: extend `_INVISIBLE_DANGEROUS_RE` and widen each of the eight sibling validators in lockstep with the additive-only invariant pinned by `tests/test_sentinel_zero_width_invisible_drift.py`. The `⁠`–`⁩` range expansion folds in the existing BiDi-isolate band (LRI/RLI/FSI/PDI) plus reserved `⁥`; the unassigned slot has no defined meaning so the additive strip is safe. The pinned inventory test `test_sibling_regex_covers_canonical_invisible_dangerous_set` (already present from the prior round) immediately fails on any future canonical widening that doesn't widen the siblings — so the next drift family is caught at PR-review time rather than during incident response. Going forward: when an "invisible-character" round lands, run `python3 -c "import unicodedata; [print(hex(cp)) for cp in range(0x110000) if unicodedata.category(chr(cp))=='Cf']"` and diff the result against the canonical regex to enumerate the next gap.
