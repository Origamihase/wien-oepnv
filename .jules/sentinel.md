## 2026-05-16 - Token Scheme Detector Drift (De-Facto HTTP `Token` Auth-Scheme Literal, Bearer-Alias Sibling Of NTLM Round): The First-Priority Adjacent-Detector Candidate Named-But-Deferred By The 2026-05-16 NTLM Round Closed In Full — A Leaked `Authorization: Token <opaque-body>` In Committed Source Carried Two Orthogonal Failure Modes (Attribution Drift For 36+ Char Opaque Tokens, Silent Undetection For All-Letter Bodies) That Lost The Token-Scheme-Specific Recovery Surface (Vendor-API-Specific Revocation Flow Per The Consuming Service's Documentation) Entirely

**Vulnerability:** The 2026-05-16 NTLM round (`security(secret-scanner): add NTLM detector per [MS-NLMP]`) named the **GitHub `Token` scheme detector** as the first-priority next-round adjacent-detector candidate (HOBA and Mutual deferred as "rare in practice"; Digest and AWS4-HMAC-SHA256 deferred pending body-structure-aware detector framework). Pre-fix `src/utils/secret_scanner.py` had NO direct `Token` auth-scheme detector — the only coverage for a leaked `Authorization: Token <body>` was via the generic entropy fallback (`_HIGH_ENTROPY_RE` = `(?<![A-Za-z0-9])[A-Za-z0-9+/=_-]{24,}(?![A-Za-z0-9])`) which matched the opaque token body generically, BUT only when the body had ≥2 character categories AND was not all-letter (the `candidate.isalpha()` short-circuit in `_HIGH_ENTROPY_RE`'s loop suppresses LongCamelCaseClassName false positives). The NTLM detector closed direct-NTLM via `(?i)NTLM\s+...` but NOT the de-facto `Token` literal — the most widely-emitted Bearer alias in real-world debug logs / HAR exports / curl traces. The literal `Token` is NOT in the IANA HTTP Authentication Scheme Registry (Basic / Bearer / Digest / HOBA / Mutual / Negotiate / OAuth / SCRAM-SHA-1 / SCRAM-SHA-256 / vapid are the registered IETF-RFC entries; NTLM and AWS4-HMAC-SHA256 are vendor entries), but it is widely used in the wild — GitHub's legacy REST API documentation prescribes `Authorization: token <PAT>` as an accepted alias for `Bearer`, Rails / Devise's default `token_authenticatable` shipped with `Authorization: Token <40-char token>`, Zendesk / Spotify legacy / many internal REST APIs accept the same shape. Two orthogonal failure modes resulted: **(1) Attribution drift** — opaque 36+-char alphanumeric/underscore/hyphen tokens with mixed character classes (the Rails / Devise 40-char-hex default, Zendesk 40+ char alphanumeric, generic 40-char opaque non-vendor-prefixed bodies) lost the Token-scheme-specific attribution and landed as generic `Hochentropischer Token-String`, so incident-response triage had to GUESS the recovery surface (Bearer: revoke at IdP; Basic Auth: rotate user password; NTLM: relay + offline crack; Kerberos: revoke ticket at KDC; Token-scheme: vendor-specific revocation per the consuming API's documentation — FIVE distinct IR flows that hinge on per-scheme attribution). **(2) Silent undetection** — all-letter bodies trip `candidate.isalpha()` and are skipped by the entropy fallback. Pre-fix every leaked `Token <all-letter-body>` was SILENTLY UNDETECTED — the CI gate passed, the opaque Token-scheme credential sat committed in the public repo indefinitely. End-to-end PoC verified pre-fix across every case variation (`token` / `Token` / `TOKEN` / `ToKeN` / `tOkEn`) and every body class (long-mixed 60-char alphanumeric, all-letter 40-char, Rails / Devise 40-char hex, GitHub PAT 40-char `ghp_<36>`): only `Verdächtige Zuweisung eines potentiellen Secrets` or `Hochentropischer Token-String` fired (when the variable name happened to carry a sensitive keyword or the body was mixed-class above 24 chars), losing all Token-scheme-specific attribution. The cross-detector ordering invariant preserves the more-specific `_KNOWN_TOKENS` attribution for vendor-prefixed bodies (GitHub `ghp_<36>` inside `Token` header yields "GitHub Personal Access Token gefunden", not "HTTP Token Authentication Credential gefunden") — `_KNOWN_TOKENS` runs BEFORE `_AUTH_SCHEME_DETECTORS` in `_scan_content`, so the inner PAT-shape match wins and the rotation playbook correctly anchors on github.com/settings/tokens.

**Threat model:** A leaked direct-`Token`-scheme credential is structurally distinct from Bearer/Basic/Negotiate/NTLM leaks in that **the wire-protocol position is identical to Bearer** (the de-facto alias preserves the same single-body Authorization-header shape) **but the consuming API's revocation flow is vendor-specific** (no universal IdP equivalent — GitHub legacy tokens revoke at github.com/settings/tokens; Rails / Devise tokens rotate via the application's user-settings page; Zendesk tokens rotate at the Zendesk admin panel; each consuming API has its own control plane). An attacker who recovers a Token-scheme credential therefore holds: (a) **the consuming API's scope** — the issuing user / service principal's full access to the consuming API as defined by the API's authorization model; the blast radius is API-specific and ranges from read-only metadata (low) to full account control (high); (b) **the password-equivalent surface** — Token-scheme credentials are typically long-lived (no automatic expiry on legacy GitHub PATs; Rails / Devise tokens persist until the user manually rotates), so the leak window is multi-month by default unless the operator has audit-log monitoring on the consuming API; (c) **the cross-API persistence amplifier** — operators frequently reuse the same token-shape (40-char alphanumeric) across multiple internal APIs whose authorization is bridged via shared identity (the same internal user / service identity carries Token-scheme credentials to multiple Token-scheme-authenticated consuming APIs); a leaked Token-scheme credential therefore amplifies the attacker's lateral movement surface across the operator's API ecosystem; (d) **the IR-playbook ambiguity** — without Token-scheme attribution, IR triage cannot anchor on the consuming API's revocation flow without first deduplicating against Bearer / Basic / NTLM / Kerberos. Real-world emission patterns include curl `-v` debug logs for GitHub REST API calls (`Authorization: token ghp_...` — the GitHub-recommended legacy literal that survives in every operations playbook and StackOverflow answer), browser dev-tools Network tab HAR exports for sites using `Token` instead of `Bearer` (Rails / Devise default, Heroku ops dashboards, internal API admin UIs), Python `requests` library debug logs with `logging.getLogger("urllib3").setLevel(logging.DEBUG)` (emits the Authorization header verbatim), Python / Ruby / Go / Node API client docstrings hardcoding tokens in test fixtures, CI/CD pipeline debug output for any `Token`-scheme consumer (Rails / Devise applications, GitHub legacy REST API, Zendesk, Spotify legacy), Postman / Insomnia / Bruno saved-request export JSON / YAML, and Kibana / Grafana / Datadog log views ingesting application `Authorization`-header logs that were emitted before the log sink's redaction rule was applied. The blast radius of an undetected Token-scheme leak is COMPARABLE to a Bearer-Token leak (same wire-protocol position) but with HIGHER ambiguity on the recovery surface (no universal IdP — each consuming API requires per-vendor knowledge to revoke).

**Sites closed (1 detector regex + 1 table entry):**

  * `src/utils/secret_scanner.py:_TOKEN_SCHEME_RE` — new regex appended to `_AUTH_SCHEME_DETECTORS`:
    ```python
    ...
    ```
    The body alphabet `[A-Za-z0-9_\-]` is the canonical alphanumeric + underscore + hyphen token shape used by every major vendor that emits via the `Token` scheme (GitHub PATs `ghp_<36>` and the 40-char classic-PAT shape; Rails / Devise default 40-char hex; Zendesk 40-char alphanumeric). This alphabet is deliberately MORE restrictive than Bearer's `[A-Za-z0-9\-_.]` (no `.`) and Basic / Negotiate / NTLM's `[A-Za-z0-9+/=]` (no `+/=`) — JWTs and base64 tokens with padding are emitted via the Bearer / Basic / Negotiate schemes, not the Token scheme, so the alphabet narrowing reduces false positives in code that happens to embed the word `Token` followed by a dotted version string or base64-padded blob. The 36+ char body floor is the structural disambiguator against natural-language false positives — the English word `Token` appears commonly in code identifiers, prose, and docstrings, but almost never followed by 36+ contiguous chars from `[A-Za-z0-9_\-]`. The canonical leak shapes are ALL above 36 chars (GitHub PAT `ghp_<36>` = 40 chars; GitHub classic 40-char hex; Rails / Devise 40-char hex; Zendesk 40+ chars). The `(?i)` inline flag inherits the RFC 7235 §2.1 case-insensitivity contract from the Bearer / Basic / Negotiate / NTLM siblings.

  * `_AUTH_SCHEME_DETECTORS` table extension — the canonical `_scan_auth_scheme_credentials` helper (single source of truth for HTTP-auth-scheme detector processing, extracted by the 2026-05-16 Basic Auth round) processes the new entry uniformly via the existing iteration loop with the same `_looks_like_secret(candidate, is_assignment=True)` filter (`min_categories=1` so uniform-character-class bodies are NOT silently undetected) and `covered_ranges` mutation contract. No changes needed to `_scan_content` (C901 complexity unchanged) or to `_scan_auth_scheme_credentials` (no new branches). The structural invariant established by the 2026-05-16 Basic Auth round and confirmed by the Negotiate / NTLM rounds held end-to-end for the FIFTH HTTP auth-scheme detector: adding a new HTTP-auth-scheme detector requires only a new regex constant + a new tuple entry — the canonical processing helper handles it without modification. Position invariant: `_TOKEN_SCHEME_RE` lives AT THE END of `_AUTH_SCHEME_DETECTORS` (canonical Bearer-alias position) so more-specific IETF-RFC-defined schemes (Bearer / Basic / Negotiate) and the high-impact vendor scheme (NTLM) match first; the de-facto Token literal serves as the catch-all attribution carrier for opaque credentials that no more-specific detector has claimed.

**Severity:** **MEDIUM** — detection-from-zero failure for a class of realistic Token-scheme credential leaks across GitHub legacy / Rails / Devise / Zendesk / Spotify legacy / generic internal-API consumers. The high-severity case is the silent-undetection branch (all-letter bodies): the Token-scheme credential sits committed in plaintext with NO detection at all. The attribution-drift branch is medium-severity (the body IS caught generically, but the Token-scheme-specific recovery surface that determines which vendor revocation flow applies — github.com/settings/tokens, Rails app user-settings, Zendesk admin panel, etc. — is lost). Mitigated only by the requirement that the leaked credential live alongside a `Token` auth-scheme literal in the same content blob; in practice this covers every leak through curl `-v` debug output, browser HAR exports, requests/urllib3 DEBUG logging, Postman/Insomnia/Bruno exports, and CI/CD pipeline debug logs. CodeQL / Semgrep do not flag this drift because their default rules cover Bearer / Basic auth-schemes but not the de-facto `Token` literal (Token-scheme detection requires recognising the de-facto-but-not-IANA-registered scheme literal, which is rule-table dependent — no industry-standard default).
...
2. Append `(_TOKEN_SCHEME_RE, "HTTP Token Authentication Credential gefunden")` to `_AUTH_SCHEME_DETECTORS` — the canonical iteration helper `_scan_auth_scheme_credentials` processes the new entry uniformly. No code changes elsewhere — the helper preserves byte-for-byte behaviour of the Bearer/Basic/Negotiate/NTLM detectors while extending coverage to the de-facto `Token` literal.

Zero false-positive risk: the body alphabet `[A-Za-z0-9_\-]` is more restrictive than Bearer's; the structural disambiguator (`\s+` between the auth-scheme literal and a 36+ char body) is identical to the four sibling detectors. The English word `Token` is more common than NTLM / Negotiate, but the 36-char body floor is the structural disambiguator — natural English prose breaks at whitespace and punctuation, so no 36+ contiguous match from `[A-Za-z0-9_\-]` is possible inside a sentence. The `_looks_like_secret(candidate, is_assignment=True)` filter at the second layer (`min_categories=1`, uniqueness ≥ `max(6, min(len // 4, 32))`) catches any token-shaped string that does happen to follow a `token`-prefixed natural-language passage — long-repetition placeholders (`Token aaaa...aaaa`) are rejected outright by the uniqueness floor.

**Tests added:** `tests/test_sentinel_secret_scanner_token_scheme_drift.py` — 46 tests across 10 categories:
  1. Per-case attribution PoCs (5 case variants × 60-char mixed-class body) — proves attribution-drift closes for every RFC-7235-conforming Token literal.
  2. Rails / Devise canonical 40-char-hex shape — proves the most common Token-scheme emission outside GitHub yields Token-scheme attribution.
  3. Per-case silent-undetection PoCs (4 case variants × 40-char all-letter body) — proves `candidate.isalpha()` skip bypass closes for Token-prefixed tokens.
  4. Negative cases (16 natural-language passages, code-shape candidates, sub-floor bodies, dot/plus-containing bodies, all-repetition bodies) — proves false-positive surface is bounded.
  5. Cross-detector ordering regression guard — proves GitHub PAT (`ghp_<36>`) in `Token` header still yields GitHub PAT attribution (the `_KNOWN_TOKENS` matching runs BEFORE auth-scheme detectors), and that the wider Token-scheme span does NOT also fire for the same coverage region.
  ...
  8. Auth-scheme detector table membership invariant — pins `_TOKEN_SCHEME_RE` membership in `_AUTH_SCHEME_DETECTORS` and reason-string membership so a future regression that adds the regex but forgets the table binding (or typos the reason) fails fast.
  9. End-to-end emission-shape inventory (9 real-world emission patterns: curl `-v` debug logs, requests/urllib3 DEBUG, browser HAR exports, YAML config, Python docstrings, CI/CD pipeline logs, log-management ingest, Ruby/Go API clients).
 10. Masking-contract test — proves the raw opaque token body never appears unmasked in finding output (mirrors Bearer/Basic/Negotiate/NTLM masking-contract tests).
 11. Auth-scheme detector position invariant — pins `_TOKEN_SCHEME_RE` as the LAST entry in `_AUTH_SCHEME_DETECTORS` (canonical Bearer-alias position) so more-specific schemes match first; future table-rearrangement that violates this ordering fails fast.

**Learning:** The HTTP-auth-scheme detector family extends BEYOND IETF-RFC-defined schemes AND vendor-spec schemes ([MS-NLMP] NTLM) to include **de-facto-but-not-IANA-registered** literals like the legacy GitHub `Token` alias — the structural invariant ("HTTP-auth-scheme literals go to `_AUTH_SCHEME_DETECTORS` regardless of their underlying authoring body") proved end-to-end for the FIFTH detector. The 2026-05-16 Basic Auth round's `_AUTH_SCHEME_DETECTORS` tuple + `_scan_auth_scheme_credentials` helper architecture continues to hold byte-for-byte: adding a de-facto auth-scheme (legacy GitHub `Token` alias) required ONLY a new regex constant + a new tuple entry. ZERO changes to the iteration helper, ZERO changes to `_scan_content`, ZERO C901 complexity impact. **The body-length floor remains the per-scheme structural disambiguator** — Token uses 36+ chars (matches GitHub PAT `ghp_<36>` = 40 chars / Rails-Devise 40-char hex / Zendesk 40-char alphanumeric while excluding the common English word `Token` in prose at the natural identifier-name boundary). **The body alphabet remains determined by the canonical leak shapes** — Token uses `[A-Za-z0-9_\-]` (alphanumeric + underscore + hyphen, deliberately MORE restrictive than Bearer / Basic / Negotiate / NTLM because Token-scheme leaks are dominated by GitHub-PAT / Rails-Devise / Zendesk shapes, not by JWTs or base64-padded blobs which use Bearer / Basic / Negotiate). **The classification question — "de-facto literal in `_AUTH_SCHEME_DETECTORS` or skip entirely?" — resolves to the auth-scheme table** when the literal IS used as an HTTP auth-scheme on the wire (the wire-protocol position determines placement, NOT the IANA registration status). `Token` is an HTTP auth-scheme literal that happens to be de-facto rather than IANA-registered; the placement is in `_AUTH_SCHEME_DETECTORS` for uniformity with Bearer / Basic / Negotiate / NTLM. The five-detector ladder (Bearer / Basic / Negotiate / NTLM / Token) now covers every realistic Authorization-header emission shape from the single-body family — Digest and AWS4-HMAC-SHA256 remain the deferred multi-field shapes pending body-structure-aware framework.

**Next-round candidates (named-but-deferred):** **HOBA detector** (`(?i)HOBA\s+...`) — RFC 7486 HTTP Origin-Bound Auth using a signed-claim dot-separated structure (kid.challenge.nonce.signature); rare in practice but a complete auth-scheme enumeration would include it. **Mutual auth detector** (`(?i)Mutual\s+...`) — RFC 8120 HTTP Mutual Authentication using cryptographic keys with key=value structure; very rare. **vapid detector** (`(?i)vapid\s+t=...,k=...`) — RFC 8292 Voluntary Application Server Identification for Web Push; the body is multi-field (`t=<JWT>,k=<base64url-encoded-public-key>`) like Digest, so similarly deferred pending body-structure-aware framework. **SCRAM-SHA-256 / SCRAM-SHA-1 detector** (`(?i)SCRAM-SHA-(?:1|256)\s+...`) — RFC 7804 Salted Challenge Response Authentication Mechanism; the body is multi-step base64 message data; deferred pending body-structure-aware framework. **Digest Auth detector** (`(?i)Digest\s+username=...`) — RFC 7616 multi-field challenge-response auth STILL deferred pending body-structure-aware detector framework (the credential is split across the `nonce` / `response` / `cnonce` fields; this remains the longest-deferred candidate from the auth-scheme family because the architectural lift is meaningfully larger than the per-detector additions of the past five rounds). **AWS4-HMAC-SHA256 detector** (`(?i)AWS4-HMAC-SHA256\s+Credential=...`) — AWS Sig v4 signed-request auth-scheme; the body is multi-field (`Credential` / `SignedHeaders` / `Signature`); similarly deferred pending body-structure-aware framework.
