...
## 2026-02-02 - Secure Response Consumption for POST Requests
**Vulnerability:** The `GooglePlacesClient` was using `requests.post(...).json()` without limiting the response size or verifying the destination IP address (SSRF/DNS Rebinding), exposing the application to DoS via memory exhaustion or internal network scanning. Existing safe fetch utilities only supported GET requests.
**Learning:** Security primitives like `fetch_content_safe` must be flexible enough to support different HTTP methods (POST) or return types (streamed response vs bytes), or composable parts must be exposed.
**Prevention:** Refactored `src/utils/http.py` to expose `read_response_safe(response, max_bytes)` and `verify_response_ip(response)` as public helpers. Updated `src/places/client.py` to manually construct safe request flows: `stream=True`, `verify_response_ip`, `read_response_safe`, then JSON decoding.

## 2026-02-03 - Proxy Configuration Bypasses SSRF Protection
**Vulnerability:** Application-level SSRF protection (DNS resolution and IP verification) is ineffective when a proxy is configured (e.g., via `HTTP_PROXY`). `requests` delegates the connection to the proxy, so `verify_response_ip` validates the proxy's IP instead of the destination server's IP. This allows a malicious or misconfigured proxy to route requests to internal networks (Split-Horizon DNS), bypassing the application's guardrails.
**Learning:** Application-layer IP filtering is fundamentally incompatible with forward proxies unless the proxy itself is trusted to filter traffic or the application can inspect the proxy's resolution. Relying on `getpeername()` is misleading in proxy environments.
**Prevention:** Added a startup warning in `session_with_retries` to alert administrators if proxy environment variables are detected while `trust_env` is enabled. For high-security environments, proxies should be explicitly disabled (`trust_env=False`) or the proxy infrastructure must be trusted to enforce network segmentation.

## 2026-02-04 - Synchronization of Sensitive Keys in Logging
**Vulnerability:** A discrepancy existed between the sensitive keys defined in `src/utils/http.py` (used for URL error sanitization) and `src/utils/logging.py` (used for general log sanitization). New keys added to `http.py` (e.g., `tenant_id`, `oid`, `code_challenge`) were not automatically reflected in the global log sanitizer, leading to potential leaks if these keys appeared in general logs.
**Learning:** Maintaining duplicate lists of sensitive constants across different modules invites drift and security gaps. When a new sensitive parameter is introduced in one part of the system (e.g., HTTP client), it must be immediately protected in all security layers (logging, error reporting).
**Prevention:** I manually synchronized the lists and added a regression test (`tests/test_logging_sanitize.py`) to verify that all known sensitive keys are redacted. Future improvements should centralize these constants into a single source of truth.
