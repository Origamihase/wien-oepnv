...
- Plain-Text-Beschreibung (TV-tauglich)
...
def _session() -> requests.Session:
    ...
    s.headers.update({
        ...
        "User-Agent": "Origamihase-wien-oepnv/2.7 (+https://github.com/Origamihase/wien-oepnv)"
    })
    return s

S = _session()

# ---------- Relevanz-/Ausschluss-Keywords ----------
...
_LABEL_HEAD_RE = re.compile(r"^\s*(?:(?:" + "|".join(_LABELS) + r")\s*(?:[-:–—/]\s*|\s+))+", re.IGNORECASE)
...
def _tidy_title_wl(title: str) -> str:
    ...
    if stripped and _is_informative(stripped): t = stripped
    t = re.sub(r"[<>«»‹›]+", "", t)               # spitze Klammern u. ä. raus
    return re.sub(r"\s{2,}", " ", t).strip(" -–—:/\t")

# ---------- HTML → Text (für Beschreibung) ----------
...
def _best_ts(obj: Dict[str, Any]) -> Optional[datetime]:
    t = obj.get("time") or {}
    for cand in (_iso(t.get("start")), _iso(t.get("end")),
...
# Präfix-Stripper (entfernt vorhandene Linienblöcke)
...
def _strip_existing_line_block(title: str) -> str:
    ...
    if ":" in t:   # vor ':' evtl. Restliste
        pre, post = t.split(":", 1)
        if ("," in pre) or ("Rufbus" in pre) or ("(" in pre): t = post.strip()
    return t
def _ensure_line_prefix(title: str, lines_disp: List[str]) -> str:
    if not lines_disp: return title
    wanted = "/".join(lines_disp)
    if re.match(rf"^\s*{re.escape(wanted)}\s*:\s*", title, re.IGNORECASE): return title
    stripped = _strip_existing_line_block(title)
    return f"{wanted}: {stripped}".strip()

# Fallback-Linien aus Text — vorher Datum/Zeit/Adressen maskieren
...
def fetch_events(timeout: int = 20) -> List[Dict[str, Any]]:
    ...
    for ev in raw:
        ...
        if not b:
            buckets[key] = {
                "source": ev["source"], "category": ev["category"], "title": ev["title"],
            ...
            if ev["pubDate"] and (not b["pubDate"] or ev["pubDate"] < b["pubDate"]): b["pubDate"] = ev["pubDate"]
    ...
    for b in buckets.values():
        ...
        # Anzahl Halte (falls wir Namen kennen)
        ...
        desc = re.sub(r"[<>]+", "", desc)                 # einzelnes "<" etc. entsorgen
