...
def _unescape_value(value: str, quote_char: str) -> str:
    """Unescape a quoted value."""
    parts: list[str] = []
    idx = 0
    length = len(value)
...
def _parse_env_file(content: str) -> Dict[str, str]:
    ...
    while idx < length:
        ...
        if content[idx] == '"' or content[idx] == "'":
            ...
            # Content strictly inside quotes
            raw_inner = content[val_start:idx]

            # Unescape
            parsed[key] = _unescape_value(raw_inner, quote_char)

            if idx < length and content[idx] == quote_char:
