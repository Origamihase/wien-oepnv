...
def test_wl_entry_matches_existing_name_without_ids(stations_path: Path) -> None:
    stations_path.write_text(
        json.dumps(
            [
                {
                    "name": "Wien Beispielplatz",
                    "aliases": ["Beispielplatz"],
                    "source": "manual",
                }
            ]
        ),
    ...
    wl_entries = [
        {
            "name": "Wien Beispielplatz (WL)",
            "aliases": ["Wien Beispielplatz", "Beispielplatz"],
            "wl_diva": "60201234",
            "wl_stops": [
                {
                    "stop_id": "60201234",
                    "name": "Beispielplatz U",
                }
            ],
            "source": "wl",
        }
    ...
    assert entry["wl_diva"] == "60201234"
    assert entry["wl_stops"] == wl_entries[0]["wl_stops"]
    assert set(entry["aliases"]) == {"Wien Beispielplatz", "Beispielplatz"}
