...
def test_base_url_appends_missing_version(monkeypatch):
    monkeypatch.setenv("VOR_BASE_URL", "https://example.test/vao/restproxy")
    monkeypatch.setenv("VOR_VERSION", "v9.9.9")

    importlib.reload(vor)

    assert vor.VOR_BASE_URL == "https://example.test/vao/restproxy/v9.9.9/"
...
def test_base_url_replaces_mismatched_version(monkeypatch):
    monkeypatch.setenv("VOR_BASE_URL", "https://example.test/vao/restproxy/v1.2.3")
    monkeypatch.setenv("VOR_VERSION", "v4.5.6")

    importlib.reload(vor)

    assert vor.VOR_BASE_URL == "https://example.test/vao/restproxy/v4.5.6/"
    assert vor.VOR_VERSION == "v4.5.6"
