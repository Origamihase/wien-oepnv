...
def test_assign_vor_ids_updates_existing_value() -> None:
    station = make_station("Wien Aspern Nord")
    station.vor_id = "old"
    stops = [make_stop("900100", "Wien Aspern Nord", municipality="Wien")]
    usd._assign_vor_ids([station], stops)
...
def test_assign_vor_ids_uses_short_name_hint() -> None:
    station = make_station("Wien Mitte")
    stops = [
        make_stop("900401", "Landstraße Bahnhof", municipality="Wien", short_name="Wien Mitte"),
        make_stop("900402", "Landstraße Bahnhof", municipality="Wien"),
    ]
    usd._assign_vor_ids([station], stops)
    assert station.vor_id == "900401"


def test_assign_vor_ids_prefers_matching_municipality_token() -> None:
    station = make_station("Tullnerfeld", in_vienna=False)
    stops = [
        make_stop("900801", "Tullnerfeld Bahnhof", municipality="Wien"),
        make_stop("900802", "Tullnerfeld Bahnhof", municipality="Gemeinde Tullnerfeld"),
    ]
    usd._assign_vor_ids([station], stops)
    assert station.vor_id == "900802"
