...
def test_collect_items_cancelled_future():
    ...
    with patch.object(bf, "feed_config", mock_feed_config), \
        ...
        assert any("Timeout nach 0.05s" in arg for arg in error_calls), f"Expected timeout error, got: {error_calls}"

        # It should NOT have the "Fetch abgebrochen" error
        assert not any("Fetch abgebrochen" in arg for arg in error_calls), f"Found 'Fetch abgebrochen' in error calls: {error_calls}"
