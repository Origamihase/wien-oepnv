...
# ============================================================================
# scripts/update_vor_stations.py — merge_into_stations (HIGH)
# ============================================================================
#
# Pre-fix: caught only ``FileNotFoundError``. A depth-bombed stations.json
# raises ``RecursionError`` past the catch; a regular malformed file raises
# ``json.JSONDecodeError`` past the catch. Both crash the VOR merge step
# after the API quota has already been debited for the run, with the
# subprocess-orchestrated cron aborting via ``CalledProcessError``. Post-fix:
# treat both as "start fresh" (mirrors the Round-4-fixed
# ``update_wl_stations.merge_into_stations`` contract). The merge then
# writes the merged result over the poisoned file, restoring the canonical
# schema for the next run.


def test_update_vor_merge_into_stations_handles_depth_bomb(
    ...
    # Pre-fix: a bare RecursionError escaped here. Post-fix: treated as
    # starting-fresh and an empty vor_entries list is merged without error.
    # ``RecursionError`` MUST NOT propagate out of the call.
    update_vor_stations.merge_into_stations(poisoned, [])

    # Verify the file was overwritten with the canonical merge schema —
    # the function always emits a ``{"stations": [...]}`` wrapped object
    # via ``atomic_write``. The list may be non-empty because
    # ``STATIC_VOR_ENTRIES`` always seeds entries, but the key
    # post-condition is that the depth-bomb did not crash the merge AND
    # the poisoned file is restored to a valid JSON shape for the next
    # run (the sibling Round-4-fixed contract).
    rewritten = json.loads(poisoned.read_text(encoding="utf-8"))
    assert isinstance(rewritten, dict)
    assert isinstance(rewritten.get("stations"), list)
    # The recovery warning was emitted (pre-fix nothing logged because
    # the error escaped uncaught).
    assert any(
        "starting VOR merge from empty state" in record.getMessage()
        for record in caplog.records
    )


def test_update_vor_merge_into_stations_handles_malformed_json(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """Regular ``json.JSONDecodeError`` was already escaping pre-fix.

    The pre-fix catch was only ``FileNotFoundError``, so any non-empty
    but malformed JSON crashed here too — not just depth-bombs. Verify
    the post-fix shape catches both failure modes via the same except
    tuple.
    """
    from scripts import update_vor_stations

    poisoned = tmp_path / "stations.json"
    poisoned.write_text("{not valid json", encoding="utf-8")
