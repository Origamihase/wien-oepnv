"""Regression tests for :mod:`scripts.update_wl_stations` with wrapped JSON."""
...
@pytest.fixture()
def stations_path_wrapped(tmp_path: Path) -> Path:
    path = tmp_path / "stations.json"
    path.write_text('{"stations": []}', encoding="utf-8")
    return path
...
def test_merge_wl_data_into_existing_wrapped_entry(stations_path_wrapped: Path) -> None:
    stations_path_wrapped.write_text(
        json.dumps(
            {
                "stations": [
                    {
    ...
    update_wl_stations.merge_into_stations(stations_path_wrapped, wl_entries)

    # Check content
    raw = json.loads(stations_path_wrapped.read_text(encoding="utf-8"))
    ...
    raw_rerun = json.loads(stations_path_wrapped.read_text(encoding="utf-8"))
    assert raw_rerun == raw


def test_unmatched_wl_entry_is_appended_wrapped(stations_path_wrapped: Path) -> None:
    wl_entries = [
        {
