...
def test_fetch_events_raises_when_all_stationboards_fail(monkeypatch):
    monkeypatch.setattr(vor, "_fetch_departure_board_for_station", lambda sid, now, counter=None, session=None, stop_event=None: None)
...
def test_fetch_events_returns_results_when_some_stationboards_succeed(monkeypatch):
    ...
    def fake_fetch(station_id: str, now, counter=None, session=None, stop_event=None):
