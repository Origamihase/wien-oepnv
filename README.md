...
> **Status:** Der historische Standardzugang `VAO` wird inzwischen vom Backend mit `API_AUTH` abgelehnt. Hinterlege daher zwingend einen gültigen Schlüssel via `VOR_ACCESS_ID` oder `VAO_ACCESS_ID`. Die Abfragen lesen das Secret vor jedem Request neu ein und hängen den Token automatisch als Query-Parameter `accessId=<Token>` an, sodass alle Requests die hinterlegte Berechtigung nutzen. Mit `scripts/check_vor_auth.py` lässt sich die Konfiguration vorab testen (siehe unten).
...
Die in der Ausgabe protokollierte URL maskiert den hinterlegten Zugangsschlüssel automatisch.
