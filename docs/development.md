...
- `update-stammstrecke-status.yml` – `workflow_dispatch`-only-Escape-Hatch für die VOR/VAO `/trip`-Abfrage; der reguläre Lauf erfolgt als Schritt in `update-cycle.yml` (~alle 30 Min auf :00/:30, IFTTT-getriggert) und schreibt die Beobachtung als CSV-Zeile in `data/stats/stammstrecke_<YYYY>.csv`.
- `generate-stats.yml` – `workflow_dispatch`-only-Escape-Hatch für eine reine Dashboard-Regeneration aus den CSV-Ledgers; die reguläre tägliche Aktualisierung von `docs/statistik.md` erfolgt in `update-cycle.yml` beim ersten Cycle-Tick nach Mitternacht Europe/Vienna.
